...
export const get_price_surv_history_list_by_tds = async (tds_code) => {
  if (!tds_code) throw new Error("TDS Code is required");

  try {
    const path = `/DB_TEST/TBL_PRICE_SURVEY_HISTORY/DATA`;
    ...
    console.error("Error in get_price_surv_history_list_by_tds:", error);
    throw error;
  }
};

export const get_price_surv_history_list_by_date = async (target_date) => {
  ...
  try {
    ...
    console.error("Error in get_price_surv_history_list_by_date:", error);
    throw error;
  }
};

...
export const get_tr_history_list_by_tds = async (tds_code) => {
  if (!tds_code) throw new Error("TDS Code is required");

  try {
    const path = `/DB2_BENBY_MERCH_APP/TBL_TAP_HISTORY/DATA`;
    ...
    console.error("Error in get_tr_history_list_by_tds:", error);
    throw error;
  }
};

export const get_tr_history_list_by_date = async (target_date) => {
  ...
  try {
    ...
    console.error("Error in get_tr_history_list_by_date:", error);
    throw error;
  }
};

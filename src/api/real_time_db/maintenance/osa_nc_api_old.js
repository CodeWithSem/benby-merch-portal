...
export const get_all_osa_nc_temp_list = async () => {
  try {
    // 1. Point to the root DATA node to get all stores
    const path = `/DB2_BENBY_MERCH_APP/TBL_OSA_NOT_CARRIED_BY_STORE/DATA`;
...
export const get_all_osa_nc_temp_id_list = async () => {
  try {
    // 1. Point to the root DATA node to get all stores
    const path = `/DB2_BENBY_MERCH_APP/TBL_OSA_NOT_CARRIED_BY_STORE/DATA`;
    const snapshot = await get(ref(realtime_db, path));
    ...
    // 2. Kukunin lang natin ang mga Keys (Parent IDs / Store Codes)
    // Object.keys(data) ay magbabalik ng array ng mga IDs (e.g., ["STORE_001", "STORE_002"])
...
// + PUSH TO CLOUD
export const push_osa_nc_temp_to_cloud = async (data, on_progress, signal) => {
  ...
  for (let i = 0; i < total_records; i += batch_size) {
    ...
    const updates = {};
    batch.forEach((item) => {
      const path = `/DB2_BENBY_MERCH_APP/TBL_OSA_NOT_CARRIED_BY_STORE/DATA/${item.storecode}/${item.matcode}`;
...
export const delete_all_osa_nc_by_batch = async () => {
  try {
    console.log("Start: Fetching all Parent IDs...");

    // 1. Call your previous function to get the list of IDs
    const parentIds = await get_all_osa_nc_temp_id_list();
    ...
    console.log(`Found ${parentIds.length} IDs. Starting batch deletion...`);

    // 2. Loop through each ID to delete them sequentially
    let count = 0;
    for (const storeId of parentIds) {
      const deletePath = `/DB2_BENBY_MERCH_APP/TBL_OSA_NOT_CARRIED_BY_STORE/DATA/${storeId}`;

      // Delete the specific store ID path
      await remove(ref(realtime_db, deletePath));

      count++;
      // Logs progress for tracking large amounts of data
      console.log(
        `[${count}/${parentIds.length}] Successfully deleted path: ${deletePath}`,
      );
    }

    console.log("Batch deletion completed successfully! All data cleared.");
    return true;
  } catch (error) {
    console.error("Error encountered during batch deletion:", error);
    throw error;
  }
};
...
//   const dataPathBase = "/DB2_BENBY_MERCH_APP/TBL_OSA_NOT_CARRIED_BY_STORE/DATA";

...
// + GET ALL
export const get_all_mcl_nerm_list = async () => {
  try {
    const path = `/DB_TEST/TBL_MCL_NERM/DATA`;
    ...
    console.error("Error in get_all_mcl_nerm_list:", error);
    throw error;
  }
};
// - GET ALL
...
// + PUSH TO CLOUD
export const push_mcl_nerm_to_cloud = async (data, on_progress, signal) => {
  ...
  for (let i = 0; i < total_records; i += batch_size) {
    ...
    batch.forEach((item) => {
      ...
      // 1. DATA PATH
      const path = `/DB_TEST/TBL_MCL_NERM/DATA/${item.channel}/${item.tagging}/${sanitizedPosition}/${item.matcode}`;

      // 2. REGISTRY PATH (To align with your OSA NC logic)
      const registryPath = `/DB_DELETE_PATH/TBL_MCL_NERM/DATA/${item.channel}`;
...
// + TRUNCATE
export const truncate_mcl_nerm = async (on_progress = null) => {
  const registryPath = "/DB_DELETE_PATH/TBL_MCL_NERM/DATA";
  const dataPathBase = "/DB_TEST/TBL_MCL_NERM/DATA";

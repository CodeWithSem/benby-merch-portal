...
// - [Get]
// + [Create]
export const api_create_data_name = async (new_data, user) => {
  try {
    const tbl_data_name_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TABLE_NAME)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_data_name = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_data_name_ref, updated_edit_data);

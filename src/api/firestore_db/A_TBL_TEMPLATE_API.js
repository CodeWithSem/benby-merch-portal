...
// + [Get]
export const api_get_data_name_list = async () => {
  try {
    const tbl_data_name_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TABLE_NAME)
    );

    const query_snapshot = await getDocs(tbl_data_name_ref);
...
// - [Get]
// + [Create]
export const api_create_data_name = async (new_data, user, show_toast) => {
  try {
    const tbl_data_name_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TABLE_NAME)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE data_name_code
    // ---------------------------------------------
    const q_code = query(
      tbl_data_name_ref,
      where("data_name_code", "==", new_data.data_name_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE data_name_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_data_name_ref,
      where("data_name_desc", "==", new_data.data_name_desc)
    );
    ...
    const doc_ref = doc(tbl_data_name_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_data_name_increment(new_data.id);
...
export const api_update_data_name_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_data_name_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TABLE_NAME)
    );

    await set(tbl_data_name_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_data_name = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.TABLE_NAME);
    const tbl_data_name_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE data_name_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_data_name_ref,
      where("data_name_desc", "==", edit_data.data_name_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_data_name = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_data_name_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_data_name = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_data_name_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_data_name = async () => {
  try {
    const tbl_data_name_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TABLE_NAME)
    ...
    const snapshot = await getDocs(tbl_data_name_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_data_name_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_data_name_increment();
...
export const api_reset_data_name_increment = async () => {
  try {
    const tbl_data_name_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TABLE_NAME)
    ...
    await set(tbl_data_name_incre_ref, 1);
...
export const api_set_data_name_increment = async (new_id) => {

...
// - [Get]
// + [Create]
export const api_create_app_matrix = async (new_data, user) => {
  try {
    const tbl_app_matrix_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.APPROVAL_MATRIX)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_app_matrix = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_app_matrix_ref, updated_edit_data);

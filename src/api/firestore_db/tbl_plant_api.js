...
// - [Get]
// + [Create]
export const api_create_plant = async (new_data, user) => {
  try {
    const tbl_plant_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PLANT)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_plant = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_plant_ref, updated_edit_data);

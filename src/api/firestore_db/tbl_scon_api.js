...
// - [Get]
// + [Create]
export const api_create_scon = async (new_data, user) => {
  try {
    const tbl_scon_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_CONDITION)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_scon = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_scon_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
      "TBL_STORAGE_CONDITION",
      "DATA",
      String(edit_data.id)
    );
    ...
    await setDoc(tbl_scon_ref, updated_edit_data);

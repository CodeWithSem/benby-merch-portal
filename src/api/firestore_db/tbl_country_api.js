...
// - [Get]
// + [Create]
export const api_create_country = async (new_data, user) => {
  try {
    const tbl_country_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.COUNTRY)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_country = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_country_ref, updated_edit_data);

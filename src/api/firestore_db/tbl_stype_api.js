...
// - [Get]
// + [Create]
export const api_create_stype = async (new_data, user) => {
  try {
    const tbl_stype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_TYPE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_stype = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_stype_ref, updated_edit_data);

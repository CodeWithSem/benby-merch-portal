...
// - [Get]
// + [Create]
export const api_create_po_type = async (new_data, user) => {
  try {
    const tbl_po_type_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASE_ORDER_TYPE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_po_type = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_po_type_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
      "TBL_PURCHASE_ORDER_TYPE",
      "DATA",
      String(edit_data.id)
    );
    ...
    await setDoc(tbl_po_type_ref, updated_edit_data);

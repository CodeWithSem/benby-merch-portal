// Data Name: inv_acc_center
// Table Name: INV_ACC_CENTER
...
// + [Get]
export const api_get_inv_acc_center_list = async () => {
  try {
    const tbl_inv_acc_center_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.INV_ACC_CENTER)
    );

    const query_snapshot = await getDocs(tbl_inv_acc_center_ref);
...
// - [Get]
// + [Create]
export const api_create_inv_acc_center = async (new_data, user, show_toast) => {
  try {
    const tbl_inv_acc_center_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.INV_ACC_CENTER)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE inv_acc_center_code
    // ---------------------------------------------
    const q_code = query(
      tbl_inv_acc_center_ref,
      where("inv_acc_center_code", "==", new_data.inv_acc_center_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE inv_acc_center_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_inv_acc_center_ref,
      where("inv_acc_center_desc", "==", new_data.inv_acc_center_desc)
    );
    ...
    const doc_ref = doc(tbl_inv_acc_center_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_inv_acc_center_increment(new_data.id);
...
export const api_update_inv_acc_center_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_inv_acc_center_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.INV_ACC_CENTER)
    );

    await set(tbl_inv_acc_center_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_inv_acc_center = async (
  edit_data,
  user,
  show_toast
) => {
  try {
    ...
    const tbl_path = get_firestore_path(TABLES.INV_ACC_CENTER);
    const tbl_inv_acc_center_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE inv_acc_center_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_inv_acc_center_ref,
      where("inv_acc_center_desc", "==", edit_data.inv_acc_center_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_inv_acc_center = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_inv_acc_center_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_inv_acc_center = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_inv_acc_center_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_inv_acc_center = async () => {
  try {
    const tbl_inv_acc_center_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.INV_ACC_CENTER)
    ...
    const snapshot = await getDocs(tbl_inv_acc_center_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_inv_acc_center_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_inv_acc_center_increment();
...
export const api_reset_inv_acc_center_increment = async () => {
  try {
    const tbl_inv_acc_center_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.INV_ACC_CENTER)
    ...
    await set(tbl_inv_acc_center_incre_ref, 1);
...
export const api_set_inv_acc_center_increment = async (new_id) => {

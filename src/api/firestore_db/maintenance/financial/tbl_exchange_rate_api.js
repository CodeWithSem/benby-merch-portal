...
// + [Get]
export const api_get_exchange_rate_list = async () => {
  try {
    const tbl_exchange_rate_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.EXCHANGE_RATE)
    );

    const query_snapshot = await getDocs(tbl_exchange_rate_ref);
...
// - [Get]
// + [Create]
export const api_create_exchange_rate = async (new_data, user, show_toast) => {
  try {
    const tbl_exchange_rate_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.EXCHANGE_RATE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE exchange_rate_code
    // ---------------------------------------------
    const q_code = query(
      tbl_exchange_rate_ref,
      where("exchange_rate_code", "==", new_data.exchange_rate_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE exchange_rate_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_exchange_rate_ref,
      where("exchange_rate_desc", "==", new_data.exchange_rate_desc)
    );
    ...
    const doc_ref = doc(tbl_exchange_rate_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_exchange_rate_increment(new_data.id);
...
export const api_update_exchange_rate_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_exchange_rate_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.EXCHANGE_RATE)
    );

    await set(tbl_exchange_rate_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_exchange_rate = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.EXCHANGE_RATE);
    const tbl_exchange_rate_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE exchange_rate_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_exchange_rate_ref,
      where("exchange_rate_desc", "==", edit_data.exchange_rate_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_exchange_rate = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_exchange_rate_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_exchange_rate = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_exchange_rate_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_exchange_rate = async () => {
  try {
    const tbl_exchange_rate_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.EXCHANGE_RATE)
    ...
    const snapshot = await getDocs(tbl_exchange_rate_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_exchange_rate_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_exchange_rate_increment();
...
export const api_reset_exchange_rate_increment = async () => {
  try {
    const tbl_exchange_rate_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.EXCHANGE_RATE)
    ...
    await set(tbl_exchange_rate_incre_ref, 1);
...
export const api_set_exchange_rate_increment = async (new_id) => {

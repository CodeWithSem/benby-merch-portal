...
// - [Get]
// + [Create]
export const api_create_taxation = async (new_data, user) => {
  try {
    const tbl_taxation_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TAXATION)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_taxation = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_taxation_ref, updated_edit_data);

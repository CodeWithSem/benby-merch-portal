...
// + [Get]
export const api_get_taxation_list = async () => {
  try {
    const tbl_taxation_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TAXATION)
    );

    const query_snapshot = await getDocs(tbl_taxation_ref);
...
// - [Get]
// + [Create]
export const api_create_taxation = async (new_data, user, show_toast) => {
  try {
    const tbl_taxation_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TAXATION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE taxation_code
    // ---------------------------------------------
    const q_code = query(
      tbl_taxation_ref,
      where("taxation_code", "==", new_data.taxation_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE taxation_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_taxation_ref,
      where("taxation_desc", "==", new_data.taxation_desc)
    );
    ...
    const doc_ref = doc(tbl_taxation_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_taxation_increment(new_data.id);
...
export const api_update_taxation_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_taxation_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TAXATION)
    );

    await set(tbl_taxation_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_taxation = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.TAXATION);
    const tbl_taxation_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE taxation_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_taxation_ref,
      where("taxation_desc", "==", edit_data.taxation_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_taxation = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_taxation_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_taxation = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_taxation_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_taxation = async () => {
  try {
    const tbl_taxation_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TAXATION)
    ...
    const snapshot = await getDocs(tbl_taxation_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_taxation_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_taxation_increment();
...
export const api_reset_taxation_increment = async () => {
  try {
    const tbl_taxation_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TAXATION)
    ...
    await set(tbl_taxation_incre_ref, 1);
...
export const api_set_taxation_increment = async (new_id) => {

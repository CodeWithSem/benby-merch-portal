...
// + [Get]
export const api_get_payment_method_list = async () => {
  try {
    const tbl_payment_method_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PAYMENT_METHOD)
    );

    const query_snapshot = await getDocs(tbl_payment_method_ref);
...
// - [Get]
// + [Create]
export const api_create_payment_method = async (new_data, user, show_toast) => {
  try {
    const tbl_payment_method_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PAYMENT_METHOD)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE payment_method_code
    // ---------------------------------------------
    const q_code = query(
      tbl_payment_method_ref,
      where("payment_method_code", "==", new_data.payment_method_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE payment_method_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_payment_method_ref,
      where("payment_method_desc", "==", new_data.payment_method_desc)
    );
    ...
    const doc_ref = doc(tbl_payment_method_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_payment_method_increment(new_data.id);
...
export const api_update_payment_method_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_payment_method_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PAYMENT_METHOD)
    );

    await set(tbl_payment_method_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_payment_method = async (
  edit_data,
  user,
  show_toast
) => {
  try {
    ...
    const tbl_path = get_firestore_path(TABLES.PAYMENT_METHOD);
    const tbl_payment_method_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE payment_method_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_payment_method_ref,
      where("payment_method_desc", "==", edit_data.payment_method_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_payment_method = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_payment_method_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_payment_method = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_payment_method_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_payment_method = async () => {
  try {
    const tbl_payment_method_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PAYMENT_METHOD)
    ...
    const snapshot = await getDocs(tbl_payment_method_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_payment_method_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_payment_method_increment();
...
export const api_reset_payment_method_increment = async () => {
  try {
    const tbl_payment_method_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PAYMENT_METHOD)
    ...
    await set(tbl_payment_method_incre_ref, 1);
...
export const api_set_payment_method_increment = async (new_id) => {

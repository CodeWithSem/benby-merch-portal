...
// + [Get]
export const api_get_payment_term_list = async () => {
  try {
    const tbl_payment_term_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PAYMENT_TERM)
    );

    const query_snapshot = await getDocs(tbl_payment_term_ref);
...
// - [Get]
// + [Create]
export const api_create_payment_term = async (new_data, user, show_toast) => {
  try {
    const tbl_payment_term_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PAYMENT_TERM)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE payment_term_code
    // ---------------------------------------------
    const q_code = query(
      tbl_payment_term_ref,
      where("payment_term_code", "==", new_data.payment_term_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE payment_term_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_payment_term_ref,
      where("payment_term_desc", "==", new_data.payment_term_desc)
    );
    ...
    const doc_ref = doc(tbl_payment_term_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_payment_term_increment(new_data.id);
...
export const api_update_payment_term_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_payment_term_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PAYMENT_TERM)
    );

    await set(tbl_payment_term_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_payment_term = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.PAYMENT_TERM);
    const tbl_payment_term_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE payment_term_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_payment_term_ref,
      where("payment_term_desc", "==", edit_data.payment_term_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_payment_term = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_payment_term_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_payment_term = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_payment_term_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_payment_term = async () => {
  try {
    const tbl_payment_term_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PAYMENT_TERM)
    ...
    const snapshot = await getDocs(tbl_payment_term_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_payment_term_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_payment_term_increment();
...
export const api_reset_payment_term_increment = async () => {
  try {
    const tbl_payment_term_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PAYMENT_TERM)
    ...
    await set(tbl_payment_term_incre_ref, 1);
...
export const api_set_payment_term_increment = async (new_id) => {

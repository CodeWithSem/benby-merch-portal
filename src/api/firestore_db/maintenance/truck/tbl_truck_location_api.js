...
// + [Get]
export const api_get_truck_location_list = async () => {
  try {
    const tbl_truck_location_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TRUCK_LOCATION)
    );

    const query_snapshot = await getDocs(tbl_truck_location_ref);
...
// - [Get]
// + [Create]
export const api_create_truck_location = async (new_data, user, show_toast) => {
  try {
    const tbl_truck_location_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TRUCK_LOCATION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE truck_location_code
    // ---------------------------------------------
    const q_code = query(
      tbl_truck_location_ref,
      where("truck_location_code", "==", new_data.truck_location_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE truck_location_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_truck_location_ref,
      where("truck_location_desc", "==", new_data.truck_location_desc)
    );
    ...
    const doc_ref = doc(tbl_truck_location_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_truck_location_increment(new_data.id);
...
export const api_update_truck_location_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_truck_location_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TRUCK_LOCATION)
    );

    await set(tbl_truck_location_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_truck_location = async (
  edit_data,
  user,
  show_toast
) => {
  try {
    ...
    const tbl_path = get_firestore_path(TABLES.TRUCK_LOCATION);
    const tbl_truck_location_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE truck_location_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_truck_location_ref,
      where("truck_location_desc", "==", edit_data.truck_location_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_truck_location = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_truck_location_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_truck_location = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_truck_location_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_truck_location = async () => {
  try {
    const tbl_truck_location_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TRUCK_LOCATION)
    ...
    const snapshot = await getDocs(tbl_truck_location_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_truck_location_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_truck_location_increment();
...
export const api_reset_truck_location_increment = async () => {
  try {
    const tbl_truck_location_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TRUCK_LOCATION)
    ...
    await set(tbl_truck_location_incre_ref, 1);
...
export const api_set_truck_location_increment = async (new_id) => {

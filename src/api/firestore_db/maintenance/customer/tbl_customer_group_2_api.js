...
// + [Get]
export const api_get_customer_group_2_list = async () => {
  try {
    const tbl_customer_group_2_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.CUSTOMER_GROUP_2)
    );

    const query_snapshot = await getDocs(tbl_customer_group_2_ref);
...
// - [Get]
// + [Create]
export const api_create_customer_group_2 = async (
  new_data,
  user,
  show_toast
) => {
  try {
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE customer_group_2_code
    // ---------------------------------------------
    const q_code = query(
      tbl_customer_group_2_ref,
      where("customer_group_2_code", "==", new_data.customer_group_2_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE customer_group_2_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_customer_group_2_ref,
      where("customer_group_2_desc", "==", new_data.customer_group_2_desc)
    );
    ...
    const doc_ref = doc(tbl_customer_group_2_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_customer_group_2_increment(new_data.id);
...
export const api_update_customer_group_2_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_customer_group_2_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.CUSTOMER_GROUP_2)
    );

    await set(tbl_customer_group_2_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_customer_group_2 = async (
  edit_data,
  user,
  show_toast
) => {
  try {
    ...
    const tbl_path = get_firestore_path(TABLES.CUSTOMER_GROUP_2);
    const tbl_customer_group_2_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE customer_group_2_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_customer_group_2_ref,
      where("customer_group_2_desc", "==", edit_data.customer_group_2_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_customer_group_2 = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_customer_group_2_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_customer_group_2 = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_customer_group_2_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_customer_group_2 = async () => {
  try {
    const tbl_customer_group_2_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.CUSTOMER_GROUP_2)
    ...
    const snapshot = await getDocs(tbl_customer_group_2_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_customer_group_2_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_customer_group_2_increment();
...
export const api_reset_customer_group_2_increment = async () => {
  try {
    const tbl_customer_group_2_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.CUSTOMER_GROUP_2)
    ...
    await set(tbl_customer_group_2_incre_ref, 1);
...
export const api_set_customer_group_2_increment = async (new_id) => {

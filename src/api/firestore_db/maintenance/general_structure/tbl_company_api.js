...
// - [Get]
// + [Create]
export const api_create_company = async (new_data, user, show_toast) => {
  try {
    const tbl_company_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.COMPANY)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE company_code
    // ---------------------------------------------
    const q_code = query(
      tbl_company_ref,
      where("company_code", "==", new_data.company_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE company_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_company_ref,
      where("company_desc", "==", new_data.company_desc)
    );
...
// - [Update Incremental ID]
// + [Update]
export const api_update_company = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.COMPANY);
    const tbl_company_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE company_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_company_ref,
      where("company_desc", "==", edit_data.company_desc)
    );
...
export const api_set_company_increment = async (new_id) => {

...
// - [Get]
// + [Create]
export const api_create_currency = async (new_data, user, show_toast) => {
  try {
    const tbl_currency_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.CURRENCY)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE currency_code
    // ---------------------------------------------
    const q_code = query(
      tbl_currency_ref,
      where("currency_code", "==", new_data.currency_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE currency_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_currency_ref,
      where("currency_desc", "==", new_data.currency_desc)
    );
...
// - [Update Incremental ID]
// + [Update]
export const api_update_currency = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.CURRENCY);
    const tbl_currency_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE currency_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_currency_ref,
      where("currency_desc", "==", edit_data.currency_desc)
    );
...
export const api_set_currency_increment = async (new_id) => {

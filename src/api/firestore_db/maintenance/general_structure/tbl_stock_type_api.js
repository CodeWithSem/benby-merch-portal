...
// - [Get]
// + [Create]
export const api_create_stock_type = async (new_data, user) => {
  try {
    const tbl_stock_type_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STOCK_TYPE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_stock_type = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_stock_type_ref, updated_edit_data);

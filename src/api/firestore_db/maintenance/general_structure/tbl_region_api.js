...
// + [Get]
export const api_get_region_list = async () => {
  try {
    const tbl_region_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.REGION)
    );

    const query_snapshot = await getDocs(tbl_region_ref);
...
// - [Get]
// + [Create]
export const api_create_region = async (new_data, user, show_toast) => {
  try {
    const tbl_region_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.REGION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE region_code
    // ---------------------------------------------
    const q_code = query(
      tbl_region_ref,
      where("region_code", "==", new_data.region_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE region_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_region_ref,
      where("region_desc", "==", new_data.region_desc)
    );
    ...
    const doc_ref = doc(tbl_region_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_region_increment(new_data.id);
...
export const api_update_region_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_region_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.REGION)
    );

    await set(tbl_region_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_region = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.REGION);
    const tbl_region_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE region_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_region_ref,
      where("region_desc", "==", edit_data.region_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_region = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_region_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_region = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_region_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_region = async () => {
  try {
    const tbl_region_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.REGION)
    ...
    const snapshot = await getDocs(tbl_region_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_region_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_region_increment();
...
export const api_reset_region_increment = async () => {
  try {
    const tbl_region_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.REGION)
    ...
    await set(tbl_region_incre_ref, 1);
...
export const api_set_region_increment = async (new_id) => {

...
// + [Get]
export const api_get_uom_list = async () => {
  try {
    const tbl_uom_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.UNIT_OF_MEASURE)
    );

    const query_snapshot = await getDocs(tbl_uom_ref);
...
// - [Get]
// + [Create]
export const api_create_uom = async (new_data, user, show_toast) => {
  try {
    const tbl_uom_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.UNIT_OF_MEASURE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE uom_code
    // ---------------------------------------------
    const q_code = query(
      tbl_uom_ref,
      where("uom_code", "==", new_data.uom_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE uom_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_uom_ref,
      where("uom_desc", "==", new_data.uom_desc)
    );
    ...
    const doc_ref = doc(tbl_uom_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_uom_increment(new_data.id);
...
export const api_update_uom_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_uom_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.UNIT_OF_MEASURE)
    );

    await set(tbl_uom_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_uom = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.UNIT_OF_MEASURE);
    const tbl_uom_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE uom_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_uom_ref,
      where("uom_desc", "==", edit_data.uom_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_uom = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_uom_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_uom = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_uom_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_uom = async () => {
  try {
    const tbl_uom_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.UNIT_OF_MEASURE)
    ...
    const snapshot = await getDocs(tbl_uom_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_uom_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_uom_increment();
...
export const api_reset_uom_increment = async () => {
  try {
    const tbl_uom_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.UNIT_OF_MEASURE)
...
export const api_set_uom_increment = async (new_id) => {

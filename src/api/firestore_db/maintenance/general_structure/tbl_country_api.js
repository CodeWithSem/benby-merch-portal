...
// + [Get]
export const api_get_country_list = async () => {
  try {
    const tbl_country_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.COUNTRY)
    );

    const query_snapshot = await getDocs(tbl_country_ref);
...
// - [Get]
// + [Create]
export const api_create_country = async (new_data, user, show_toast) => {
  try {
    const tbl_country_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.COUNTRY)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE country_code
    // ---------------------------------------------
    const q_code = query(
      tbl_country_ref,
      where("country_code", "==", new_data.country_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE country_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_country_ref,
      where("country_desc", "==", new_data.country_desc)
    );
    ...
    const doc_ref = doc(tbl_country_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_country_increment(new_data.id);
...
export const api_update_country_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_country_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.COUNTRY)
    );

    await set(tbl_country_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_country = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.COUNTRY);
    const tbl_country_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE country_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_country_ref,
      where("country_desc", "==", edit_data.country_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_country = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_country_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_country = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_country_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_country = async () => {
  try {
    const tbl_country_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.COUNTRY)
    ...
    const snapshot = await getDocs(tbl_country_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_country_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_country_increment();
...
export const api_reset_country_increment = async () => {
  try {
    const tbl_country_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.COUNTRY)
    ...
    await set(tbl_country_incre_ref, 1);
...
export const api_set_country_increment = async (new_id) => {

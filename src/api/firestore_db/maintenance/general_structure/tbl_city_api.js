...
// - [Get]
// + [Create]
export const api_create_city = async (new_data, user, show_toast) => {
  try {
    const tbl_city_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.CITY)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE city_code
    // ---------------------------------------------
    const q_code = query(
      tbl_city_ref,
      where("city_code", "==", new_data.city_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE city_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_city_ref,
      where("city_desc", "==", new_data.city_desc)
    );
...
// - [Update Incremental ID]
// + [Update]
export const api_update_city = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.CITY);
    const tbl_city_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE city_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_city_ref,
      where("city_desc", "==", edit_data.city_desc)
    );
...
export const api_set_city_increment = async (new_id) => {

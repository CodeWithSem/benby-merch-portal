...
// + [Get]
export const api_get_purc_org_list = async () => {
  try {
    const tbl_purc_org_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASING_ORG)
    );

    const query_snapshot = await getDocs(tbl_purc_org_ref);
...
// - [Get]
// + [Create]
export const api_create_purc_org = async (new_data, user, show_toast) => {
  try {
    const tbl_purc_org_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASING_ORG)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE purc_org_code
    // ---------------------------------------------
    const q_code = query(
      tbl_purc_org_ref,
      where("purc_org_code", "==", new_data.purc_org_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE purc_org_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_purc_org_ref,
      where("purc_org_desc", "==", new_data.purc_org_desc)
    );
    ...
    const doc_ref = doc(tbl_purc_org_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_purc_org_increment(new_data.id);
...
export const api_update_purc_org_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_purc_org_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PURCHASING_ORG)
    );

    await set(tbl_purc_org_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_purc_org = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.PURCHASING_ORG);
    const tbl_purc_org_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE purc_org_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_purc_org_ref,
      where("purc_org_desc", "==", edit_data.purc_org_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_purc_org = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_purc_org_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_purc_org = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_purc_org_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_purc_org = async () => {
  try {
    const tbl_purc_org_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASING_ORG)
    ...
    const snapshot = await getDocs(tbl_purc_org_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_purc_org_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_purc_org_increment();
...
export const api_reset_purc_org_increment = async () => {
  try {
    const tbl_purc_org_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PURCHASING_ORG)
    ...
    await set(tbl_purc_org_incre_ref, 1);
...
export const api_set_purc_org_increment = async (new_id) => {

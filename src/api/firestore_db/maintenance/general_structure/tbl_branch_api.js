...
// + [Get]
export const api_get_branch_list = async () => {
  try {
    const tbl_branch_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BRANCH)
    );

    const query_snapshot = await getDocs(tbl_branch_ref);
...
// - [Get]
// + [Create]
export const api_create_branch = async (new_data, user, show_toast) => {
  try {
    const tbl_branch_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BRANCH)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE branch_code
    // ---------------------------------------------
    const q_code = query(
      tbl_branch_ref,
      where("branch_code", "==", new_data.branch_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE branch_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_branch_ref,
      where("branch_desc", "==", new_data.branch_desc)
    );
    ...
    const doc_ref = doc(tbl_branch_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_branch_increment(new_data.id);
...
export const api_update_branch_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_branch_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.BRANCH)
    );

    await set(tbl_branch_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_branch = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.BRANCH);
    const tbl_branch_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE branch_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_branch_ref,
      where("branch_desc", "==", edit_data.branch_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_branch = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_branch_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_branch = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_branch_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_branch = async () => {
  try {
    const tbl_branch_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BRANCH)
    ...
    const snapshot = await getDocs(tbl_branch_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_branch_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_branch_increment();
...
export const api_reset_branch_increment = async () => {
  try {
    const tbl_branch_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.BRANCH)
    ...
    await set(tbl_branch_incre_ref, 1);
...
export const api_set_branch_increment = async (new_id) => {

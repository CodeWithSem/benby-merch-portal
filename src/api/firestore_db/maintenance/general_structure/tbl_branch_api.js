...
// - [Get]
// + [Create]
export const api_create_branch = async (new_data, user) => {
  try {
    const tbl_branch_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BRANCH)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_branch = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_branch_ref, updated_edit_data);

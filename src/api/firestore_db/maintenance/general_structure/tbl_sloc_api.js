...
// + [Get]
export const api_get_sloc_list = async () => {
  try {
    const tbl_sloc_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_LOCATION)
    );

    const query_snapshot = await getDocs(tbl_sloc_ref);
...
// - [Get]
// + [Create]
export const api_create_sloc = async (new_data, user, show_toast) => {
  try {
    const tbl_sloc_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_LOCATION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE sloc_code
    // ---------------------------------------------
    const q_code = query(
      tbl_sloc_ref,
      where("sloc_code", "==", new_data.sloc_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE sloc_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_sloc_ref,
      where("sloc_desc", "==", new_data.sloc_desc)
    );
    ...
    const doc_ref = doc(tbl_sloc_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_sloc_increment(new_data.id);
...
export const api_update_sloc_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_sloc_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_LOCATION)
    );

    await set(tbl_sloc_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sloc = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_LOCATION);
    const tbl_sloc_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE sloc_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_sloc_ref,
      where("sloc_desc", "==", edit_data.sloc_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_sloc = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_sloc_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_sloc = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_sloc_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_sloc = async () => {
  try {
    const tbl_sloc_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_LOCATION)
    ...
    const snapshot = await getDocs(tbl_sloc_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_sloc_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_sloc_increment();
...
export const api_reset_sloc_increment = async () => {
  try {
    const tbl_sloc_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_LOCATION)
    ...
    await set(tbl_sloc_incre_ref, 1);
...
export const api_set_sloc_increment = async (new_id) => {

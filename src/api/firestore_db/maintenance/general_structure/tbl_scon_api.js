...
// + [Get]
export const api_get_scon_list = async () => {
  try {
    const tbl_scon_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_CONDITION)
    );

    const query_snapshot = await getDocs(tbl_scon_ref);
...
// - [Get]
// + [Create]
export const api_create_scon = async (new_data, user, show_toast) => {
  try {
    const tbl_scon_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_CONDITION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE scon_code
    // ---------------------------------------------
    const q_code = query(
      tbl_scon_ref,
      where("scon_code", "==", new_data.scon_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE scon_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_scon_ref,
      where("scon_desc", "==", new_data.scon_desc)
    );
    ...
    const doc_ref = doc(tbl_scon_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_scon_increment(new_data.id);
...
export const api_update_scon_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_scon_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_CONDITION)
    );

    await set(tbl_scon_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_scon = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_CONDITION);
    const tbl_scon_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE scon_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_scon_ref,
      where("scon_desc", "==", edit_data.scon_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_scon = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_scon_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_scon = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_scon_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_scon = async () => {
  try {
    const tbl_scon_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_CONDITION)
    ...
    const snapshot = await getDocs(tbl_scon_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_scon_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_scon_increment();
...
export const api_reset_scon_increment = async () => {
  try {
    const tbl_scon_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_CONDITION)
    ...
    await set(tbl_scon_incre_ref, 1);
...
export const api_set_scon_increment = async (new_id) => {

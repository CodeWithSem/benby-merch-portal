...
// + [Get]
export const api_get_language_list = async () => {
  try {
    const tbl_language_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.LANGUAGE)
    );

    const query_snapshot = await getDocs(tbl_language_ref);
...
// - [Get]
// + [Create]
export const api_create_language = async (new_data, user, show_toast) => {
  try {
    const tbl_language_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.LANGUAGE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE language_code
    // ---------------------------------------------
    const q_code = query(
      tbl_language_ref,
      where("language_code", "==", new_data.language_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE language_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_language_ref,
      where("language_desc", "==", new_data.language_desc)
    );
    ...
    const doc_ref = doc(tbl_language_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_language_increment(new_data.id);
...
export const api_update_language_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_language_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.LANGUAGE)
    );

    await set(tbl_language_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_language = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.LANGUAGE);
    const tbl_language_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE language_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_language_ref,
      where("language_desc", "==", edit_data.language_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_language = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_language_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_language = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_language_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_language = async () => {
  try {
    const tbl_language_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.LANGUAGE)
    ...
    const snapshot = await getDocs(tbl_language_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_language_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_language_increment();
...
export const api_reset_language_increment = async () => {
  try {
    const tbl_language_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.LANGUAGE)
    ...
    await set(tbl_language_incre_ref, 1);
...
export const api_set_language_increment = async (new_id) => {

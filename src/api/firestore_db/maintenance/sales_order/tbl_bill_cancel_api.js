...
// Table Name: BILLING_CANCELLATION
...
// + [Get]
export const api_get_bill_cancel_list = async () => {
  try {
    const tbl_bill_cancel_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BILLING_CANCELLATION)
    );

    const query_snapshot = await getDocs(tbl_bill_cancel_ref);
...
// - [Get]
// + [Create]
export const api_create_bill_cancel = async (new_data, user, show_toast) => {
  try {
    const tbl_bill_cancel_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BILLING_CANCELLATION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE bill_cancel_code
    // ---------------------------------------------
    const q_code = query(
      tbl_bill_cancel_ref,
      where("bill_cancel_code", "==", new_data.bill_cancel_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE bill_cancel_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_bill_cancel_ref,
      where("bill_cancel_desc", "==", new_data.bill_cancel_desc)
    );
    ...
    const doc_ref = doc(tbl_bill_cancel_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_bill_cancel_increment(new_data.id);
...
export const api_update_bill_cancel_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_bill_cancel_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.BILLING_CANCELLATION)
    );

    await set(tbl_bill_cancel_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_bill_cancel = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.BILLING_CANCELLATION);
    const tbl_bill_cancel_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE bill_cancel_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_bill_cancel_ref,
      where("bill_cancel_desc", "==", edit_data.bill_cancel_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_bill_cancel = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_bill_cancel_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_bill_cancel = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_bill_cancel_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_bill_cancel = async () => {
  try {
    const tbl_bill_cancel_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BILLING_CANCELLATION)
    ...
    const snapshot = await getDocs(tbl_bill_cancel_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_bill_cancel_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_bill_cancel_increment();
...
export const api_reset_bill_cancel_increment = async () => {
  try {
    const tbl_bill_cancel_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.BILLING_CANCELLATION)
    ...
    await set(tbl_bill_cancel_incre_ref, 1);
...
export const api_set_bill_cancel_increment = async (new_id) => {

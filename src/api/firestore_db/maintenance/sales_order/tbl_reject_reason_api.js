...
// - [Get]
// + [Create]
export const api_create_reject_reason = async (new_data, user) => {
  try {
    const tbl_reject_reason_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.REJECTION_REASON)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_reject_reason = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_reject_reason_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
      "TBL_REJECTION_REASON",
      "DATA",
      String(edit_data.id)
    );
    ...
    await setDoc(tbl_reject_reason_ref, updated_edit_data);

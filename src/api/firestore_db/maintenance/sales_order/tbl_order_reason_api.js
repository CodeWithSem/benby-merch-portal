...
// + [Get]
export const api_get_order_reason_list = async () => {
  try {
    const tbl_order_reason_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ORDERING_REASON)
    );

    const query_snapshot = await getDocs(tbl_order_reason_ref);
...
// - [Get]
// + [Create]
export const api_create_order_reason = async (new_data, user, show_toast) => {
  try {
    const tbl_order_reason_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ORDERING_REASON)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE order_reason_code
    // ---------------------------------------------
    const q_code = query(
      tbl_order_reason_ref,
      where("order_reason_code", "==", new_data.order_reason_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE order_reason_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_order_reason_ref,
      where("order_reason_desc", "==", new_data.order_reason_desc)
    );
    ...
    const doc_ref = doc(tbl_order_reason_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_order_reason_increment(new_data.id);
...
export const api_update_order_reason_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_order_reason_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ORDERING_REASON)
    );

    await set(tbl_order_reason_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_order_reason = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.ORDERING_REASON);
    const tbl_order_reason_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE order_reason_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_order_reason_ref,
      where("order_reason_desc", "==", edit_data.order_reason_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_order_reason = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_order_reason_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_order_reason = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_order_reason_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_order_reason = async () => {
  try {
    const tbl_order_reason_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ORDERING_REASON)
    ...
    const snapshot = await getDocs(tbl_order_reason_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_order_reason_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_order_reason_increment();
...
export const api_reset_order_reason_increment = async () => {
  try {
    const tbl_order_reason_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ORDERING_REASON)
    ...
    await set(tbl_order_reason_incre_ref, 1);
...
export const api_set_order_reason_increment = async (new_id) => {

...
export const api_get_so_type_list = async () => {
  try {
    ...
    const query_snapshot = await getDocs(tbl_so_type_ref);
...
export const api_create_so_type = async (new_data, user) => {
  try {
    ...
    const doc_ref = doc(tbl_so_type_ref, String(new_data.id));
...
export const api_update_so_type_increment = async (id) => {
  const new_id = id + 1;
  try {
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_so_type = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_so_type_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_so_type = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_so_type_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_so_type = async () => {
  try {
    const tbl_so_type_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SALES_ORDER_TYPE)
    ...
    const snapshot = await getDocs(tbl_so_type_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_so_type_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_so_type_increment();
...
export const api_reset_so_type_increment = async () => {
  try {
    const tbl_so_type_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.SALES_ORDER_TYPE)
    ...
    await set(tbl_so_type_incre_ref, 1);

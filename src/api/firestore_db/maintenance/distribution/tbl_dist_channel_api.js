...
// + [Get]
export const api_get_dist_channel_list = async () => {
  try {
    const tbl_dist_channel_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.DISTRIBUTION_CHANNEL)
    );

    const query_snapshot = await getDocs(tbl_dist_channel_ref);
...
// - [Get]
// + [Create]
export const api_create_dist_channel = async (new_data, user, show_toast) => {
  try {
    const tbl_dist_channel_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.DISTRIBUTION_CHANNEL)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE dist_channel_code
    // ---------------------------------------------
    const q_code = query(
      tbl_dist_channel_ref,
      where("dist_channel_code", "==", new_data.dist_channel_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE dist_channel_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_dist_channel_ref,
      where("dist_channel_desc", "==", new_data.dist_channel_desc)
    );
    ...
    const doc_ref = doc(tbl_dist_channel_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_dist_channel_increment(new_data.id);
...
export const api_update_dist_channel_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_dist_channel_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.DISTRIBUTION_CHANNEL)
    );

    await set(tbl_dist_channel_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_dist_channel = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.DISTRIBUTION_CHANNEL);
    const tbl_dist_channel_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE dist_channel_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_dist_channel_ref,
      where("dist_channel_desc", "==", edit_data.dist_channel_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_dist_channel = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_dist_channel_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_dist_channel = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_dist_channel_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_dist_channel = async () => {
  try {
    const tbl_dist_channel_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.DISTRIBUTION_CHANNEL)
    ...
    const snapshot = await getDocs(tbl_dist_channel_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_dist_channel_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_dist_channel_increment();
...
export const api_reset_dist_channel_increment = async () => {
  try {
    const tbl_dist_channel_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.DISTRIBUTION_CHANNEL)
    ...
    await set(tbl_dist_channel_incre_ref, 1);
...
export const api_set_dist_channel_increment = async (new_id) => {

...
// + [Get]
export const api_get_sales_office_list = async () => {
  try {
    const tbl_sales_office_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SALES_OFFICE)
    );

    const query_snapshot = await getDocs(tbl_sales_office_ref);
...
// - [Get]
// + [Create]
export const api_create_sales_office = async (new_data, user, show_toast) => {
  try {
    const tbl_sales_office_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SALES_OFFICE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE sales_office_code
    // ---------------------------------------------
    const q_code = query(
      tbl_sales_office_ref,
      where("sales_office_code", "==", new_data.sales_office_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE sales_office_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_sales_office_ref,
      where("sales_office_desc", "==", new_data.sales_office_desc)
    );
    ...
    const doc_ref = doc(tbl_sales_office_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_sales_office_increment(new_data.id);
...
export const api_update_sales_office_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_sales_office_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.SALES_OFFICE)
    );

    await set(tbl_sales_office_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sales_office = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.SALES_OFFICE);
    const tbl_sales_office_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE sales_office_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_sales_office_ref,
      where("sales_office_desc", "==", edit_data.sales_office_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_sales_office = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_sales_office_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_sales_office = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_sales_office_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_sales_office = async () => {
  try {
    const tbl_sales_office_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SALES_OFFICE)
    ...
    const snapshot = await getDocs(tbl_sales_office_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_sales_office_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_sales_office_increment();
...
export const api_reset_sales_office_increment = async () => {
  try {
    const tbl_sales_office_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.SALES_OFFICE)
    ...
    await set(tbl_sales_office_incre_ref, 1);
...
export const api_set_sales_office_increment = async (new_id) => {

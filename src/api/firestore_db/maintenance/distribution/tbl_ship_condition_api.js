...
export const api_get_ship_condition_list = async () => {
  try {
    ...
    const query_snapshot = await getDocs(tbl_ship_condition_ref);
...
export const api_create_ship_condition = async (new_data, user) => {
  try {
    ...
    const doc_ref = doc(tbl_ship_condition_ref, String(new_data.id));
...
export const api_update_ship_condition_increment = async (id) => {
  const new_id = id + 1;
  try {
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_ship_condition = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_ship_condition_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_ship_condition = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_ship_condition_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_ship_condition = async () => {
  try {
    const tbl_ship_condition_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SHIPPING_CONDITION)
    ...
    const snapshot = await getDocs(tbl_ship_condition_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_ship_condition_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_ship_condition_increment();
...
export const api_reset_ship_condition_increment = async () => {
  try {
    const tbl_ship_condition_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.SHIPPING_CONDITION)
    ...
    await set(tbl_ship_condition_incre_ref, 1);

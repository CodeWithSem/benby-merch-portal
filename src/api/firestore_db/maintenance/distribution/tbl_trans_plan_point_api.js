...
// Table Name: TRANSPORTATION_PLAN_POINT
...
// + [Get]
export const api_get_trans_plan_point_list = async () => {
  try {
    const tbl_trans_plan_point_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TRANSPORTATION_PLAN_POINT)
    );

    const query_snapshot = await getDocs(tbl_trans_plan_point_ref);
...
// - [Get]
// + [Create]
export const api_create_trans_plan_point = async (
  new_data,
  user,
  show_toast
) => {
  try {
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE trans_plan_point_code
    // ---------------------------------------------
    const q_code = query(
      tbl_trans_plan_point_ref,
      where("trans_plan_point_code", "==", new_data.trans_plan_point_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE trans_plan_point_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_trans_plan_point_ref,
      where("trans_plan_point_desc", "==", new_data.trans_plan_point_desc)
    );
    ...
    const doc_ref = doc(tbl_trans_plan_point_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_trans_plan_point_increment(new_data.id);
...
export const api_update_trans_plan_point_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_trans_plan_point_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TRANSPORTATION_PLAN_POINT)
    );

    await set(tbl_trans_plan_point_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_trans_plan_point = async (
  edit_data,
  user,
  show_toast
) => {
  try {
    ...
    const tbl_path = get_firestore_path(TABLES.TRANSPORTATION_PLAN_POINT);
    const tbl_trans_plan_point_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE trans_plan_point_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_trans_plan_point_ref,
      where("trans_plan_point_desc", "==", edit_data.trans_plan_point_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_trans_plan_point = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_trans_plan_point_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_trans_plan_point = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_trans_plan_point_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_trans_plan_point = async () => {
  try {
    const tbl_trans_plan_point_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.TRANSPORTATION_PLAN_POINT)
    ...
    const snapshot = await getDocs(tbl_trans_plan_point_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_trans_plan_point_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_trans_plan_point_increment();
...
export const api_reset_trans_plan_point_increment = async () => {
  try {
    const tbl_trans_plan_point_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.TRANSPORTATION_PLAN_POINT)
    ...
    await set(tbl_trans_plan_point_incre_ref, 1);
...
export const api_set_trans_plan_point_increment = async (new_id) => {

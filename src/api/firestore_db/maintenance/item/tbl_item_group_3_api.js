...
// + [Get]
export const api_get_item_group_3_list = async () => {
  try {
    const tbl_item_group_3_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_GROUP_3)
    );

    const query_snapshot = await getDocs(tbl_item_group_3_ref);
...
// - [Get]
// + [Create]
export const api_create_item_group_3 = async (new_data, user, show_toast) => {
  try {
    const tbl_item_group_3_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_GROUP_3)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE item_group_3_code
    // ---------------------------------------------
    const q_code = query(
      tbl_item_group_3_ref,
      where("item_group_3_code", "==", new_data.item_group_3_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE item_group_3_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_item_group_3_ref,
      where("item_group_3_desc", "==", new_data.item_group_3_desc)
    );
    ...
    const doc_ref = doc(tbl_item_group_3_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_item_group_3_increment(new_data.id);
...
export const api_update_item_group_3_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_item_group_3_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ITEM_GROUP_3)
    );

    await set(tbl_item_group_3_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_item_group_3 = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.ITEM_GROUP_3);
    const tbl_item_group_3_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE item_group_3_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_item_group_3_ref,
      where("item_group_3_desc", "==", edit_data.item_group_3_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_item_group_3 = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_item_group_3_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_item_group_3 = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_item_group_3_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_item_group_3 = async () => {
  try {
    const tbl_item_group_3_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_GROUP_3)
    ...
    const snapshot = await getDocs(tbl_item_group_3_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_item_group_3_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_item_group_3_increment();
...
export const api_reset_item_group_3_increment = async () => {
  try {
    const tbl_item_group_3_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ITEM_GROUP_3)
    ...
    await set(tbl_item_group_3_incre_ref, 1);
...
export const api_set_item_group_3_increment = async (new_id) => {

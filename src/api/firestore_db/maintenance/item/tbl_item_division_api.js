...
// + [Get]
export const api_get_item_division_list = async () => {
  try {
    const tbl_item_division_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_DIVISION)
    );

    const query_snapshot = await getDocs(tbl_item_division_ref);
...
// - [Get]
// + [Create]
export const api_create_item_division = async (new_data, user, show_toast) => {
  try {
    const tbl_item_division_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_DIVISION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE item_division_code
    // ---------------------------------------------
    const q_code = query(
      tbl_item_division_ref,
      where("item_division_code", "==", new_data.item_division_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE item_division_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_item_division_ref,
      where("item_division_desc", "==", new_data.item_division_desc)
    );
    ...
    const doc_ref = doc(tbl_item_division_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_item_division_increment(new_data.id);
...
export const api_update_item_division_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_item_division_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ITEM_DIVISION)
    );

    await set(tbl_item_division_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_item_division = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.ITEM_DIVISION);
    const tbl_item_division_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE item_division_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_item_division_ref,
      where("item_division_desc", "==", edit_data.item_division_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_item_division = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_item_division_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_item_division = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_item_division_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_item_division = async () => {
  try {
    const tbl_item_division_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_DIVISION)
    ...
    const snapshot = await getDocs(tbl_item_division_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_item_division_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_item_division_increment();
...
export const api_reset_item_division_increment = async () => {
  try {
    const tbl_item_division_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ITEM_DIVISION)
    ...
    await set(tbl_item_division_incre_ref, 1);
...
export const api_set_item_division_increment = async (new_id) => {

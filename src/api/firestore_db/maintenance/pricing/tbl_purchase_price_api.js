...
// + [Get]
export const api_get_purchase_price_list = async () => {
  try {
    const tbl_purchase_price_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASE_PRICING)
    );

    const query_snapshot = await getDocs(tbl_purchase_price_ref);
...
// - [Get]
// + [Create]
export const api_create_purchase_price = async (new_data, user, show_toast) => {
  try {
    const tbl_purchase_price_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASE_PRICING)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE purchase_price_code
    // ---------------------------------------------
    const q_code = query(
      tbl_purchase_price_ref,
      where("purchase_price_code", "==", new_data.purchase_price_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE purchase_price_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_purchase_price_ref,
      where("purchase_price_desc", "==", new_data.purchase_price_desc)
    );
    ...
    const doc_ref = doc(tbl_purchase_price_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_purchase_price_increment(new_data.id);
...
export const api_update_purchase_price_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_purchase_price_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PURCHASE_PRICING)
    );

    await set(tbl_purchase_price_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_purchase_price = async (
  edit_data,
  user,
  show_toast
) => {
  try {
    ...
    const tbl_path = get_firestore_path(TABLES.PURCHASE_PRICING);
    const tbl_purchase_price_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE purchase_price_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_purchase_price_ref,
      where("purchase_price_desc", "==", edit_data.purchase_price_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_purchase_price = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_purchase_price_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_purchase_price = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_purchase_price_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_purchase_price = async () => {
  try {
    const tbl_purchase_price_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASE_PRICING)
    ...
    const snapshot = await getDocs(tbl_purchase_price_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_purchase_price_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_purchase_price_increment();
...
export const api_reset_purchase_price_increment = async () => {
  try {
    const tbl_purchase_price_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PURCHASE_PRICING)
    ...
    await set(tbl_purchase_price_incre_ref, 1);
...
export const api_set_purchase_price_increment = async (new_id) => {

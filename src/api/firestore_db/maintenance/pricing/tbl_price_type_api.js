...
// + [Get]
export const api_get_price_type_list = async () => {
  try {
    const tbl_price_type_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PRICING_TYPE)
    );

    const query_snapshot = await getDocs(tbl_price_type_ref);
...
// - [Get]
// + [Create]
export const api_create_price_type = async (new_data, user, show_toast) => {
  try {
    const tbl_price_type_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PRICING_TYPE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE price_type_code
    // ---------------------------------------------
    const q_code = query(
      tbl_price_type_ref,
      where("price_type_code", "==", new_data.price_type_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE price_type_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_price_type_ref,
      where("price_type_desc", "==", new_data.price_type_desc)
    );
    ...
    const doc_ref = doc(tbl_price_type_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_price_type_increment(new_data.id);
...
export const api_update_price_type_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_price_type_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PRICING_TYPE)
    );

    await set(tbl_price_type_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_price_type = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.PRICING_TYPE);
    const tbl_price_type_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE price_type_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_price_type_ref,
      where("price_type_desc", "==", edit_data.price_type_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_price_type = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_price_type_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_price_type = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_price_type_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_price_type = async () => {
  try {
    const tbl_price_type_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PRICING_TYPE)
    ...
    const snapshot = await getDocs(tbl_price_type_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_price_type_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_price_type_increment();
...
export const api_reset_price_type_increment = async () => {
  try {
    const tbl_price_type_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PRICING_TYPE)
    ...
    await set(tbl_price_type_incre_ref, 1);
...
export const api_set_price_type_increment = async (new_id) => {

// Data Name: source_hub
// Table Name: SOURCE_HUB
...
// + [Get]
export const api_get_source_hub_list = async () => {
  try {
    const tbl_source_hub_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SOURCE_HUB)
    );

    const query_snapshot = await getDocs(tbl_source_hub_ref);
...
// - [Get]
// + [Create]
export const api_create_source_hub = async (new_data, user, show_toast) => {
  try {
    const tbl_source_hub_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SOURCE_HUB)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE source_hub_code
    // ---------------------------------------------
    const q_code = query(
      tbl_source_hub_ref,
      where("source_hub_code", "==", new_data.source_hub_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE source_hub_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_source_hub_ref,
      where("source_hub_desc", "==", new_data.source_hub_desc)
    );
    ...
    const doc_ref = doc(tbl_source_hub_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_source_hub_increment(new_data.id);
...
export const api_update_source_hub_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_source_hub_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.SOURCE_HUB)
    );

    await set(tbl_source_hub_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_source_hub = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.SOURCE_HUB);
    const tbl_source_hub_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE source_hub_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_source_hub_ref,
      where("source_hub_desc", "==", edit_data.source_hub_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_source_hub = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_source_hub_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_source_hub = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_source_hub_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_source_hub = async () => {
  try {
    const tbl_source_hub_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SOURCE_HUB)
    ...
    const snapshot = await getDocs(tbl_source_hub_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_source_hub_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_source_hub_increment();
...
export const api_reset_source_hub_increment = async () => {
  try {
    const tbl_source_hub_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.SOURCE_HUB)
    ...
    await set(tbl_source_hub_incre_ref, 1);
...
export const api_set_source_hub_increment = async (new_id) => {

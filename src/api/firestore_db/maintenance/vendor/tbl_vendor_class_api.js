...
// + [Get]
export const api_get_vendor_class_list = async () => {
  try {
    const tbl_vendor_class_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.VENDOR_CLASS)
    );

    const query_snapshot = await getDocs(tbl_vendor_class_ref);
...
// - [Get]
// + [Create]
export const api_create_vendor_class = async (new_data, user, show_toast) => {
  try {
    const tbl_vendor_class_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.VENDOR_CLASS)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE vendor_class_code
    // ---------------------------------------------
    const q_code = query(
      tbl_vendor_class_ref,
      where("vendor_class_code", "==", new_data.vendor_class_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE vendor_class_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_vendor_class_ref,
      where("vendor_class_desc", "==", new_data.vendor_class_desc)
    );
    ...
    const doc_ref = doc(tbl_vendor_class_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_vendor_class_increment(new_data.id);
...
export const api_update_vendor_class_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_vendor_class_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.VENDOR_CLASS)
    );

    await set(tbl_vendor_class_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_vendor_class = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.VENDOR_CLASS);
    const tbl_vendor_class_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE vendor_class_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_vendor_class_ref,
      where("vendor_class_desc", "==", edit_data.vendor_class_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_vendor_class = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_vendor_class_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_vendor_class = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_vendor_class_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_vendor_class = async () => {
  try {
    const tbl_vendor_class_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.VENDOR_CLASS)
    ...
    const snapshot = await getDocs(tbl_vendor_class_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_vendor_class_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_vendor_class_increment();
...
export const api_reset_vendor_class_increment = async () => {
  try {
    const tbl_vendor_class_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.VENDOR_CLASS)
    ...
    await set(tbl_vendor_class_incre_ref, 1);
...
export const api_set_vendor_class_increment = async (new_id) => {

...
// + [Get]
export const api_get_incoterms_list = async () => {
  try {
    const tbl_incoterms_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.INCOTERMS)
    );

    const query_snapshot = await getDocs(tbl_incoterms_ref);
...
// - [Get]
// + [Create]
export const api_create_incoterms = async (new_data, user, show_toast) => {
  try {
    const tbl_incoterms_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.INCOTERMS)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE incoterms_code
    // ---------------------------------------------
    const q_code = query(
      tbl_incoterms_ref,
      where("incoterms_code", "==", new_data.incoterms_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE incoterms_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_incoterms_ref,
      where("incoterms_desc", "==", new_data.incoterms_desc)
    );
    ...
    const doc_ref = doc(tbl_incoterms_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_incoterms_increment(new_data.id);
...
export const api_update_incoterms_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_incoterms_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.INCOTERMS)
    );

    await set(tbl_incoterms_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_incoterms = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.INCOTERMS);
    const tbl_incoterms_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE incoterms_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_incoterms_ref,
      where("incoterms_desc", "==", edit_data.incoterms_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_incoterms = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_incoterms_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_incoterms = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_incoterms_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_incoterms = async () => {
  try {
    const tbl_incoterms_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.INCOTERMS)
    ...
    const snapshot = await getDocs(tbl_incoterms_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_incoterms_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_incoterms_increment();
...
export const api_reset_incoterms_increment = async () => {
  try {
    const tbl_incoterms_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.INCOTERMS)
    ...
    await set(tbl_incoterms_incre_ref, 1);
...
export const api_set_incoterms_increment = async (new_id) => {

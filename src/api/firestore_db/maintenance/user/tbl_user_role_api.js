...
// - [Get]
// + [Create]
export const api_create_user_role = async (new_data, user) => {
  try {
    const tbl_user_role_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.USER_ROLE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_user_role = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_user_role_ref, updated_edit_data);

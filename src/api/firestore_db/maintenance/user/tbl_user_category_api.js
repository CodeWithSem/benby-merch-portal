...
// + [Get]
export const api_get_user_category_list = async () => {
  try {
    const tbl_user_category_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.USER_CATEGORY)
    );

    const query_snapshot = await getDocs(tbl_user_category_ref);
...
// - [Get]
// + [Create]
export const api_create_user_category = async (new_data, user, show_toast) => {
  try {
    const tbl_user_category_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.USER_CATEGORY)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE user_category_code
    // ---------------------------------------------
    const q_code = query(
      tbl_user_category_ref,
      where("user_category_code", "==", new_data.user_category_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE user_category_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_user_category_ref,
      where("user_category_desc", "==", new_data.user_category_desc)
    );
    ...
    const doc_ref = doc(tbl_user_category_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_user_category_increment(new_data.id);
...
export const api_update_user_category_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_user_category_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.USER_CATEGORY)
    );

    await set(tbl_user_category_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_user_category = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.USER_CATEGORY);
    const tbl_user_category_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE user_category_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_user_category_ref,
      where("user_category_desc", "==", edit_data.user_category_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_user_category = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_user_category_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_user_category = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_user_category_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_user_category = async () => {
  try {
    const tbl_user_category_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.USER_CATEGORY)
    ...
    const snapshot = await getDocs(tbl_user_category_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_user_category_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_user_category_increment();
...
export const api_reset_user_category_increment = async () => {
  try {
    const tbl_user_category_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.USER_CATEGORY)
    ...
    await set(tbl_user_category_incre_ref, 1);
...
export const api_set_user_category_increment = async (new_id) => {

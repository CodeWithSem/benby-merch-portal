...
// + [Get]
export const api_get_app_matrix_list = async () => {
  try {
    const tbl_app_matrix_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.APPROVAL_MATRIX)
    );

    const query_snapshot = await getDocs(tbl_app_matrix_ref);
...
// - [Get]
// + [Create]
export const api_create_app_matrix = async (new_data, user, show_toast) => {
  try {
    const tbl_app_matrix_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.APPROVAL_MATRIX)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE app_matrix_code
    // ---------------------------------------------
    const q_code = query(
      tbl_app_matrix_ref,
      where("app_matrix_code", "==", new_data.app_matrix_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE app_matrix_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_app_matrix_ref,
      where("app_matrix_desc", "==", new_data.app_matrix_desc)
    );
    ...
    const doc_ref = doc(tbl_app_matrix_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_app_matrix_increment(new_data.id);
...
export const api_update_app_matrix_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_app_matrix_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.APPROVAL_MATRIX)
    );

    await set(tbl_app_matrix_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_app_matrix = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.APPROVAL_MATRIX);
    const tbl_app_matrix_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE app_matrix_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_app_matrix_ref,
      where("app_matrix_desc", "==", edit_data.app_matrix_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_app_matrix = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_app_matrix_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_app_matrix = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_app_matrix_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_app_matrix = async () => {
  try {
    const tbl_app_matrix_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.APPROVAL_MATRIX)
    ...
    const snapshot = await getDocs(tbl_app_matrix_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_app_matrix_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_app_matrix_increment();
...
export const api_reset_app_matrix_increment = async () => {
  try {
    const tbl_app_matrix_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.APPROVAL_MATRIX)
    ...
    await set(tbl_app_matrix_incre_ref, 1);
...
export const api_set_app_matrix_increment = async (new_id) => {

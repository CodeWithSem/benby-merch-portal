...
// + [Get]
export const api_get_module_access_list = async () => {
  try {
    const tbl_module_access_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.MODULE_ACCESS)
    );

    const query_snapshot = await getDocs(tbl_module_access_ref);
...
// - [Get]
// + [Create]
export const api_create_module_access = async (new_data, user, show_toast) => {
  try {
    const tbl_module_access_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.MODULE_ACCESS)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE module_access_code
    // ---------------------------------------------
    const q_code = query(
      tbl_module_access_ref,
      where("module_access_code", "==", new_data.module_access_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE module_access_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_module_access_ref,
      where("module_access_desc", "==", new_data.module_access_desc)
    );
    ...
    const doc_ref = doc(tbl_module_access_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_module_access_increment(new_data.id);
...
export const api_update_module_access_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_module_access_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.MODULE_ACCESS)
    );

    await set(tbl_module_access_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_module_access = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.MODULE_ACCESS);
    const tbl_module_access_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE module_access_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_module_access_ref,
      where("module_access_desc", "==", edit_data.module_access_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_module_access = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_module_access_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_module_access = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_module_access_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_module_access = async () => {
  try {
    const tbl_module_access_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.MODULE_ACCESS)
    ...
    const snapshot = await getDocs(tbl_module_access_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_module_access_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_module_access_increment();
...
export const api_reset_module_access_increment = async () => {
  try {
    const tbl_module_access_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.MODULE_ACCESS)
    ...
    await set(tbl_module_access_incre_ref, 1);
...
export const api_set_module_access_increment = async (new_id) => {

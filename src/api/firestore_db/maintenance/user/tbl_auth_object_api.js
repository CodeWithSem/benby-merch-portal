...
// Table Name: AUTHORIZATION_OBJECT
...
// + [Get]
export const api_get_auth_object_list = async () => {
  try {
    const tbl_auth_object_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.AUTHORIZATION_OBJECT)
    );

    const query_snapshot = await getDocs(tbl_auth_object_ref);
...
// - [Get]
// + [Create]
export const api_create_auth_object = async (new_data, user, show_toast) => {
  try {
    const tbl_auth_object_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.AUTHORIZATION_OBJECT)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE auth_object_code
    // ---------------------------------------------
    const q_code = query(
      tbl_auth_object_ref,
      where("auth_object_code", "==", new_data.auth_object_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE auth_object_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_auth_object_ref,
      where("auth_object_desc", "==", new_data.auth_object_desc)
    );
    ...
    const doc_ref = doc(tbl_auth_object_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_auth_object_increment(new_data.id);
...
export const api_update_auth_object_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_auth_object_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.AUTHORIZATION_OBJECT)
    );

    await set(tbl_auth_object_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_auth_object = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.AUTHORIZATION_OBJECT);
    const tbl_auth_object_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE auth_object_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_auth_object_ref,
      where("auth_object_desc", "==", edit_data.auth_object_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_auth_object = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_auth_object_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_auth_object = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_auth_object_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_auth_object = async () => {
  try {
    const tbl_auth_object_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.AUTHORIZATION_OBJECT)
    ...
    const snapshot = await getDocs(tbl_auth_object_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_auth_object_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_auth_object_increment();
...
export const api_reset_auth_object_increment = async () => {
  try {
    const tbl_auth_object_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.AUTHORIZATION_OBJECT)
    ...
    await set(tbl_auth_object_incre_ref, 1);
...
export const api_set_auth_object_increment = async (new_id) => {

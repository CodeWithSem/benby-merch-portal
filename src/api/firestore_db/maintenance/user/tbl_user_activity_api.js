...
// + [Get]
export const api_get_user_activity_list = async () => {
  try {
    const tbl_user_activity_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.USER_ACTIVITY)
    );

    const query_snapshot = await getDocs(tbl_user_activity_ref);
...
// - [Get]
// + [Create]
export const api_create_user_activity = async (new_data, user, show_toast) => {
  try {
    const tbl_user_activity_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.USER_ACTIVITY)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE user_activity_code
    // ---------------------------------------------
    const q_code = query(
      tbl_user_activity_ref,
      where("user_activity_code", "==", new_data.user_activity_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE user_activity_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_user_activity_ref,
      where("user_activity_desc", "==", new_data.user_activity_desc)
    );
    ...
    const doc_ref = doc(tbl_user_activity_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_user_activity_increment(new_data.id);
...
export const api_update_user_activity_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_user_activity_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.USER_ACTIVITY)
    );

    await set(tbl_user_activity_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_user_activity = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.USER_ACTIVITY);
    const tbl_user_activity_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE user_activity_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_user_activity_ref,
      where("user_activity_desc", "==", edit_data.user_activity_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_user_activity = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_user_activity_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_user_activity = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_user_activity_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_user_activity = async () => {
  try {
    const tbl_user_activity_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.USER_ACTIVITY)
    ...
    const snapshot = await getDocs(tbl_user_activity_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_user_activity_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_user_activity_increment();
...
export const api_reset_user_activity_increment = async () => {
  try {
    const tbl_user_activity_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.USER_ACTIVITY)
    ...
    await set(tbl_user_activity_incre_ref, 1);
...
export const api_set_user_activity_increment = async (new_id) => {

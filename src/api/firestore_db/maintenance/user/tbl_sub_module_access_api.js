...
// + [Get]
export const api_get_sub_module_access_list = async () => {
  try {
...
export const api_create_sub_module_access = async (
  ...
  try {
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE sub_module_access_code
    // ---------------------------------------------
    const q_code = query(
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE sub_module_access_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_sub_module_access_ref,
    ...
    const doc_ref = doc(tbl_sub_module_access_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_sub_module_access_increment(new_data.id);
...
export const api_update_sub_module_access_increment = async (id) => {
  const new_id = id + 1;
  try {
...
export const api_update_sub_module_access = async (
  ...
  try {
    ...
    const tbl_path = get_firestore_path(TABLES.SUB_MODULE_ACCESS);
    const tbl_sub_module_access_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE sub_module_access_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_sub_module_access_ref,
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_sub_module_access = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_sub_module_access_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_sub_module_access = async (id) => {
  if (!id) {
    return {
      success: false,
...
// - [Delete]
// + [Truncate]
export const api_truncate_sub_module_access = async () => {
  try {
    const tbl_sub_module_access_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SUB_MODULE_ACCESS),
    ...
    await api_reset_sub_module_access_increment();
...
export const api_reset_sub_module_access_increment = async () => {
  try {
    const tbl_sub_module_access_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.SUB_MODULE_ACCESS),
...
export const api_set_sub_module_access_increment = async (new_id) => {

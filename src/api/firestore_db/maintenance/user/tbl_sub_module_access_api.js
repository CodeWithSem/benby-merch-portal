...
// - [Get]
// + [Create]
export const api_create_sub_module_access = async (new_data, user) => {
  try {
    const tbl_sub_module_access_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.SUB_MODULE_ACCESS)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sub_module_access = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_sub_module_access_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
      "TBL_SUB_MODULE_ACCESS",
      "DATA",
      String(edit_data.id)
    );
    ...
    await setDoc(tbl_sub_module_access_ref, updated_edit_data);

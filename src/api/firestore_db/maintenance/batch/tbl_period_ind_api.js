...
// + [Get]
export const api_get_period_ind_list = async () => {
  try {
    const tbl_period_ind_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PERIOD_INDICATOR)
    );

    const query_snapshot = await getDocs(tbl_period_ind_ref);
...
// - [Get]
// + [Create]
export const api_create_period_ind = async (new_data, user, show_toast) => {
  try {
    const tbl_period_ind_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PERIOD_INDICATOR)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE period_ind_code
    // ---------------------------------------------
    const q_code = query(
      tbl_period_ind_ref,
      where("period_ind_code", "==", new_data.period_ind_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE period_ind_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_period_ind_ref,
      where("period_ind_desc", "==", new_data.period_ind_desc)
    );
    ...
    const doc_ref = doc(tbl_period_ind_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_period_ind_increment(new_data.id);
...
export const api_update_period_ind_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_period_ind_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PERIOD_INDICATOR)
    );

    await set(tbl_period_ind_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_period_ind = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.PERIOD_INDICATOR);
    const tbl_period_ind_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE period_ind_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_period_ind_ref,
      where("period_ind_desc", "==", edit_data.period_ind_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_period_ind = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_period_ind_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_period_ind = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_period_ind_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_period_ind = async () => {
  try {
    const tbl_period_ind_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PERIOD_INDICATOR)
    ...
    const snapshot = await getDocs(tbl_period_ind_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_period_ind_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_period_ind_increment();
...
export const api_reset_period_ind_increment = async () => {
  try {
    const tbl_period_ind_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PERIOD_INDICATOR)
    ...
    await set(tbl_period_ind_incre_ref, 1);
...
export const api_set_period_ind_increment = async (new_id) => {

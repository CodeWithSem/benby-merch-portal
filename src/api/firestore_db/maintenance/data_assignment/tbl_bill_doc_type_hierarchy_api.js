// Data Name: bill_doc_type_hierarchy
// Table Name: BILLING_DOCUMENT_TYPE_HIERARCHY
...
export const api_get_bill_doc_type_hierarchy_list = async () => {
  try {
    ...
    const query_snapshot = await getDocs(tbl_bill_doc_type_hierarchy_ref);
...
export const api_update_bill_doc_type_hierarchy_increment = async (id) => {
  const new_id = id + 1;
  try {
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_bill_doc_type_hierarchy = async (
  upload_data_list
) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_bill_doc_type_hierarchy_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_bill_doc_type_hierarchy = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_bill_doc_type_hierarchy_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_bill_doc_type_hierarchy = async () => {
  try {
    const tbl_bill_doc_type_hierarchy_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BILLING_DOCUMENT_TYPE_HIERARCHY)
    ...
    const snapshot = await getDocs(tbl_bill_doc_type_hierarchy_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_bill_doc_type_hierarchy_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_bill_doc_type_hierarchy_increment();
...
export const api_reset_bill_doc_type_hierarchy_increment = async () => {
  try {
    const tbl_bill_doc_type_hierarchy_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.BILLING_DOCUMENT_TYPE_HIERARCHY)
    ...
    await set(tbl_bill_doc_type_hierarchy_incre_ref, 1);

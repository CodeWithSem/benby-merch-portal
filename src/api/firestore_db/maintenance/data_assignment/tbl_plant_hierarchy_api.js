...
// + [Get]
export const api_get_plant_hierarchy_list = async () => {
  try {
    const tbl_plant_hierarchy_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PLANT_HIERARCHY)
    );

    const query_snapshot = await getDocs(tbl_plant_hierarchy_ref);
...
// - [Get]
// + [Create]
export const api_create_plant_hierarchy = async (
  new_data,
  user,
  show_toast
) => {
  try {
    ...
    // -----------------------------
    const custom_id = `${new_data.plant_code}_${new_data.sloc_code}`;

    // -------------------------------------
    // VALIDATE DUPLICATE custom_id
    // -------------------------------------
    const q = query(
      tbl_plant_hierarchy_ref,
    ...
    const doc_ref = doc(tbl_plant_hierarchy_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_plant_hierarchy_increment(new_data.id);
...
export const api_update_plant_hierarchy_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_plant_hierarchy_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PLANT_HIERARCHY)
    );

    await set(tbl_plant_hierarchy_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_plant_hierarchy = async (
  edit_data,
  user,
  show_toast
) => {
  try {
    ...
    // ---------------------------------------------------
    const custom_id = `${edit_data.plant_code}_${edit_data.sloc_code}`;
    ...
    if (!custom_snap.empty) {
      ...
      if (existing.id !== String(edit_data.id)) {
        ...
        return {
          success: false,
          message: "This plant hierarchy already exists.",
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_plant_hierarchy = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_plant_hierarchy_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_plant_hierarchy = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_plant_hierarchy_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_plant_hierarchy = async () => {
  try {
    const tbl_plant_hierarchy_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PLANT_HIERARCHY)
    ...
    const snapshot = await getDocs(tbl_plant_hierarchy_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_plant_hierarchy_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_plant_hierarchy_increment();
...
export const api_reset_plant_hierarchy_increment = async () => {
  try {
    const tbl_plant_hierarchy_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PLANT_HIERARCHY)
    ...
    await set(tbl_plant_hierarchy_incre_ref, 1);
...
export const api_set_plant_hierarchy_increment = async (new_id) => {

...
// - [Get]
// + [Create]
export const api_create_warehouse_hierarchy = async (new_data, user) => {
  try {
    const tbl_warehouse_hierarchy_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.WAREHOUSE_HIERARCHY)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_warehouse_hierarchy = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_warehouse_hierarchy_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
    ...
    await setDoc(tbl_warehouse_hierarchy_ref, updated_edit_data);

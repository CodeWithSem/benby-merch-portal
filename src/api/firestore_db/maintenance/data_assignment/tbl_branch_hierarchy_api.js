...
export const api_get_branch_hierarchy_list = async () => {
  try {
    ...
    const query_snapshot = await getDocs(tbl_branch_hierarchy_ref);
...
export const api_update_branch_hierarchy_increment = async (id) => {
  const new_id = id + 1;
  try {
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_branch_hierarchy = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_branch_hierarchy_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_branch_hierarchy = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_branch_hierarchy_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_branch_hierarchy = async () => {
  try {
    const tbl_branch_hierarchy_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.BRANCH_HIERARCHY)
    ...
    const snapshot = await getDocs(tbl_branch_hierarchy_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_branch_hierarchy_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_branch_hierarchy_increment();
...
export const api_reset_branch_hierarchy_increment = async () => {
  try {
    const tbl_branch_hierarchy_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.BRANCH_HIERARCHY)
    ...
    await set(tbl_branch_hierarchy_incre_ref, 1);

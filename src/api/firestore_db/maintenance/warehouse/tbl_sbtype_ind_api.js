...
// - [Get]
// + [Create]
export const api_create_sbtype_ind = async (new_data, user, show_toast) => {
  try {
    const tbl_sbtype_ind_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_BIN_TYPE_INDICATOR)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE sbtype_ind_code
    // ---------------------------------------------
    const q_code = query(
      tbl_sbtype_ind_ref,
      where("sbtype_ind_code", "==", new_data.sbtype_ind_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE sbtype_ind_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_sbtype_ind_ref,
      where("sbtype_ind_desc", "==", new_data.sbtype_ind_desc)
    );
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sbtype_ind = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_BIN_TYPE_INDICATOR);
    const tbl_sbtype_ind_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE sbtype_ind_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_sbtype_ind_ref,
      where("sbtype_ind_desc", "==", edit_data.sbtype_ind_desc)
    );
...
export const api_set_sbtype_ind_increment = async (new_id) => {

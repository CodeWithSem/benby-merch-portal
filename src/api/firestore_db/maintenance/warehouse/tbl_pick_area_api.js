...
// + [Get]
export const api_get_pick_area_list = async () => {
  try {
    const tbl_pick_area_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PICKING_AREA)
    );

    const query_snapshot = await getDocs(tbl_pick_area_ref);
...
// - [Get]
// + [Create]
export const api_create_pick_area = async (new_data, user, show_toast) => {
  try {
    const tbl_pick_area_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PICKING_AREA)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE pick_area_code
    // ---------------------------------------------
    const q_code = query(
      tbl_pick_area_ref,
      where("pick_area_code", "==", new_data.pick_area_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE pick_area_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_pick_area_ref,
      where("pick_area_desc", "==", new_data.pick_area_desc)
    );
    ...
    const doc_ref = doc(tbl_pick_area_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_pick_area_increment(new_data.id);
...
export const api_update_pick_area_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_pick_area_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PICKING_AREA)
    );

    await set(tbl_pick_area_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_pick_area = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.PICKING_AREA);
    const tbl_pick_area_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE pick_area_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_pick_area_ref,
      where("pick_area_desc", "==", edit_data.pick_area_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_pick_area = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_pick_area_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_pick_area = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_pick_area_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_pick_area = async () => {
  try {
    const tbl_pick_area_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PICKING_AREA)
    ...
    const snapshot = await getDocs(tbl_pick_area_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_pick_area_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_pick_area_increment();
...
export const api_reset_pick_area_increment = async () => {
  try {
    const tbl_pick_area_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PICKING_AREA)
    ...
    await set(tbl_pick_area_incre_ref, 1);
...
export const api_set_pick_area_increment = async (new_id) => {

...
// - [Get]
// + [Create]
export const api_create_sbtype = async (new_data, user) => {
  try {
    const tbl_sbtype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_BIN_TYPE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sbtype = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_sbtype_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
      "TBL_STORAGE_BIN_TYPE",
      "DATA",
      String(edit_data.id)
    );
    ...
    await setDoc(tbl_sbtype_ref, updated_edit_data);

...
// + [Get]
export const api_get_sbtype_list = async () => {
  try {
    const tbl_sbtype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_BIN_TYPE)
    );

    const query_snapshot = await getDocs(tbl_sbtype_ref);
...
// - [Get]
// + [Create]
export const api_create_sbtype = async (new_data, user, show_toast) => {
  try {
    const tbl_sbtype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_BIN_TYPE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE sbtype_code
    // ---------------------------------------------
    const q_code = query(
      tbl_sbtype_ref,
      where("sbtype_code", "==", new_data.sbtype_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE sbtype_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_sbtype_ref,
      where("sbtype_desc", "==", new_data.sbtype_desc)
    );
    ...
    const doc_ref = doc(tbl_sbtype_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_sbtype_increment(new_data.id);
...
export const api_update_sbtype_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_sbtype_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_BIN_TYPE)
    );

    await set(tbl_sbtype_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sbtype = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_BIN_TYPE);
    const tbl_sbtype_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE sbtype_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_sbtype_ref,
      where("sbtype_desc", "==", edit_data.sbtype_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_sbtype = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_sbtype_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_sbtype = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_sbtype_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_sbtype = async () => {
  try {
    const tbl_sbtype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_BIN_TYPE)
    ...
    const snapshot = await getDocs(tbl_sbtype_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_sbtype_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_sbtype_increment();
...
export const api_reset_sbtype_increment = async () => {
  try {
    const tbl_sbtype_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_BIN_TYPE)
    ...
    await set(tbl_sbtype_incre_ref, 1);
...
export const api_set_sbtype_increment = async (new_id) => {

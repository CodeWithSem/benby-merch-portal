...
// + [Get]
export const api_get_sutype_list = async () => {
  try {
    const tbl_sutype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_UNIT_TYPE)
    );

    const query_snapshot = await getDocs(tbl_sutype_ref);
...
// - [Get]
// + [Create]
export const api_create_sutype = async (new_data, user, show_toast) => {
  try {
    const tbl_sutype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_UNIT_TYPE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE sutype_code
    // ---------------------------------------------
    const q_code = query(
      tbl_sutype_ref,
      where("sutype_code", "==", new_data.sutype_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE sutype_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_sutype_ref,
      where("sutype_desc", "==", new_data.sutype_desc)
    );
    ...
    const doc_ref = doc(tbl_sutype_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_sutype_increment(new_data.id);
...
export const api_update_sutype_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_sutype_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_UNIT_TYPE)
    );

    await set(tbl_sutype_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sutype = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_UNIT_TYPE);
    const tbl_sutype_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE sutype_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_sutype_ref,
      where("sutype_desc", "==", edit_data.sutype_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_sutype = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_sutype_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_sutype = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_sutype_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_sutype = async () => {
  try {
    const tbl_sutype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_UNIT_TYPE)
    ...
    const snapshot = await getDocs(tbl_sutype_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_sutype_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_sutype_increment();
...
export const api_reset_sutype_increment = async () => {
  try {
    const tbl_sutype_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_UNIT_TYPE)
    ...
    await set(tbl_sutype_incre_ref, 1);
...
export const api_set_sutype_increment = async (new_id) => {

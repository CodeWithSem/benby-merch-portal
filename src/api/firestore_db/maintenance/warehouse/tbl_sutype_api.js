...
// - [Get]
// + [Create]
export const api_create_sutype = async (new_data, user) => {
  try {
    const tbl_sutype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_UNIT_TYPE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sutype = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_sutype_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
      "TBL_STORAGE_UNIT_TYPE",
      "DATA",
      String(edit_data.id)
    );
    ...
    await setDoc(tbl_sutype_ref, updated_edit_data);

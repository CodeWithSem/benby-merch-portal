...
// + [Get]
export const api_get_stype_list = async () => {
  try {
    const tbl_stype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_TYPE)
    );

    const query_snapshot = await getDocs(tbl_stype_ref);
...
// - [Get]
// + [Create]
export const api_create_stype = async (new_data, user, show_toast) => {
  try {
    const tbl_stype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_TYPE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE stype_code
    // ---------------------------------------------
    const q_code = query(
      tbl_stype_ref,
      where("stype_code", "==", new_data.stype_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE stype_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_stype_ref,
      where("stype_desc", "==", new_data.stype_desc)
    );
    ...
    const doc_ref = doc(tbl_stype_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_stype_increment(new_data.id);
...
export const api_update_stype_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_stype_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_TYPE)
    );

    await set(tbl_stype_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_stype = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_TYPE);
    const tbl_stype_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE stype_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_stype_ref,
      where("stype_desc", "==", edit_data.stype_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_stype = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_stype_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_stype = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_stype_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_stype = async () => {
  try {
    const tbl_stype_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_TYPE)
    ...
    const snapshot = await getDocs(tbl_stype_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_stype_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_stype_increment();
...
export const api_reset_stype_increment = async () => {
  try {
    const tbl_stype_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_TYPE)
    ...
    await set(tbl_stype_incre_ref, 1);
...
export const api_set_stype_increment = async (new_id) => {

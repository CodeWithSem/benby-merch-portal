...
// - [Get]
// + [Create]
export const api_create_ssec = async (new_data, user) => {
  try {
    const tbl_ssec_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_SECTION)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_ssec = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_ssec_ref, updated_edit_data);

...
// + [Get]
export const api_get_ssec_list = async () => {
  try {
    const tbl_ssec_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_SECTION)
    );

    const query_snapshot = await getDocs(tbl_ssec_ref);
...
// - [Get]
// + [Create]
export const api_create_ssec = async (new_data, user, show_toast) => {
  try {
    const tbl_ssec_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_SECTION)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE ssec_code
    // ---------------------------------------------
    const q_code = query(
      tbl_ssec_ref,
      where("ssec_code", "==", new_data.ssec_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE ssec_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_ssec_ref,
      where("ssec_desc", "==", new_data.ssec_desc)
    );
    ...
    const doc_ref = doc(tbl_ssec_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_ssec_increment(new_data.id);
...
export const api_update_ssec_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_ssec_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_SECTION)
    );

    await set(tbl_ssec_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_ssec = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_SECTION);
    const tbl_ssec_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE ssec_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_ssec_ref,
      where("ssec_desc", "==", edit_data.ssec_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_ssec = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_ssec_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_ssec = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_ssec_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_ssec = async () => {
  try {
    const tbl_ssec_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_SECTION)
    ...
    const snapshot = await getDocs(tbl_ssec_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_ssec_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_ssec_increment();
...
export const api_reset_ssec_increment = async () => {
  try {
    const tbl_ssec_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_SECTION)
    ...
    await set(tbl_ssec_incre_ref, 1);
...
export const api_set_ssec_increment = async (new_id) => {

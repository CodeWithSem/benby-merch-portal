...
// - [Get]
// + [Create]
export const api_create_warehouse = async (new_data, user) => {
  try {
    const tbl_warehouse_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.WAREHOUSE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_warehouse = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_warehouse_ref, updated_edit_data);

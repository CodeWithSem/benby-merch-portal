...
// + [Get]
export const api_get_warehouse_list = async () => {
  try {
    const tbl_warehouse_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.WAREHOUSE)
    );

    const query_snapshot = await getDocs(tbl_warehouse_ref);
...
// - [Get]
// + [Create]
export const api_create_warehouse = async (new_data, user, show_toast) => {
  try {
    const tbl_warehouse_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.WAREHOUSE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE warehouse_code
    // ---------------------------------------------
    const q_code = query(
      tbl_warehouse_ref,
      where("warehouse_code", "==", new_data.warehouse_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE warehouse_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_warehouse_ref,
      where("warehouse_desc", "==", new_data.warehouse_desc)
    );
    ...
    const doc_ref = doc(tbl_warehouse_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_warehouse_increment(new_data.id);
...
export const api_update_warehouse_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_warehouse_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.WAREHOUSE)
    );

    await set(tbl_warehouse_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_warehouse = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.WAREHOUSE);
    const tbl_warehouse_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE warehouse_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_warehouse_ref,
      where("warehouse_desc", "==", edit_data.warehouse_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_warehouse = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_warehouse_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_warehouse = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_warehouse_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_warehouse = async () => {
  try {
    const tbl_warehouse_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.WAREHOUSE)
    ...
    const snapshot = await getDocs(tbl_warehouse_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_warehouse_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_warehouse_increment();
...
export const api_reset_warehouse_increment = async () => {
  try {
    const tbl_warehouse_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.WAREHOUSE)
    ...
    await set(tbl_warehouse_incre_ref, 1);
...
export const api_set_warehouse_increment = async (new_id) => {

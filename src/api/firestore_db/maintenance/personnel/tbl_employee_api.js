...
// + [Get]
export const api_get_employee_list = async () => {
  try {
    const tbl_employee_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.EMPLOYEE)
    );

    const query_snapshot = await getDocs(tbl_employee_ref);
...
// - [Get]
// + [Create]
export const api_create_employee = async (new_data, user, show_toast) => {
  try {
    const tbl_employee_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.EMPLOYEE)
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE employee_code
    // ---------------------------------------------
    const q_code = query(
      tbl_employee_ref,
      where("employee_code", "==", new_data.employee_code)
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE employee_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_employee_ref,
      where("employee_desc", "==", new_data.employee_desc)
    );
    ...
    const doc_ref = doc(tbl_employee_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_employee_increment(new_data.id);
...
export const api_update_employee_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_employee_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.EMPLOYEE)
    );

    await set(tbl_employee_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_employee = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.EMPLOYEE);
    const tbl_employee_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------------
    // 1. CHECK DUPLICATE employee_desc (exclude same ID)
    // ---------------------------------------------------
    const q_desc = query(
      tbl_employee_ref,
      where("employee_desc", "==", edit_data.employee_desc)
    );
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_employee = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_employee_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_employee = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_employee_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_employee = async () => {
  try {
    const tbl_employee_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.EMPLOYEE)
    ...
    const snapshot = await getDocs(tbl_employee_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_employee_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_employee_increment();
...
export const api_reset_employee_increment = async () => {
  try {
    const tbl_employee_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.EMPLOYEE)
    ...
    await set(tbl_employee_incre_ref, 1);
...
export const api_set_employee_increment = async (new_id) => {

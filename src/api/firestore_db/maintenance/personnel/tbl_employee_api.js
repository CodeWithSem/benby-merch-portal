...
// - [Get]
// + [Create]
export const api_create_employee = async (new_data, user) => {
  try {
    const tbl_employee_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.EMPLOYEE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_employee = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_employee_ref, updated_edit_data);

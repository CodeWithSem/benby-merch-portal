...
// - [Get]
// + [Create]
export const api_create_item_group = async (new_data, user) => {
  try {
    const tbl_item_group_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_GROUP)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_item_group = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_item_group_ref, updated_edit_data);

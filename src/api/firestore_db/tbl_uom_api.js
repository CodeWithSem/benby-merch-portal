...
// - [Get]
// + [Create]
export const api_create_uom = async (new_data, user) => {
  try {
    const tbl_uom_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.UNIT_OF_MEASURE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_uom = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_uom_ref, updated_edit_data);

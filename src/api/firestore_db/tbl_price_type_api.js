...
// - [Get]
// + [Create]
export const api_create_price_type = async (new_data, user) => {
  try {
    const tbl_price_type_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PRICING_TYPE)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_price_type = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_price_type_ref, updated_edit_data);

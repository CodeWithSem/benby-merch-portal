...
// + [Get]
export const api_get_pl_status_list = async () => {
  try {
    const tbl_pl_status_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PLANT_STATUS)
    );

    const query_snapshot = await getDocs(tbl_pl_status_ref);
...
// - [Get]
// + [Create]
export const api_create_pl_status = async (new_data, user) => {
  try {
    const tbl_pl_status_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PLANT_STATUS)
    ...
    const doc_ref = doc(tbl_pl_status_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_pl_status_increment(new_data.id);
    return {
      success: true,
...
export const api_update_pl_status_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_pl_status_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PLANT_STATUS)
    );

    await set(tbl_pl_status_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_pl_status = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_pl_status_ref, updated_edit_data);
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_pl_status = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_pl_status_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_pl_status = async (id) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    const doc_ref = doc(tbl_pl_status_ref, String(id));
...
// - [Delete]
// + [Truncate]
export const api_truncate_pl_status = async () => {
  try {
    const tbl_pl_status_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PLANT_STATUS)
    ...
    const snapshot = await getDocs(tbl_pl_status_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_pl_status_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_pl_status_increment();
...
export const api_reset_pl_status_increment = async () => {
  try {
    const tbl_pl_status_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.PLANT_STATUS)
    ...
    await set(tbl_pl_status_incre_ref, 1);

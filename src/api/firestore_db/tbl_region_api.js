...
// - [Get]
// + [Create]
export const api_create_region = async (new_data, user) => {
  try {
    const tbl_region_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.REGION)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_region = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_region_ref, updated_edit_data);

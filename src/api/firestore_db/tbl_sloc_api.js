...
// - [Get]
// + [Create]
export const api_create_sloc = async (new_data, user) => {
  try {
    const tbl_sloc_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_LOCATION)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sloc = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_sloc_ref = doc(
      firestore_db,
      "DB1_ERP_SYSTEM",
      "TBL_STORAGE_LOCATION",
      "DATA",
      String(edit_data.id)
    );
    ...
    await setDoc(tbl_sloc_ref, updated_edit_data);

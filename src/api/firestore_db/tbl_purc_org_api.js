...
// - [Get]
// + [Create]
export const api_create_purc_org = async (new_data, user) => {
  try {
    const tbl_purc_org_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.PURCHASING_ORG)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_purc_org = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_purc_org_ref, updated_edit_data);

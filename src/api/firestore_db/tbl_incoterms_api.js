...
// - [Get]
// + [Create]
export const api_create_incoterms = async (new_data, user) => {
  try {
    const tbl_incoterms_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.INCOTERMS)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_incoterms = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_incoterms_ref, updated_edit_data);

...
// + [Get]
export const api_get_sbin_list = async () => {
  try {
...
// - [Get]
// + [Create]
export const api_create_sbin = async (new_data, user, show_toast) => {
  try {
    const tbl_sbin_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_BIN)
    ...
    // ---------------------------------------------
    // 3. CREATE NEW DATA USING PROVIDED ID
    // ---------------------------------------------
    ...
    // Use the ID from new_data
    const doc_ref = doc(tbl_sbin_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_sbin_increment(new_data.id);
...
export const api_update_sbin_increment = async (id) => {
  const new_id = id + 1;
  try {
...
// - [Update Incremental ID]
// + [Update]
export const api_update_sbin = async (edit_data, user, show_toast) => {
  try {
    // ---------------------------------------------------
    // 0. VALIDATE ID
    // ---------------------------------------------------
    ...
    const tbl_path = get_firestore_path(TABLES.STORAGE_BIN);
    const tbl_sbin_ref = collection(firestore_db, ...tbl_path);
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_sbin = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_sbin_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_sbin = async (id, show_toast) => {
  if (!id) {
    return {
      success: false,
...
// - [Delete]
// + [Truncate]
export const api_truncate_sbin = async (show_toast) => {
  try {
    const tbl_sbin_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.STORAGE_BIN)
    ...
    await api_reset_sbin_increment();
...
export const api_reset_sbin_increment = async () => {
  try {
    const tbl_sbin_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.STORAGE_BIN)
...
export const api_set_sbin_increment = async (new_id) => {

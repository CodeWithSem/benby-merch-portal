// Data Name: item_ext_wm2
// Table Name: ITEM_EXT_WM_DATA_2
...
// + [Get]
export const api_get_item_ext_wm2_list = async (item_code) => {
  try {
    const tbl_item_ext_wm2_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_WM_DATA_2)
    );

    const q = query(tbl_item_ext_wm2_ref, where("item_code", "==", item_code));
...
// - [Get]
// + [Create]
export const api_create_item_ext_wm2 = async (new_data, user) => {
  try {
    const tbl_item_ext_wm2_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_WM_DATA_2)
    ...
    const custom_id = `${new_data.item_code}_${new_data.branch_code}_${new_data.plant_code}_${new_data.sloc_code}_${new_data.warehouse_code}_${new_data.stype_code}`;
    const doc_ref = doc(tbl_item_ext_wm2_ref, custom_id);
...
// - [Create]
// + [Delete]
export const api_delete_item_ext_wm2 = async (id) => {
  if (!id) {
    return {
      success: false,
...
// - [Delete]
// + [Truncate]
export const api_truncate_item_ext_wm2 = async () => {
  try {
    const tbl_item_ext_wm2_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_WM_DATA_2)
    ...
    const snapshot = await getDocs(tbl_item_ext_wm2_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_item_ext_wm2_ref, document.id))
    );

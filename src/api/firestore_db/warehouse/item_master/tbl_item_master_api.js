...
// + [Get]
export const api_get_item_master_list = async () => {
  try {
    const tbl_item_master_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_MASTER)
    );

    const query_snapshot = await getDocs(tbl_item_master_ref);
...
// - [Get]
// + [Get By Description]
export const api_get_item_master_by_desc = async (item_desc) => {
  ...
  try {
    ...
    const q = query(tbl_item_master_ref, where("item_desc", "==", clean_desc));
...
// - [Get By Description]
// + [Create]
export const api_create_item_master = async (new_data, user) => {
  try {
    const tbl_item_master_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_MASTER)
    ...
    const doc_ref = doc(tbl_item_master_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_item_master_increment(new_data.id);
    return {
      success: true,
...
export const api_update_item_master_increment = async (id) => {
  const new_id = id + 1;
  try {
    const tbl_item_master_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ITEM_MASTER)
    );

    await set(tbl_item_master_incre_ref, new_id);
...
// - [Update Incremental ID]
// + [Update]
export const api_update_item_master = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_item_master_ref, updated_edit_data);
...
// - [Update]
// + [Bulk Upload]
export const api_bulk_upload_item_master = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_item_master_ref, String(item.id));
...
// - [Bulk Upload]
// + [Delete]
export const api_delete_item_master = async (id) => {
  if (!id) {
    return {
      success: false,
...
// - [Delete]
// + [Truncate]
export const api_truncate_item_master = async () => {
  try {
    const tbl_item_master_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_MASTER)
    ...
    const snapshot = await getDocs(tbl_item_master_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_item_master_ref, document.id))
    );

    await Promise.all(delete_promises);
    await api_reset_item_master_increment();
...
export const api_reset_item_master_increment = async () => {
  try {
    const tbl_item_master_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.ITEM_MASTER)
    ...
    await set(tbl_item_master_incre_ref, 1);
...
export const api_set_item_master_increment = async (new_id) => {

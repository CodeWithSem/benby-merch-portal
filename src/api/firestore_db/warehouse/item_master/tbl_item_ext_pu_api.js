// Data Name: item_ext_pu
// Table Name: ITEM_EXT_PURCHASING
...
// + [Get]
export const api_get_item_ext_pu_list = async (item_code) => {
  try {
    const tbl_item_ext_pu_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_PURCHASING)
    );

    const q = query(tbl_item_ext_pu_ref, where("item_code", "==", item_code));
...
// - [Get]
// + [Create]
export const api_create_item_ext_pu = async (new_data, user) => {
  try {
    const tbl_item_ext_pu_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_PURCHASING)
    ...
    const doc_ref = doc(tbl_item_ext_pu_ref, custom_id);
...
// - [Create]
// + [Delete]
export const api_delete_item_ext_pu = async (id) => {
  if (!id) {
    return {
      success: false,
...
// - [Delete]
// + [Truncate]
export const api_truncate_item_ext_pu = async () => {
  try {
    const tbl_item_ext_pu_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_PURCHASING)
    ...
    const snapshot = await getDocs(tbl_item_ext_pu_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_item_ext_pu_ref, document.id))
    );

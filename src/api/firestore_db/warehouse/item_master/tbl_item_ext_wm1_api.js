// Data Name: item_ext_wm1
// Table Name: ITEM_EXT_WM_DATA_1
...
// + [Get]
export const api_get_item_ext_wm1_list = async (item_code) => {
  try {
    const tbl_item_ext_wm1_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_WM_DATA_1)
    );

    const q = query(tbl_item_ext_wm1_ref, where("item_code", "==", item_code));
...
// - [Get]
// + [Create]
export const api_create_item_ext_wm1 = async (new_data, user) => {
  try {
    const tbl_item_ext_wm1_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_WM_DATA_1)
    ...
    const custom_id = `${new_data.item_code}_${new_data.branch_code}_${new_data.plant_code}_${new_data.sloc_code}`;
    const doc_ref = doc(tbl_item_ext_wm1_ref, custom_id);
...
// - [Create]
// + [Delete]
export const api_delete_item_ext_wm1 = async (id) => {
  if (!id) {
    return {
      success: false,
...
// - [Delete]
// + [Truncate]
export const api_truncate_item_ext_wm1 = async () => {
  try {
    const tbl_item_ext_wm1_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.ITEM_EXT_WM_DATA_1)
    ...
    const snapshot = await getDocs(tbl_item_ext_wm1_ref);

    const delete_promises = snapshot.docs.map((document) =>
      deleteDoc(doc(tbl_item_ext_wm1_ref, document.id))
    );

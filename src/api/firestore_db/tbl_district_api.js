...
// - [Get]
// + [Create]
export const api_create_district = async (new_data, user) => {
  try {
    const tbl_district_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.DISTRICT)
...
// - [Update Incremental ID]
// + [Update]
export const api_update_district = async (edit_data, user) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    await setDoc(tbl_district_ref, updated_edit_data);

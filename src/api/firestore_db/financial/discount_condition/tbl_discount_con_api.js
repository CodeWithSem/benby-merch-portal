...
/* ============================================================
   + [Get]
   ============================================================ */
export const api_get_discount_con_list = async () => {
  try {
    const tbl_discount_con_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.DISCOUNT_CONDITION),
    );

    const query_snapshot = await getDocs(tbl_discount_con_ref);
...
/* ============================================================
   + [Create]
   ============================================================ */
export const api_create_discount_con = async (new_data, user, show_toast) => {
  try {
    const tbl_discount_con_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.DISCOUNT_CONDITION),
    ...
    // ---------------------------------------------
    // 1. CHECK DUPLICATE discount_con_code
    // ---------------------------------------------
    const q_code = query(
      tbl_discount_con_ref,
      where("discount_con_code", "==", new_data.discount_con_code),
    );
    ...
    // ---------------------------------------------
    // 2. CHECK DUPLICATE discount_con_desc
    // ---------------------------------------------
    const q_desc = query(
      tbl_discount_con_ref,
      where("discount_con_desc", "==", new_data.discount_con_desc),
    );
    ...
    const doc_ref = doc(tbl_discount_con_ref, String(new_data.id));

    await setDoc(doc_ref, final_new_data);
    await api_update_discount_con_increment(new_data.id);
...
/* ============================================================
   ...
   ============================================================ */
export const api_update_discount_con_increment = async (id) => {
  const new_id = id + 1;

  try {
    const tbl_discount_con_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.DISCOUNT_CONDITION),
    );

    await set(tbl_discount_con_incre_ref, new_id);
...
/* ============================================================
   + [Update]
   ============================================================ */
export const api_update_discount_con = async (edit_data, user, show_toast) => {
  try {
    if (!edit_data.id) {
      return {
        success: false,
    ...
    const tbl_path = get_firestore_path(TABLES.DISCOUNT_CONDITION);
    const tbl_discount_con_ref = collection(firestore_db, ...tbl_path);

    // ---------------------------------------------
    // CHECK DUPLICATE discount_con_desc (exclude same ID)
    // ---------------------------------------------
    const q_desc = query(
      tbl_discount_con_ref,
      where("discount_con_desc", "==", edit_data.discount_con_desc),
    );
...
/* ============================================================
   + [Bulk Upload]
   ============================================================ */
export const api_bulk_upload_discount_con = async (upload_data_list) => {
  ...
  try {
    ...
    upload_data_list.forEach((item) => {
      const doc_ref = doc(tbl_discount_con_ref, String(item.id));
...
/* ============================================================
   + [Delete]
   ============================================================ */
export const api_delete_discount_con = async (id, show_toast) => {
  if (!id) {
    return {
      success: false,
  ...
  try {
    ...
    await deleteDoc(doc(tbl_discount_con_ref, String(id)));
...
/* ============================================================
   + [Truncate]
   ============================================================ */
export const api_truncate_discount_con = async (show_toast) => {
  try {
    const tbl_discount_con_ref = collection(
      firestore_db,
      ...get_firestore_path(TABLES.DISCOUNT_CONDITION),
    ...
    const snapshot = await getDocs(tbl_discount_con_ref);

    const delete_promises = snapshot.docs.map((docu) =>
      deleteDoc(doc(tbl_discount_con_ref, docu.id)),
    );

    await Promise.all(delete_promises);
    await api_reset_discount_con_increment();
...
/* ============================================================
   ...
   ============================================================ */
export const api_reset_discount_con_increment = async () => {
  try {
    const tbl_discount_con_incre_ref = ref(
      realtime_db,
      get_incremental_path(TABLES.DISCOUNT_CONDITION),
    ...
    await set(tbl_discount_con_incre_ref, 1);
...
/* ============================================================
   ...
   ============================================================ */
export const api_set_discount_con_increment = async (new_id) => {

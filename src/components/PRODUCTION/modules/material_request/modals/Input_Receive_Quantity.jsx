...
const Input_Receive_Quantity = ({
  ...
  // Handlers
  // ---------------------------------------------
  const handle_number = (value) => {
    // Prevent multiple dots
    if (value === "." && quantity.includes(".")) return;

    // If first input is dot, prepend 0
    if (value === "." && quantity === "") {
      set_quantity("0.");
      return;
    }
  ...
  const render_key = (value) => (
    <Button
      ...
      on_click={() => handle_number(String(value))}
    >
      {value}
    </Button>
  );
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="grid grid-cols-2 gap-16 w-full max-w-[1000px]">
            ...
            <div className="flex flex-col items-center justify-center gap-4">
              ...
              <div className="w-full h-[120px] rounded-xl border-2 border-gray-700 flex items-center justify-center text-4xl font-bold">
                {quantity || 0}
              </div>
            </div>
...
export default Input_Receive_Quantity;

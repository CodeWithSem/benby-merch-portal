...
import {
  get_tl_history_list_by_date,
  get_tl_history_list_by_tds,
} from "api/real_time_db/data_history/tl_history_api";
...
const TL_History = () => {
  ...
  const columns = [
    ...
    { key: "plantilla_code", label: "PLANTILLA CODE", sortable: true },
    { key: "store_code", label: "STORE CODE", sortable: true },
    { key: "survey_date", label: "SURVEY DATE", sortable: true },
  ...
  const [tl_history_list, set_tl_history_list] = useState([]);

  const handle_get_tl_history_list = async () => {
    set_loading(true);
    try {
      let results = [];
      if (search_mode === "code") {
        ...
        results = await get_tl_history_list_by_tds(input_tds_code);
      } else {
        if (!input_date) {
          alert("Please enter a date.");
          return;
        }
        results = await get_tl_history_list_by_date(input_date);
      }

      set_tl_history_list(results);
      set_current_page(1);
    } catch (error) {
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(tl_history_list, columns);

  const handle_export_excel = () => {
    if (!tl_history_list || tl_history_list.length === 0) {
      alert("There is no data to export");
      return;
    }
    const result = export_excel_service(tl_history_list, columns, {
      filename_prefix: `TL_HISTORY`,
      sheet_name: "Trainig Logs History",
    });
                  ...
                  <div className="w-full flex items-center gap-2">
                    <div className="w-full">
                      {search_mode === "code" ? (
                        <Icon_Field
                          ...
                          on_key_down={(e) =>
                            e.key === "Enter" && handle_get_tl_history_list()
                          }
                        />
                      ) : (
                        <Icon_Field
                    ...
                    <div className="relative">
                      <Button
                        ...
                        width="w-[140px]"
                        loading={loading}
                        on_click={handle_get_tl_history_list}
                      >
                        Load Data
                      </Button>
                    </div>

...
import {
  get_as_history_list_by_date,
  get_as_history_list_by_tds,
} from "api/real_time_db/data_history/as_history_api";
...
const AS_History = () => {
  ...
  const [as_history_list, set_as_history_list] = useState([]);
  ...
  const handle_get_as_history_list = async () => {
    ...
    try {
      ...
      if (search_mode === "code") {
        ...
        results = await get_as_history_list_by_tds(input_tds_code);
      } else {
        if (!input_date) {
          alert("Please enter a date.");
          return;
        }
        results = await get_as_history_list_by_date(input_date);
      }
      set_as_history_list(results);
      set_current_page(1);
    } catch (error) {
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(as_history_list, columns);

  const handle_export_excel = () => {
    if (!as_history_list || as_history_list.length === 0) {
      alert("There is no data to export");
      return;
    }
    const result = export_excel_service(as_history_list, columns, {
      filename_prefix: `AUDIT_SURVEY_HISTORY`,
      sheet_name: "Audit Survey History",
    });

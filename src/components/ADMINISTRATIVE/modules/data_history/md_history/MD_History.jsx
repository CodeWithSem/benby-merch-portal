...
import { get_md_history_list_by_date } from "api/real_time_db/data_history/md_history_api";
...
const MD_History = () => {
  ...
  const columns = [
    ...
    { key: "diserID", label: "DISER ID", sortable: true },
    { key: "diserName", label: "MERCHANDISER NAME", sortable: true },
    { key: "tdsID", label: "TDS ID", sortable: true },
    { key: "benbyId", label: "BENBY ID", sortable: true },
    { key: "datetoday", label: "DATE", sortable: true },
    { key: "timeIn", label: "TIME IN", sortable: true },
    { key: "timeOut", label: "TIME OUT", sortable: true },
    { key: "remarks1", label: "REMARKS 1", sortable: true },
    { key: "remarks2", label: "REMARKS 2", sortable: true },
    { key: "remarks3", label: "REMARKS 3", sortable: true },
  ...
  const handle_get_md_history_list = async (selected_date) => {
    if (!selected_date) {
    ...
    try {
      ...
      const results = await get_md_history_list_by_date(formatted_date);

      // 3. Update state with results
      set_md_history_list(results);
      set_current_page(1);

      // Optional: Add a "No data found" alert if the list is empty
      if (results.length === 0) {
        console.log(`No MD History found for ${formatted_date}`);
      }
    } catch (error) {
      console.error("Fetch failed for MD history:", error);
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(md_history_list, columns);

  const handle_export_excel = () => {
    if (!md_history_list || md_history_list.length === 0) {
      alert("There is no data to export");
      return;
    }
    const result = export_excel_service(md_history_list, columns, {
      filename_prefix: `MD_HISTORY`,
                  ...
                  <div className="w-[200px]">
                    <Date_Field
                      ...
                      on_change={(e) => {
                        ...
                        if (selected_date) {
                          handle_get_md_history_list(selected_date);
                        }
                      }}
                    />
                  </div>
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={() => handle_get_md_history_list(input_date)}
                      />
                    </div>

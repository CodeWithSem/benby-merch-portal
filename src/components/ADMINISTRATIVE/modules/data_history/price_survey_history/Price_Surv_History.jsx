...
import {
  get_price_surv_history_list_by_date,
  get_price_surv_history_list_by_tds,
} from "api/real_time_db/data_history/price_surv_history_api";

const Price_Surv_History = () => {
  ...
  const columns = [
    ...
    { key: "dateUpload", label: "DATE UPLOADED", sortable: true, hidden: true },
  ...
  const [price_surv_history_list, set_price_surv_history_list] = useState([]);

  const handle_get_price_surv_history_list = async () => {
    set_loading(true);
    try {
      let results = [];
      if (search_mode === "code") {
        ...
        results = await get_price_surv_history_list_by_tds(input_tds_code);
      } else {
        if (!input_date) {
          alert("Please enter a date.");
          return;
        }
        results = await get_price_surv_history_list_by_date(input_date);
      }
      set_price_surv_history_list(results);
      set_current_page(1);
    } catch (error) {
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(price_surv_history_list, columns);

  const handle_export_excel = () => {
    if (!price_surv_history_list || price_surv_history_list.length === 0) {
      alert("There is no data to export");
      return;
    }
    const result = export_excel_service(price_surv_history_list, columns, {
      filename_prefix: `PRICE_SURVEY_HISTORY`,
      sheet_name: "Price Survey History",
    });
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    ...
                    <span className="text-gray-800">Price Survey</span>
                  </li>
                </ol>
              </nav>
            </div>
                  ...
                  <div className="w-full flex items-center gap-2">
                    <div className="w-full">
                      {search_mode === "code" ? (
                        <Icon_Field
                          ...
                          on_key_down={(e) =>
                            e.key === "Enter" &&
                            handle_get_price_surv_history_list()
                          }
                        />
                      ) : (
                        <Icon_Field
                    ...
                    <div className="relative">
                      <Button
                        ...
                        width="w-[140px]"
                        loading={loading}
                        on_click={handle_get_price_surv_history_list}
                      >
                        Load Data
                      </Button>
                    </div>
...
export default Price_Surv_History;

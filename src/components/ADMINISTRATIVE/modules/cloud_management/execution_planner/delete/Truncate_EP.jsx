...
import { truncate_execution_planner } from "api/real_time_db/cloud_management/execution_planner_api";
import Button from "assets/elements/Button";

const Truncate_EP = ({ isOpen, onClose, on_success }) => {
  ...
  const handle_confirm = async () => {
    ...
    const confirmMsg =
      deleteMode === "all"
        ? "Are you sure? This will wipe ALL Execution Planner data from the cloud."
        : `Are you sure you want to delete data for TDS Code: ${tdsCode}?`;
    ...
    try {
      ...
      const result = await truncate_execution_planner(target, (progress) => {
        console.log(`EP Deletion progress: ${progress}%`);
      });
      ...
      <div className="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-center justify-between p-5 border-b bg-gray-50">
          <div className="flex items-center gap-2 text-red-600">
            <Trash2 size={20} />
            <h2 className="font-bold text-lg">Truncate EP</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          <div className="bg-amber-50 border-l-4 border-amber-500 p-4 flex gap-3">
            ...
            <p className="text-sm text-amber-800">
              This action is <strong>irreversible</strong>. Deleted Execution
              Planner cloud data cannot be recovered.
            </p>
          </div>

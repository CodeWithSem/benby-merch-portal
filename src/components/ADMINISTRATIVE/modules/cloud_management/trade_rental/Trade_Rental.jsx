...
import {
  ...
  RefreshCw,
  PlusCircle,
  User, // Pwedeng palitan ng Store o Home icon kung mayroon ka
  HardDriveUpload,
  View,
  Trash,
  Edit,
...
// Import ang targeted function para sa Store Code
import { get_trade_rental_by_store_code } from "api/real_time_db/cloud_management/trade_rental_api";
import Upload_TR from "./upload/Upload_TR";
import Truncate_TR from "./delete/Truncate_TR";
...
const Trade_Rental = () => {
  ...
  // Isang input state na lang para sa Store Code
  const [search_mode, set_search_mode] = useState("code");
  const [input_store_code, set_input_store_code] = useState("");
  ...
  const [trade_rent, set_trade_rent] = useState([]);
  ...
  const columns = [
    ...
    { key: "a9_Dateuploaded", label: "DATE UPLOADED", sortable: true },
    ...
    { key: "d9_TypeOfActivity", label: "TYPE OF ACTIVITY", sortable: true },
  ...
  // Handler para mag-load ng data gamit ang Store Code
  const handle_get_as_history_list = async () => {
    if (!input_store_code.trim()) {
    ...
    try {
      set_loading(true);
      set_trade_rent([]); // Fresh state loader

      // Direktang tawag sa bagong store code API helper
      const response = await get_trade_rental_by_store_code(
        input_store_code.trim(),
      );

      set_trade_rent(response);
      set_current_page(1);

      show_toast({
        type: "success",
        title: "Data Loaded",
        message: `Successfully fetched ${response.length} records for Store ${input_store_code.toUpperCase()}.`,
      });
    } catch (error) {
      console.error("Fetch filtered store cloud error:", error);
      show_toast({
        type: "error",
        title: "Fetch Error",
        message: "Failed to load store data from cloud server.",
      });
    } finally {
      set_loading(false);
    }
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(trade_rent, columns);
  ...
  return (
    <React.Fragment>
        ...
        <React.Fragment>
          <div className="w-full">
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              {/* + SECTION 1: CLEAN SINGLE TARGET STORE CODE SEARCH PANEL */}
                  ...
                  <div className="w-full flex items-center gap-2">
                    <div className="w-full">
                      <Icon_Field
                        ...
                        icon={Store}
                        icon_position="left"
                        value={input_store_code}
                        on_change={(e) => set_input_store_code(e.target.value)}
                        on_key_down={(e) =>
                          e.key === "Enter" && handle_get_as_history_list()
                        }
                      />
                    </div>
                    <div className="relative">
                      <Button
                        ...
                        width="w-[140px]"
                        loading={loading}
                        on_click={handle_get_as_history_list}
                      >
                        Load Data
                      </Button>
                    </div>
              ...
              {/* Grid Control Sub-Header Layout */}
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  ...
                  {/* Table Render Implementation Block */}
      ...
      {page === "upload" && (
        <Upload_TR
          set_page={set_page}
          //   on_success={() => handle_get_as_history_list()}
        />
      )}
      {/* - Pages */}
      {/* + Modals */}
      <Truncate_TR
        isOpen={display_modal === "truncate"}

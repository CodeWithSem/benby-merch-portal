...
// Import the new Trade Rental API
import { truncate_trade_rental } from "api/real_time_db/cloud_management/trade_rental_api";
import Button from "assets/elements/Button";

const Truncate_TR = ({ isOpen, onClose, on_success }) => {
  const [deleteMode, setDeleteMode] = useState("all"); // "all" | "specific"
  const [storeCode, setTdsCode] = useState("");
  ...
  const handle_confirm = async () => {
    ...
    // Validation for specific mode (Store Code)
    if (deleteMode === "specific" && !storeCode.trim()) {
      setError("Please enter a valid Store Code.");
      return;
    }

    const confirmMsg =
      deleteMode === "all"
        ? "Are you sure? This will wipe ALL Trade Rental data from the cloud."
        : `Are you sure you want to delete Trade Rentals for Store Code: ${storeCode}?`;
    ...
    try {
      ...
      const target = deleteMode === "all" ? null : storeCode.trim();

      const result = await truncate_trade_rental(target, (progress) => {
        console.log(`TR Deletion progress: ${progress}%`);
      });
      ...
      <div className="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-center justify-between p-5 border-b bg-gray-50">
          <div className="flex items-center gap-2 text-red-600">
            <Trash2 size={20} />
            <h2 className="font-bold text-lg">Truncate Trade Rental</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          <div className="bg-amber-50 border-l-4 border-amber-500 p-4 flex gap-3">
            ...
            <p className="text-sm text-amber-800">
              This action is <strong>irreversible</strong>. Deleted Trade Rental

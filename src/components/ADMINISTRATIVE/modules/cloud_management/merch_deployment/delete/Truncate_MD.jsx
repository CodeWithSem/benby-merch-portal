...
import { truncate_merch_deployment } from "api/real_time_db/cloud_management/merch_deployment_api";
import Button from "assets/elements/Button";

const Truncate_MD = ({ isOpen, onClose, on_success }) => {
  ...
  const handle_confirm = async () => {
    ...
    const confirmMsg =
      deleteMode === "all"
        ? "Are you sure? This will wipe ALL Merch Deployment data from the cloud."
        : `Are you sure you want to delete data for Store: ${storeCode}?`;

    if (!window.confirm(confirmMsg)) return;

    setLoading(true);
    try {
      // If mode is all, pass null. If specific, pass the store code.
      const target = deleteMode === "all" ? null : storeCode.trim();

      const result = await truncate_merch_deployment(target, (progress) => {
      ...
      <div className="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-center justify-between p-5 border-b bg-gray-50">
          <div className="flex items-center gap-2 text-red-600">
            <Trash2 size={20} />
            <h2 className="font-bold text-lg">Truncate MD</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          <div className="bg-amber-50 border-l-4 border-amber-500 p-4 flex gap-3">
            ...
            <p className="text-sm text-amber-800">
              This action is <strong>irreversible</strong>. Deleted cloud data
              cannot be recovered.
            </p>
          </div>

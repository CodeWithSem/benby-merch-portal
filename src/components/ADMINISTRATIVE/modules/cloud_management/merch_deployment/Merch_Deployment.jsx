...
import { get_all_merch_deployments } from "api/real_time_db/cloud_management/merch_deployment_api";
import Truncate_MD from "./delete/Truncate_MD";

const Merch_Deployment = () => {
  ...
  const columns = [
    ...
    { key: "a2_BenbyID", label: "BENBY ID", sortable: true },
    { key: "a3_Storecode", label: "STORE CODE", sortable: true },
    { key: "a3_Storename", label: "STORE NAME", sortable: true },
    { key: "a4_Region", label: "REGION", sortable: true },
    { key: "a5_Agency", label: "AGENCY", sortable: true },
    { key: "a7_PlantillaCode", label: "PLANTILLA", sortable: true },
    { key: "b1_MerchandiserFullName", label: "NAME", sortable: true },
    { key: "b8_DiserStatus", label: "STATUS", sortable: true },
    { key: "c4_DeployStatus", label: "DEPLOYMENT", sortable: true },
    { key: "z1_LoginStatus", label: "LOGIN", sortable: true },
    { key: "b3_Dateuploaded", label: "DATE UPLOAD", sortable: true },
    { key: "b4_UploadedBy", label: "UPLOADED BY", sortable: true },
  ...
  const handle_get_md_list = async () => {
    try {
      set_loading(true);
      const response = await get_all_merch_deployments();
      set_md_list(response);
      set_current_page(1);
    } catch (error) {
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(md_list, columns);
  ...
  return (
    <React.Fragment>
          ...
          <div className="w-full">
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    ...
                    <span className="text-gray-800">Merch Deployment</span>
                  </li>
                </ol>
              </nav>
            </div>
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <h1 className="text-lg">Merch Deployment</h1>
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      variant="danger"
                      ...
                      icon_position="left"
                      width="w-[110px]"
                      on_click={() => set_display_modal("truncate_md")}
                    >
                      Truncate
                    </Button>
                  )}
      ...
      {page === "upload" && (
        <Upload_MD
          set_page={set_page}
          on_success={() => handle_get_md_list()}
        />
      )}
      {/* - PAGES */}
      {/* + MODALS */}
      <Truncate_MD
        isOpen={display_modal === "truncate_md"}
...
export default Merch_Deployment;

...
import { api_create_data_name } from "api/firestore_db/tbl_data_name_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Create_FUNCTION_NAME = ({
  ...
  const handle_change_data_name_code = (value) => {
    set_new_data((prev) => ({
      ...prev,
      data_name_code: value,
    }));
  };

  const handle_change_data_name_desc = (value) => {
    set_new_data((prev) => ({
      ...prev,
      data_name_desc: value,
    }));
  };

  const validate_new_data = () => {
    if (!new_data.data_name_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!new_data.data_name_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_create_data_name = async () => {
    ...
    try {
      ...
      const response = await api_create_data_name(
        new_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_data_name_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm TITLE_NAME Creation
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new TITLE_NAME. Once created, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_create_data_name}
              >
                Yes
              </Button>
              <Button
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">TITLE_NAME Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  value={new_data.data_name_code || ""}
                  on_change={(e) =>
                    handle_change_data_name_code(e.target.value)
                  }
                />
              </div>
              <div className="col-span-2">
                <Text_Field
                  ...
                  value={new_data.data_name_desc || ""}
                  on_change={(e) =>
                    handle_change_data_name_desc(e.target.value)
                  }
                />
              </div>
            </div>
...
export default Create_FUNCTION_NAME;

...
const Select_SDP_Inv_Center = ({
  ...
  const [selected_inv_acc_center, set_selected_inv_acc_center] = useState(null);

  useEffect(() => {
    let data = [...inv_acc_center_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.inv_acc_center_code.toLowerCase().includes(q) ||
          data.inv_acc_center_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_inv_acc_center_list(data.slice(start_idx, end_idx));
  }, [inv_acc_center_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    inv_acc_center_list.filter(
      (data) =>
        data.inv_acc_center_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.inv_acc_center_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_inv_acc_center = () => {
    if (!selected_inv_acc_center) {
      alert("Please select a inv_acc_center before proceeding.");
      return;
    }
    alert(`Selected: ${selected_inv_acc_center.inv_acc_center_desc}`);
  };
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[97] px-4">
        ...
        >
          ...
          <div className="text-lg md:text-xl font-bold mb-5 px-7">
            Inventory Account Center Selection
          </div>
          {/* - Modal Label */}
          {/* + Modal Body */}
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_inv_acc_center_list.length === 0 ? (
                      ...
                      filtered_inv_acc_center_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_inv_acc_center?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_inv_acc_center(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                on_change={() =>
                                  set_selected_inv_acc_center(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_inv_acc_center}
                class_name="w-full md:w-[100px]"
                disabled={!selected_inv_acc_center}
              >
                Proceed
              </Button>
              <Button
...
export default Select_SDP_Inv_Center;

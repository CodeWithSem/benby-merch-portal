...
const Select_Product_Class_1 = ({
  ...
  const [selected_product_class_1, set_selected_product_class_1] =
    useState(null);

  useEffect(() => {
    let data = [...product_class_1_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.product_class_1_code.toLowerCase().includes(q) ||
          data.product_class_1_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_product_class_1_list(data.slice(start_idx, end_idx));
  }, [product_class_1_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    product_class_1_list.filter(
      (data) =>
        data.product_class_1_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.product_class_1_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_product_class_1 = () => {
    if (!selected_product_class_1) {
      alert("Please select a product_class_1 before proceeding.");
      return;
    }
    alert(`Selected: ${selected_product_class_1.product_class_1_desc}`);
  };
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[97] px-4">
        ...
        >
          ...
          <div className="text-lg md:text-xl font-bold mb-5 px-7">
            Product Classification 1 Selection
          </div>
          {/* - Modal Label */}
          {/* + Modal Body */}
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_product_class_1_list.length === 0 ? (
                      ...
                      filtered_product_class_1_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_product_class_1?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_product_class_1(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                on_change={() =>
                                  set_selected_product_class_1(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_product_class_1}
                class_name="w-full md:w-[100px]"
                disabled={!selected_product_class_1}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Product_Class_1;

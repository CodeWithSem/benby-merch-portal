...
const Select_PD_SCON = ({
  ...
  const [selected_scon, set_selected_scon] = useState(null);
  ...
  useEffect(() => {
    ...
    if (search_query.trim() !== "") {
      ...
      data = data.filter(
        (data) =>
          data.scon_code.toLowerCase().includes(q) ||
          data.scon_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_scon_list(data.slice(start_idx, end_idx));
  }, [scon_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    scon_list.filter(
      (data) =>
        data.scon_code.toLowerCase().includes(search_query.toLowerCase()) ||
        data.scon_desc.toLowerCase().includes(search_query.toLowerCase())
  ...
  const handle_select_scon = () => {
    if (!selected_scon) {
      alert("Please select a scon before proceeding.");
      return;
    }
    alert(`Selected: ${selected_scon.scon_desc}`);
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_scon_list.length === 0 ? (
                      ...
                      filtered_scon_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_scon?.id === data.id ? "bg-sky-50" : ""
                          }`}
                          onClick={() => set_selected_scon(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_scon?.id === data.id}
                                on_change={() => set_selected_scon(data)}
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_scon}
                class_name="w-full md:w-[100px]"
                disabled={!selected_scon}
              >
                Proceed
              </Button>
              <Button

...
const Select_Generic = ({
  ...
  useEffect(() => {
    ...
    if (search_query.trim() !== "") {
      ...
      data = data.filter(
        (d) =>
          d[source_code]?.toLowerCase().includes(q) ||
          d[source_desc]?.toLowerCase().includes(q)
      );
    }
  ...
  const total_pages = Math.ceil(
    source_list.filter(
      (d) =>
        d[source_code]?.toLowerCase().includes(search_query.toLowerCase()) ||
        d[source_desc]?.toLowerCase().includes(search_query.toLowerCase())
    ).length / rows_per_page
  );

  const handle_page_change = (page) => set_current_page(page);

  // SELECT HANDLER (Generic version of your logic)
  const handle_select = () => {
    if (!selected_item) {
    ...
    const value = selected_item[source_code];

    set_data((prev) => {
      const updated = { ...prev };

      if (Array.isArray(target_field)) {
        target_field.forEach((field) => {
          updated[field] = value;
        });
      } else {
  ...
  // RETURN ORIGIN with your exact design preserved
  return is_open ? (
    <React.Fragment>

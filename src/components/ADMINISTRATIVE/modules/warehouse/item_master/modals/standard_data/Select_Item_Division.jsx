...
const Select_Item_Division = ({
  ...
  const [selected_item_division, set_selected_item_division] = useState(null);

  useEffect(() => {
    let data = [...item_division_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.item_division_code.toLowerCase().includes(q) ||
          data.item_division_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_item_division_list(data.slice(start_idx, end_idx));
  }, [item_division_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    item_division_list.filter(
      (data) =>
        data.item_division_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.item_division_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_item_division = () => {
    if (!selected_item_division) {
      alert("Please select a item_division before proceeding.");
      return;
    }
    alert(`Selected: ${selected_item_division.item_division_desc}`);
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_item_division_list.length === 0 ? (
                      ...
                      filtered_item_division_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_item_division?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_item_division(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_item_division?.id === data.id}
                                on_change={() =>
                                  set_selected_item_division(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_item_division}
                class_name="w-full md:w-[100px]"
                disabled={!selected_item_division}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Item_Division;

...
import {
  api_create_item_ext_pu,
  api_delete_item_ext_pu,
  api_get_item_ext_pu_list,
  api_truncate_item_ext_pu,
} from "api/firestore_db/warehouse/item_master/tbl_item_ext_pu_api";
...
const Purchasing = ({ active_user, show_toast, item_extension_data }) => {
  ...
  const [item_ext_pu_list, set_item_ext_pu_list] = useState([]);

  const handle_get_item_ext_pu_list = async () => {
    set_loading_list(true);
    const response = await api_get_item_ext_pu_list(
      item_extension_data.item_code
    );
    if (response.success) {
      set_item_ext_pu_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_item_ext_pu_list();
  }, []);

  // + Client-Side Filtering
  const [filtered_item_ext_pu_list, set_filtered_item_ext_pu_list] = useState(
    []
  );
  ...
  useEffect(() => {
    ...
    set_filtered_item_ext_pu_list(indexed_data);
  }, [
    item_ext_pu_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : item_ext_pu_list.length) / select_option
  );
  ...
  const handle_add_extension = async () => {
    ...
    try {
      ...
      console.table(new_item_ext_pu_data);
      const response = await api_create_item_ext_pu(
        new_item_ext_pu_data,
        active_user?.username
      );
      if (response.success) {
        set_item_ext_pu_list((prev) => [...prev, response.data]);
        set_selected_data({
          branch_code: "",
          plant_code: "",
          sloc_code: "",
  ...
  const delete_extension = async () => {
    ...
    try {
      ...
      const response = await api_delete_item_ext_pu(id);
      if (response.success) {
        set_item_ext_pu_list((prev) => prev.filter((item) => item.id !== id));
  ...
  const handle_truncate = async () => {
    ...
    const response = await api_truncate_item_ext_pu();
    if (response.success) {
    ...
    <React.Fragment>
      ...
      <div className="w-full border rounded-lg">
        <div className="w-full md:flex md:justify-between p-4 gap-4">
          <div className="flex items-center text-sm gap-2">
            ...
            <Button
              ...
              on_click={handle_get_item_ext_pu_list}
            ></Button>
          </div>
        ...
        <div className="overflow-x-auto">
          {loading_list ? (
          ...
          ) : filtered_item_ext_pu_list.length === 0 ? (
            ...
            <table className="min-w-full">
              ...
              <tbody className="bg-white">
                {filtered_item_ext_pu_list.map((row, idx) => {
          ...
          <div className="mt-2 flex justify-end gap-2">
            {active_user?.category === "DEV" && (
              <Button
                ...
                disabled={item_ext_pu_list.length === 0}
              >
                Truncate
              </Button>
            )}

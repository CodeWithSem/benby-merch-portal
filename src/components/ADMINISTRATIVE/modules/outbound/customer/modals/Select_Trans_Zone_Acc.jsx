...
const Select_Trans_Zone_Acc = ({
  ...
  const [selected_trans_zone, set_selected_trans_zone] = useState(null);
  ...
  const handle_select_trans_zone = () => {
    if (!selected_trans_zone) {
      alert("Please select a trans_zone before proceeding.");
      return;
    }
    alert(`Selected: ${selected_trans_zone.trans_zone_desc}`);
  };
        ...
        >
                      ...
                      filtered_trans_zone_list.map((data) => (
                        <tr
                          ...
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_trans_zone?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_trans_zone(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_trans_zone?.id === data.id}
                                on_change={() => set_selected_trans_zone(data)}
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_trans_zone}
                class_name="w-full md:w-[100px]"
                disabled={!selected_trans_zone}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Trans_Zone_Acc;

...
const Select_Sales_Office = ({
  ...
  const [selected_sales_office, set_selected_sales_office] = useState(null);

  useEffect(() => {
    let data = [...sales_office_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.sales_office_code.toLowerCase().includes(q) ||
          data.sales_office_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_sales_office_list(data.slice(start_idx, end_idx));
  }, [sales_office_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    sales_office_list.filter(
      (data) =>
        data.sales_office_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.sales_office_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_sales_office = () => {
    if (!selected_sales_office) {
      alert("Please select a sales_office before proceeding.");
      return;
    }
    alert(`Selected: ${selected_sales_office.sales_office_desc}`);
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_sales_office_list.length === 0 ? (
                      ...
                      filtered_sales_office_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_sales_office?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_sales_office(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_sales_office?.id === data.id}
                                on_change={() =>
                                  set_selected_sales_office(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_sales_office}
                class_name="w-full md:w-[100px]"
                disabled={!selected_sales_office}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Sales_Office;

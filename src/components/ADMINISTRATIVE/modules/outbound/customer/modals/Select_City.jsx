...
const Select_City = ({
  ...
  const [selected_city, set_selected_city] = useState(null);
  ...
  const handle_select_city = () => {
    if (!selected_city) {
      alert("Please select a city before proceeding.");
      return;
    }
    alert(`Selected: ${selected_city.city_desc}`);
  };
                      ...
                      filtered_city_list.map((data) => (
                        <tr
                          ...
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_city?.id === data.id ? "bg-sky-50" : ""
                          }`}
                          onClick={() => set_selected_city(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_city?.id === data.id}
                                on_change={() => set_selected_city(data)}
                              />
                            </div>
                          </td>

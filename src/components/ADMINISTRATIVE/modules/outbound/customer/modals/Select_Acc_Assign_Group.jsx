...
const Select_Acc_Assign_Group = ({
  ...
  const [filtered_acc_assign_group_list, set_filtered_acc_assign_group_list] =
    useState([]);
  const [current_page, set_current_page] = useState(1);
  const [rows_per_page, set_rows_per_page] = useState(5);
  const [search_query, set_search_query] = useState("");
  const [selected_acc_assign_group, set_selected_acc_assign_group] =
    useState(null);

  useEffect(() => {
    let data = [...acc_assign_group_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.acc_assign_group_code.toLowerCase().includes(q) ||
          data.acc_assign_group_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_acc_assign_group_list(data.slice(start_idx, end_idx));
  }, [acc_assign_group_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    acc_assign_group_list.filter(
      (data) =>
        data.acc_assign_group_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.acc_assign_group_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_acc_assign_group = () => {
    if (!selected_acc_assign_group) {
      alert("Please select a acc_assign_group before proceeding.");
      return;
    }
    alert(`Selected: ${selected_acc_assign_group.acc_assign_group_desc}`);
  };
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[97] px-4">
        ...
        >
          ...
          <div className="text-lg md:text-xl font-bold mb-5 px-7">
            Accounting Assignment Group Selection
          </div>
          {/* - Modal Label */}
          {/* + Modal Body */}
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_acc_assign_group_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_acc_assign_group_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_acc_assign_group?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_acc_assign_group(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                on_change={() =>
                                  set_selected_acc_assign_group(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_acc_assign_group}
                class_name="w-full md:w-[100px]"
                disabled={!selected_acc_assign_group}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Acc_Assign_Group;

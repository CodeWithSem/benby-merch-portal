...
const Select_Tax_Code = ({
  ...
  const [filtered_tax_list, set_filtered_tax_list] = useState([]);
  const [current_page, set_current_page] = useState(1);
  const [rows_per_page, set_rows_per_page] = useState(5);
  const [search_query, set_search_query] = useState("");
  const [selected_tax, set_selected_tax] = useState(null);
  ...
  useEffect(() => {
    ...
    if (search_query.trim() !== "") {
      ...
      data = data.filter(
        (data) =>
          data.tax_code.toLowerCase().includes(q) ||
          data.tax_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_tax_list(data.slice(start_idx, end_idx));
  }, [tax_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    tax_list.filter(
      (data) =>
        data.tax_code.toLowerCase().includes(search_query.toLowerCase()) ||
        data.tax_desc.toLowerCase().includes(search_query.toLowerCase())
  ...
  const handle_select_tax = () => {
    if (!selected_tax) {
      alert("Please select a tax before proceeding.");
      return;
    }
    alert(`Selected: ${selected_tax.tax_desc}`);
  };
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_tax_list.length === 0 ? (
                      ...
                      filtered_tax_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_tax?.id === data.id ? "bg-sky-50" : ""
                          }`}
                          onClick={() => set_selected_tax(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_tax?.id === data.id}
                                on_change={() => set_selected_tax(data)}
                              />
                            </div>
                          </td>
...
export default Select_Tax_Code;

...
const Select_Partner_Func = ({
  ...
  const [filtered_partner_func_list, set_filtered_partner_func_list] = useState(
    []
  );
  const [current_page, set_current_page] = useState(1);
  const [rows_per_page, set_rows_per_page] = useState(5);
  const [search_query, set_search_query] = useState("");
  const [selected_partner_func, set_selected_partner_func] = useState(null);

  useEffect(() => {
    let data = [...partner_func_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.partner_func_code.toLowerCase().includes(q) ||
          data.partner_func_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_partner_func_list(data.slice(start_idx, end_idx));
  }, [partner_func_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    partner_func_list.filter(
      (data) =>
        data.partner_func_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.partner_func_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_partner_func = () => {
    if (!selected_partner_func) {
      alert("Please select a partner_func before proceeding.");
      return;
    }
    alert(`Selected: ${selected_partner_func.partner_func_desc}`);
  };
        ...
        >
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_partner_func_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_partner_func_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_partner_func?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_partner_func(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_partner_func?.id === data.id}
                                on_change={() =>
                                  set_selected_partner_func(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_partner_func}
                class_name="w-full md:w-[100px]"
                disabled={!selected_partner_func}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Partner_Func;

...
const Select_Sales_District = ({
  ...
  const [selected_sales_district, set_selected_sales_district] = useState(null);

  useEffect(() => {
    let data = [...sales_district_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.sales_district_code.toLowerCase().includes(q) ||
          data.sales_district_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_sales_district_list(data.slice(start_idx, end_idx));
  }, [sales_district_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    sales_district_list.filter(
      (data) =>
        data.sales_district_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.sales_district_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_sales_district = () => {
    if (!selected_sales_district) {
      alert("Please select a sales_district before proceeding.");
      return;
    }
    alert(`Selected: ${selected_sales_district.sales_district_desc}`);
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_sales_district_list.length === 0 ? (
                      ...
                      filtered_sales_district_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_sales_district?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_sales_district(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                on_change={() =>
                                  set_selected_sales_district(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_sales_district}
                class_name="w-full md:w-[100px]"
                disabled={!selected_sales_district}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Sales_District;

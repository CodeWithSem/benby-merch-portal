...
const Select_CS_Acc_Group = ({
  ...
  const [filtered_cs_acc_group_list, set_filtered_cs_acc_group_list] = useState(
    []
  );
  const [current_page, set_current_page] = useState(1);
  const [rows_per_page, set_rows_per_page] = useState(5);
  const [search_query, set_search_query] = useState("");
  const [selected_cs_acc_group, set_selected_cs_acc_group] = useState(null);

  useEffect(() => {
    let data = [...cs_acc_group_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.cs_acc_group_code.toLowerCase().includes(q) ||
          data.cs_acc_group_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_cs_acc_group_list(data.slice(start_idx, end_idx));
  }, [cs_acc_group_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    cs_acc_group_list.filter(
      (data) =>
        data.cs_acc_group_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.cs_acc_group_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_cs_acc_group = () => {
    if (!selected_cs_acc_group) {
      alert("Please select a cs_acc_group before proceeding.");
      return;
    }
    alert(`Selected: ${selected_cs_acc_group.cs_acc_group_desc}`);
  };
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[97] px-4">
        ...
        >
          ...
          <div className="text-lg md:text-xl font-bold mb-5 px-7">
            Customer Account Group Selection
          </div>
          {/* - Modal Label */}
          {/* + Modal Body */}
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_cs_acc_group_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_cs_acc_group_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_cs_acc_group?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_cs_acc_group(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_cs_acc_group?.id === data.id}
                                on_change={() =>
                                  set_selected_cs_acc_group(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_cs_acc_group}
                class_name="w-full md:w-[100px]"
                disabled={!selected_cs_acc_group}
              >
                Proceed
              </Button>
              <Button
...
export default Select_CS_Acc_Group;

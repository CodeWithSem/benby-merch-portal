...
const Select_Business_Area = ({
  ...
  const [selected_business_area, set_selected_business_area] = useState(null);

  useEffect(() => {
    let data = [...business_area_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.business_area_code.toLowerCase().includes(q) ||
          data.business_area_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_business_area_list(data.slice(start_idx, end_idx));
  }, [business_area_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    business_area_list.filter(
      (data) =>
        data.business_area_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.business_area_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_business_area = () => {
    if (!selected_business_area) {
      alert("Please select a business_area before proceeding.");
      return;
    }
    alert(`Selected: ${selected_business_area.business_area_desc}`);
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_business_area_list.length === 0 ? (
                      ...
                      filtered_business_area_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_business_area?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_business_area(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_business_area?.id === data.id}
                                on_change={() =>
                                  set_selected_business_area(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_business_area}
                class_name="w-full md:w-[100px]"
                disabled={!selected_business_area}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Business_Area;

...
// + Import Customer List for Lookup
import { customer_master_list } from "assets/data/customer_master_list";
// - Import Customer List

import { api_get_sales_order_list_by_date } from "api/firestore_db/outbound/sales_order/tbl_sales_order_api";

const Select_SO = ({
  is_open,
  on_close,
  width = "max-w-[900px]", // Increased width to accommodate the new column
  height = "h-[500px]",
  show_toast,
  so_start_date,
  set_so_start_date,
  ...
  const [selected_so, set_selected_so] = useState({});
  ...
  useEffect(() => {
    let temp = so_list.filter(
      (so) => so.so_status === "Posted" || so.so_status === "Partially Issued",
    );
    ...
    if (debounced_query.trim() !== "") {
      ...
      temp = temp.filter((so) => {
        ...
        // Customer lookup for search
        const customer = customer_master_list?.find(
        ...
        const fields = [
          ...
          customer?.customer_desc, // Added customer name to search fields
          so.creation_date,
        ];
  ...
  const handle_change_so_start_date = (value) =>
    set_so_start_date(format_date_1(value));
  const handle_change_so_end_date = (value) =>
    set_so_end_date(format_date_1(value));
  const handle_load_data = () => handle_get_sales_order_list();
          ...
          <div className="overflow-hidden border border-gray-200 bg-white pt-4">
            ...
            <div className="px-6 mb-5 grid grid-cols-1 gap-5 md:w-[800px] md:grid-cols-3">
              <Date_Field
                label="Start Date"
                value={so_start_date}
                on_change={(e) => handle_change_so_start_date(e.target.value)}
                placeholder="Select Date"
              />
              <Date_Field
                label="End Date"
                value={so_end_date}
                on_change={(e) => handle_change_so_end_date(e.target.value)}
                placeholder="Select Date"
              />
            ...
            <div className="flex flex-col gap-5 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="w-full">
                <Icon_Field
                  name="search"
                  placeholder="Search SO, Type, or Customer..."
                  icon={Search}
                  icon_position="left"
                  value={search_query}
            ...
            <div className="max-w-full overflow-x-auto custom-scrollbar">
              <table className="min-w-full whitespace-nowrap">
                <thead className="border-gray-100 border-y bg-gray-50">
                  <tr className="font-semibold text-xs text-gray-500">
                    <th className="px-6 py-3 w-[80px]"></th>
                    <th className="px-6 py-3 text-left">SO Number</th>
                    <th className="px-6 py-3 text-left">SO Type</th>
                        ...
                        >
                          ...
                          <td className="px-5 py-4 sm:px-6">
                            <div className="font-medium">
                              ...
                              <span className="block text-gray-800 truncate max-w-[200px]">
                                {customer?.customer_desc || "Unknown Customer"}
                              </span>
                            </div>
                          </td>
                          {/* - Customer Column Cell */}
        ...
        <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
          ...
          <div className="flex justify-center sm:justify-end gap-2 w-full">
            <Button
              ...
              disabled={!selected_so?.id || so_list.length === 0}
            >
              Proceed
            </Button>
            <Button

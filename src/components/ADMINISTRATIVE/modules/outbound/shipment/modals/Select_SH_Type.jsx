...
const Select_SH_Type = ({
  ...
  const [selected_sh_type, set_selected_sh_type] = useState(null);

  useEffect(() => {
    let data = [...sh_type_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.sh_type_code.toLowerCase().includes(q) ||
          data.sh_type_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_sh_type_list(data.slice(start_idx, end_idx));
  }, [sh_type_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    sh_type_list.filter(
      (data) =>
        data.sh_type_code.toLowerCase().includes(search_query.toLowerCase()) ||
        data.sh_type_desc.toLowerCase().includes(search_query.toLowerCase())
  ...
  const handle_select_plant = () => {
    ...
    alert(`Selected: ${selected_sh_type.sh_type_desc}`);
  };
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_sh_type_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_sh_type_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_sh_type?.id === data.id ? "bg-sky-50" : ""
                          }`}
                          onClick={() => set_selected_sh_type(data)}
                        >
                          <td className="px-5 py-4 sm:px-6">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_sh_type?.id === data.id}
                                on_change={() => set_selected_sh_type(data)}
                              />
                            </div>
                          </td>
                          <td className="px-5 py-4 sm:px-6">

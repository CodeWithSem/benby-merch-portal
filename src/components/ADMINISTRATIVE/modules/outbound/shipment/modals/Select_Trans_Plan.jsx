...
const Select_Trans_Plan = ({
  ...
  const [selected_trans_plan, set_selected_trans_plan] = useState(null);

  useEffect(() => {
    let data = [...trans_plan_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.trans_plan_code.toLowerCase().includes(q) ||
          data.trans_plan_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_trans_plan_list(data.slice(start_idx, end_idx));
  }, [trans_plan_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    trans_plan_list.filter(
      (data) =>
        data.trans_plan_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.trans_plan_desc.toLowerCase().includes(search_query.toLowerCase())
  ...
  const handle_select_plant = () => {
    if (!selected_trans_plan) {
      alert("Please select a plant before proceeding.");
      return;
    }
    alert(`Selected: ${selected_trans_plan.trans_plan_desc}`);
  };
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_trans_plan_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_trans_plan_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_trans_plan?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_trans_plan(data)}
                        >
                          <td className="px-5 py-4 sm:px-6">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_trans_plan?.id === data.id}
                                on_change={() => set_selected_trans_plan(data)}
                              />
                            </div>
                          </td>
                          <td className="px-5 py-4 sm:px-6">
...
export default Select_Trans_Plan;

...
const Select_Forward_Agent = ({
  ...
  const [selected_forward_agent, set_selected_forward_agent] = useState(null);

  useEffect(() => {
    let data = [...forward_agent_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.forward_agent_code.toLowerCase().includes(q) ||
          data.forward_agent_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_forward_agent_list(data.slice(start_idx, end_idx));
  }, [forward_agent_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    forward_agent_list.filter(
      (data) =>
        data.forward_agent_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.forward_agent_desc
          .toLowerCase()
          .includes(search_query.toLowerCase())
  ...
  const handle_select_plant = () => {
    if (!selected_forward_agent) {
      alert("Please select a plant before proceeding.");
      return;
    }
    alert(`Selected: ${selected_forward_agent.forward_agent_desc}`);
  };
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_forward_agent_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_forward_agent_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_forward_agent?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_forward_agent(data)}
                        >
                          <td className="px-5 py-4 sm:px-6">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_forward_agent?.id === data.id}
                                on_change={() =>
                                  set_selected_forward_agent(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                ...
                disabled={!selected_forward_agent}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Forward_Agent;

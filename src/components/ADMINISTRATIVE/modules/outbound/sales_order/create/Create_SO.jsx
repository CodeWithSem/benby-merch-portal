...
const Create_SO = ({ set_page, active_user, so_data }) => {
  ...
  const handle_create = async () => {
    ...
    try {
      ...
      if (response.success) {
        set_so_list((prev) => [...prev, response.data]);
        handle_go_back();
      }
    } catch (error) {
  ...
  return (
    <React.Fragment>
      ...
      <Confirm_Modal
        ...
        title="Confirm Sales Order Creation"
        description_1="You are about to create a new Sales Order. Once created, it will be added to the database."

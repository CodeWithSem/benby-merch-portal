...
const Select_Truck_Type = ({
  ...
  const [selected_truck_type, set_selected_truck_type] = useState(null);

  useEffect(() => {
    let data = [...truck_type_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.truck_type_code.toLowerCase().includes(q) ||
          data.truck_type_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_truck_type_list(data.slice(start_idx, end_idx));
  }, [truck_type_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    truck_type_list.filter(
      (data) =>
        data.truck_type_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.truck_type_desc.toLowerCase().includes(search_query.toLowerCase())
  ...
  const handle_select_truck_type = () => {
    if (!selected_truck_type) {
      alert("Please select a truck_type before proceeding.");
      return;
    }
    alert(`Selected: ${selected_truck_type.truck_type_desc}`);
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_truck_type_list.length === 0 ? (
                      ...
                      filtered_truck_type_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_truck_type?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_truck_type(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_truck_type?.id === data.id}
                                on_change={() => set_selected_truck_type(data)}
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_truck_type}
                class_name="w-full md:w-[100px]"
                disabled={!selected_truck_type}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Truck_Type;

...
import Create_User_Role from "./functions/Create_User_Role";
...
const User_Role = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_USER_ROLE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "user_role_code", label: "User Role Code", sortable: true },
    { key: "user_role_desc", label: "User Role Description", sortable: true },
  ...
  const handle_get_user_role_list = async () => {
    set_loading_list(true);
    const response = await api_get_user_role_list();
    if (response.success) {
      set_user_role_list(response.data);
    } else {
  ...
  const handle_truncate_user_role_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_user_role();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  useEffect(() => {
    ...
    set_filtered_user_role_list(indexed_data);
  }, [
    user_role_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : user_role_list.length) / select_option
  );
  ...
  const handle_create_new_user_role = () => {
    set_sub_page("create_new_user_role");
  };

  const handle_edit_user_role = (data) => {
    set_edit_data(data);
    set_sub_page("edit_user_role");
  };
  const handle_delete_user_role = (data) => {
    set_delete_data(data);
    set_display_modal("delete_user_role");
  };

  const handle_upload_user_role = () => {
    set_sub_page("upload_user_role");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_user_role_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_user_role}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_user_role}
                  >
                    Upload
                  </Button>
                </div>
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_user_role_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_user_role_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "upload_user_role" && (
        <Upload_User_Role
          handle_go_back={handle_go_back}
          handle_get_user_role_list={handle_get_user_role_list}
          show_toast={show_toast}
        />
      )}
      <Delete_User_Role
        is_open={display_modal === "delete_user_role"}

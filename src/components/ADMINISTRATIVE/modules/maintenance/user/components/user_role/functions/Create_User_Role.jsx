...
import { api_create_user_role } from "api/firestore_db/maintenance/user/tbl_user_role_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Create_User_Role = ({
  ...
  const handle_change_user_role_code = (value) => {
    set_new_data((prev) => ({
      ...prev,
      user_role_code: value,
    }));
  };

  const handle_change_user_role_desc = (value) => {
    set_new_data((prev) => ({
      ...prev,
      user_role_desc: value,
    }));
  };
  ...
  const validate_new_data = () => {
    ...
    if (!new_data.user_role_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_create_user_role = async () => {
    ...
    try {
      ...
      const response = await api_create_user_role(
        new_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_user_role_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm User Role Creation
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new User Role. Once created, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_create_user_role}
              >
                Yes
              </Button>
              <Button
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">User Role Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  value={new_data.user_role_code || ""}
                  on_change={(e) =>
                    handle_change_user_role_code(e.target.value)
                  }
                />
              </div>
              <div className="col-span-2">
                <Text_Field
                  ...
                  value={new_data.user_role_desc || ""}
                  on_change={(e) =>
                    handle_change_user_role_desc(e.target.value)
                  }
                />
              </div>
            </div>

...
import { api_bulk_upload_user_category } from "api/firestore_db/maintenance/user/tbl_user_category_api";
...
const Upload_User_Category = ({
  ...
  const [upload_user_category_list, set_upload_user_category_list] = useState(
    []
  );

  // + Client-Side Filtering
  const [
    filtered_upload_user_category_list,
    set_filtered_upload_user_category_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_user_category_list];
    ...
    set_filtered_upload_user_category_list(indexed_data);
  }, [
    upload_user_category_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_user_category_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_user_category_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "user_category_code",
      desc_field: "user_category_desc",
      set_data_callback: set_upload_user_category_list,
      show_toast,
    });
  };

  const handle_upload_user_category = async () => {
    if (upload_user_category_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_user_category(
        upload_user_category_list
      );
      ...
      if (response.success) {
        show_toast({
          ...
          message: `${upload_user_category_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_user_category_list([]);
        handle_get_user_category_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload User Category</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_user_category_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_user_category_list.map((row, idx) => {
...
export default Upload_User_Category;

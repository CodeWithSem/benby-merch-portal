...
import { api_delete_incoterms } from "api/firestore_db/maintenance/vendor/tbl_incoterms_api";
import { format_date_1, get_date_now } from "assets/scripts/format";
import { CheckCircle2, CircleX, X } from "lucide-react";
import Text_Field from "assets/elements/Text_Field";
import Button from "assets/elements/Button";

const Delete_Incoterms = ({
  is_open,
  on_close,
  width = "max-w-[700px]",
  show_toast,
  ...
  const handle_delete_incoterms = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_incoterms(id);

      if (response.success) {
        set_incoterms_list((prev) => prev.filter((item) => item.id !== id));
        show_status("success");
        close_modal();
      } else {
        show_status("error");
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to delete this Incoterms.
            </p>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={() => handle_delete_incoterms(delete_data.id)}
              >
                Yes
              </Button>
              <Button
            ...
            <div className="w-full">
              <div className="w-full bg-white rounded-lg border">
                <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                  <h1 className="text-lg">Incoterms Details</h1>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.incoterms_code}
                        disabled
                      />
                    </div>
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.incoterms_desc}
                        disabled
                      />
                    </div>
                  </div>
...
export default Delete_Incoterms;

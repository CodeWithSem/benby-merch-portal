...
import Create_Source_Hub from "./functions/Create_Source_Hub";
...
const Source_Hub = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_SOURCE_HUB", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const columns = [
    ...
    { key: "source_hub_code", label: "Source Hub Code", sortable: true },
    {
      key: "source_hub_desc",
      label: "Source Hub Description",
      sortable: true,
    },
  ...
  const handle_get_source_hub_list = async () => {
    ...
    const response = await api_get_source_hub_list();
    if (response.success) {
      set_source_hub_list(response.data);
    } else {
  ...
  const handle_truncate_source_hub_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_source_hub();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_source_hub_list, set_filtered_source_hub_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_source_hub_list(indexed_data);
  }, [
    source_hub_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : source_hub_list.length) / select_option
  );
  ...
  const handle_create_new_source_hub = () => {
    set_sub_page("create_new_source_hub");
  };

  const handle_edit_source_hub = (data) => {
    set_edit_data(data);
    set_sub_page("edit_source_hub");
  };
  const handle_delete_source_hub = (data) => {
    set_delete_data(data);
    set_display_modal("delete_source_hub");
  };

  const handle_upload_source_hub = () => {
    set_sub_page("upload_source_hub");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Source Hub</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_source_hub_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_source_hub}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_source_hub}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_source_hub_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_source_hub_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_source_hub_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_source_hub" && (
        <Create_Source_Hub
      ...
      {sub_page === "upload_source_hub" && (
        <Upload_Source_Hub
          handle_go_back={handle_go_back}
          handle_get_source_hub_list={handle_get_source_hub_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Source_Hub
        is_open={display_modal === "delete_source_hub"}

// File Name: Item_Status
// Function Name: Item_Status
// Data Name: item_status
// Column Name: Item Status
// Table Name: TBL_ITEM_STATUS
...
import Create_Item_Status from "./functions/Create_Item_Status";
import Edit_Item_Status from "./functions/Edit_Item_Status";
import Delete_Item_Status from "./functions/Delete_Item_Status";
import Upload_Item_Status from "./functions/Upload_Item_Status";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Item_Status = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_item_status_data });
  const [edit_data, set_edit_data] = useState({ ...def_item_status_data });
  const [delete_data, set_delete_data] = useState({ ...def_item_status_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_ITEM_STATUS", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        item_status_code: `ITM-S-${String(value).padStart(2, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "item_status_code", label: "Item Status Code", sortable: true },
    {
      key: "item_status_desc",
  ...
  const [item_status_list, set_item_status_list] = useState([]);

  const handle_get_item_status_list = async () => {
    set_loading_list(true);
    const response = await api_get_item_status_list();
    if (response.success) {
      set_item_status_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_item_status_list();
  }, []);

  const handle_truncate_item_status_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_item_status();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_item_status_list, set_filtered_item_status_list] = useState(
    []
  );
  ...
  useEffect(() => {
    ...
    set_filtered_item_status_list(indexed_data);
  }, [
    item_status_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : item_status_list.length) / select_option
  );
  ...
  const handle_create_new_item_status = () => {
    set_sub_page("create_new_item_status");
  };

  const handle_edit_item_status = (data) => {
    set_edit_data(data);
    set_sub_page("edit_item_status");
  };
  const handle_delete_item_status = (data) => {
    set_delete_data(data);
    set_display_modal("delete_item_status");
  };

  const handle_upload_item_status = () => {
    set_sub_page("upload_item_status");
  };
  ...
  return (
    <React.Fragment>
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Item Status</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_item_status_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_item_status}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_item_status}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_item_status_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_item_status_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_item_status_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_item_status" && (
        <Create_Item_Status
          ...
          set_item_status_list={set_item_status_list}
        />
      )}
      {sub_page === "edit_item_status" && (
        <Edit_Item_Status
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_item_status" && (
        <Upload_Item_Status
          handle_go_back={handle_go_back}
          handle_get_item_status_list={handle_get_item_status_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Item_Status
        is_open={display_modal === "delete_item_status"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_item_status_increment}
      />
    </React.Fragment>
  );
};

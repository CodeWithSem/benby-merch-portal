...
import {
  ...
  api_set_branch_hierarchy_increment,
...
import Create_Branch_H from "./functions/Create_Branch_H";
import Edit_Branch_H from "./functions/Edit_Branch_H";
import Delete_Branch_H from "./functions/Delete_Branch_H";
import Upload_Branch_H from "./functions/Upload_Branch_H";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Branch_Hierarchy = ({ set_page }) => {
  ...
  const handle_get_all_lists = async () => {
    try {
      const [branch_response, plant_response] = await Promise.all([
        api_get_branch_list(),
        api_get_plant_list(),
      ]);
      if (branch_response.success && plant_response.success) {
        set_branch_list(branch_response.data);
        set_plant_list(plant_response.data);
        const res_log = {
          branch_log: branch_response.data,
        ...
        console.error("One or more list fetches failed.", {
          branch: branch_response.message,
  ...
  const def_branch_hierarchy_data = {
    id: null,
    branch_code: "",
    plant_code: "",
    creation_date: "",
  ...
  const [new_data, set_new_data] = useState({ ...def_branch_hierarchy_data });
  const [edit_data, set_edit_data] = useState({
    ...def_branch_hierarchy_data,
  });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_BRANCH_HIERARCHY", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const [branch_hierarchy_list, set_branch_hierarchy_list] = useState([]);

  const handle_get_branch_hierarchy_list = async () => {
    set_loading_list(true);
    const response = await api_get_branch_hierarchy_list();
    if (response.success) {
      set_branch_hierarchy_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_branch_hierarchy_list();
  }, []);

  const handle_truncate_branch_hierarchy_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_branch_hierarchy();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_branch_hierarchy_list, set_filtered_branch_hierarchy_list] =
    useState([]);
  ...
  const lookup_columns = [
    { code_key: "branch_code", list: branch_list, desc_key: "branch_desc" },
    {
      code_key: "plant_code",
      list: plant_list,
      desc_key: "plant_desc",
  ...
  useEffect(() => {
    let temp = [...branch_hierarchy_list];
    ...
    set_filtered_branch_hierarchy_list(indexed_data);
  }, [
    branch_hierarchy_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? branch_hierarchy_list.filter((u) => {
          const q = debounced_query.toLowerCase();
      ...
      : branch_hierarchy_list.length) / select_option
  );
  ...
  const handle_create_new_branch_hierarchy = () => {
    set_sub_page("create_new_branch_hierarchy");
  };

  const handle_edit_branch_hierarchy = (data) => {
    set_edit_data(data);
    set_sub_page("edit_branch_hierarchy");
  };
  const handle_delete_branch_hierarchy = (data) => {
    set_delete_data(data);
    set_display_modal("delete_branch_hierarchy");
  };

  const handle_upload_branch_hierarchy = () => {
    set_sub_page("upload_branch_hierarchy");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Branch Hierarchy</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Branch Hierarchy</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_branch_hierarchy_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_branch_hierarchy}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_branch_hierarchy}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_branch_hierarchy_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_branch_hierarchy_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_branch_hierarchy_list.map((row, idx) => {
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_branch_hierarchy(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_branch_hierarchy(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_branch_hierarchy" && (
        <Create_Branch_H
          ...
          set_branch_hierarchy_list={set_branch_hierarchy_list}
          branch_list={branch_list}
          plant_list={plant_list}
        />
      )}
      {sub_page === "edit_branch_hierarchy" && (
        <Edit_Branch_H
          handle_go_back={handle_go_back}
          active_user={active_user}
          reset_new_data={reset_new_data}
      ...
      {sub_page === "upload_branch_hierarchy" && (
        <Upload_Branch_H
          handle_go_back={handle_go_back}
          handle_get_branch_hierarchy_list={handle_get_branch_hierarchy_list}
          show_toast={show_toast}
          branch_list={branch_list}
          plant_list={plant_list}
        />
      ...
      <Delete_Branch_H
        is_open={display_modal === "delete_branch_hierarchy"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_branch_hierarchy_increment}
      />
      <Load_Screen
...
export default Branch_Hierarchy;

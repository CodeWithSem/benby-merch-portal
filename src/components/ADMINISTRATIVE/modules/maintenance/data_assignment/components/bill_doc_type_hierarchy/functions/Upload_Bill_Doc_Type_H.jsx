...
import { api_bulk_upload_bill_doc_type_hierarchy } from "api/firestore_db/maintenance/data_assignment/tbl_bill_doc_type_hierarchy_api";
...
const Upload_Bill_Doc_Type_H = ({
  ...
  const [
    upload_bill_doc_type_hierarchy_list,
    set_upload_bill_doc_type_hierarchy_list,
  ] = useState([]);

  // + Client-Side Filtering
  const [
    filtered_upload_bill_doc_type_hierarchy_list,
    set_filtered_upload_bill_doc_type_hierarchy_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_bill_doc_type_hierarchy_list];
    ...
    set_filtered_upload_bill_doc_type_hierarchy_list(indexed_data);
  }, [
    upload_bill_doc_type_hierarchy_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    ...
    (debounced_query
      ? upload_bill_doc_type_hierarchy_list.filter((u) => {
          const q = debounced_query.toLowerCase();
      ...
      : upload_bill_doc_type_hierarchy_list.length) / // <-- Use full list length if no query
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_3_hierarchy({
      e,
      code_field_1: "deliv_doc_type_code",
      code_field_2: "bill_doc_type_code",
      code_field_3: "bill_cancel_code",
      set_data_callback: set_upload_bill_doc_type_hierarchy_list,
      show_toast,
    });
  };

  const handle_upload_bill_doc_type_hierarchy = async () => {
    if (upload_bill_doc_type_hierarchy_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_bill_doc_type_hierarchy(
        upload_bill_doc_type_hierarchy_list
      );
      ...
      if (response.success) {
        show_toast({
          ...
          message: `${upload_bill_doc_type_hierarchy_list.length} data has been uploaded.`,
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-base font-bold mb-4">
              Upload Billing Document Type Hierarchy
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_bill_doc_type_hierarchy_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_bill_doc_type_hierarchy_list.map(
                        (row, idx) => {
...
export default Upload_Bill_Doc_Type_H;

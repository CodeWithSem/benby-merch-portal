...
import Create_Deliv_Doc_Type_H from "./functions/Create_Deliv_Doc_Type_H";
...
const Deliv_Doc_Type_Hierarchy = ({ set_page }) => {
  ...
  const [so_type_list, set_so_type_list] = useState([]);
  ...
  const [bill_doc_type_h_list, set_bill_doc_type_h_list] = useState([]);

  const handle_get_all_lists = async () => {
    try {
      const [
        so_type_response,
        deliv_doc_type_response,
        bill_doc_type_response,
        bill_cancel_response,
        bill_doc_type_h_response,
      ] = await Promise.all([
        api_get_so_type_list(),
        api_get_deliv_doc_type_list(),
        api_get_bill_doc_type_list(),
        api_get_bill_cancel_list(),
        api_get_bill_doc_type_hierarchy_list(),
      ]);
      if (so_type_response.success && bill_doc_type_h_response.success) {
        set_so_type_list(so_type_response.data);
        set_deliv_doc_type_list(deliv_doc_type_response.data);
        set_bill_doc_type_list(bill_doc_type_response.data);
        set_bill_cancel_list(bill_cancel_response.data);
        set_bill_doc_type_h_list(bill_doc_type_h_response.data);
        set_display_modal("");
      } else {
        console.error("One or more list fetches failed.", {
          so_type: so_type_response.message,
          deliv_doc_type: deliv_doc_type_response.message,
          bill_doc_type: bill_doc_type_response.message,
          bill_cancel: bill_cancel_response.message,
          bill_doc_type_h: bill_doc_type_h_response.message,
        });
        show_error();
        handle_go_back("main");
      }
  ...
  const def_deliv_doc_type_hierarchy_data = {
    id: null,
    so_type_code: "",
    deliv_doc_type_code: "",
    bill_doc_type_code: "",
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_DELIVERY_DOCUMENT_TYPE_HIERARCHY", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const [deliv_doc_type_hierarchy_list, set_deliv_doc_type_hierarchy_list] =
    useState([]);

  const handle_get_deliv_doc_type_hierarchy_list = async () => {
    set_loading_list(true);
    const response = await api_get_deliv_doc_type_hierarchy_list();
    if (response.success) {
      set_deliv_doc_type_hierarchy_list(response.data);
    } else {
  ...
  const handle_truncate_deliv_doc_type_hierarchy_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_deliv_doc_type_hierarchy();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [
    filtered_deliv_doc_type_hierarchy_list,
    set_filtered_deliv_doc_type_hierarchy_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...deliv_doc_type_hierarchy_list];
    ...
    set_filtered_deliv_doc_type_hierarchy_list(indexed_data);
  }, [
    deliv_doc_type_hierarchy_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? deliv_doc_type_hierarchy_list.filter((u) => {
          const q = debounced_query.toLowerCase();
      ...
      : deliv_doc_type_hierarchy_list.length) / select_option
  );
  ...
  const handle_create_new_deliv_doc_type_hierarchy = () => {
    set_sub_page("create_new_deliv_doc_type_hierarchy");
  };

  const handle_edit_deliv_doc_type_hierarchy = (data) => {
    set_edit_data(data);
    set_sub_page("edit_deliv_doc_type_hierarchy");
  };
  const handle_delete_deliv_doc_type_hierarchy = (data) => {
    set_delete_data(data);
    set_display_modal("delete_deliv_doc_type_hierarchy");
  };

  const handle_upload_deliv_doc_type_hierarchy = () => {
    set_sub_page("upload_deliv_doc_type_hierarchy");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_deliv_doc_type_hierarchy_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_deliv_doc_type_hierarchy}
                  >
                    Create New Data
                  </Button>
                  <Button
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_deliv_doc_type_hierarchy_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_deliv_doc_type_hierarchy_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_deliv_doc_type_hierarchy_list.map(
                            (row, idx) => {
                                    ...
                                    <div className="flex gap-2">
                                      <div className="relative group flex jusity-center items-center">
                                        <button
                                          ...
                                          onClick={() =>
                                            handle_edit_deliv_doc_type_hierarchy(
                                              row
                                            )
                                          }
                                        >
                                      ...
                                      <div className="relative group flex jusity-center items-center">
                                        <button
                                          ...
                                          onClick={() =>
                                            handle_delete_deliv_doc_type_hierarchy(
                                              row
                                            )
                                          }
                                        >
      ...
      {sub_page === "create_new_deliv_doc_type_hierarchy" && (
        <Create_Deliv_Doc_Type_H
          ...
          set_deliv_doc_type_hierarchy_list={set_deliv_doc_type_hierarchy_list}
      ...
      {sub_page === "edit_deliv_doc_type_hierarchy" && (
        <Edit_Deliv_Doc_Type_H
          handle_go_back={handle_go_back}
          active_user={active_user}
          reset_new_data={reset_new_data}
          show_toast={show_toast}
      ...
      {sub_page === "upload_deliv_doc_type_hierarchy" && (
        <Upload_Deliv_Doc_Type_H
          handle_go_back={handle_go_back}
          handle_get_deliv_doc_type_hierarchy_list={
            handle_get_deliv_doc_type_hierarchy_list
          }
      ...
      <Delete_Deliv_Doc_Type_H
        is_open={display_modal === "delete_deliv_doc_type_hierarchy"}
...
export default Deliv_Doc_Type_Hierarchy;

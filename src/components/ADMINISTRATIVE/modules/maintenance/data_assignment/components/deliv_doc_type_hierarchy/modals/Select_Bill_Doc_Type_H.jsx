...
const Select_Bill_Doc_Type_H = ({
  ...
  const [filtered_bill_doc_type_h_list, set_filtered_bill_doc_type_h_list] =
    useState([]);
  const [current_page, set_current_page] = useState(1);
  const [rows_per_page, set_rows_per_page] = useState(5);
  const [search_query, set_search_query] = useState("");
  const [selected_bill_doc_type_h, set_selected_bill_doc_type_h] =
    useState(null);
  ...
  useEffect(() => {
    // 1. Start with bill_doc_type_h_list
    let data = apply_lookups(bill_doc_type_h_list, lookup_columns);
  ...
  const handle_select_deliv_doc_type = () => {
    if (!selected_bill_doc_type_h) {
      alert("Please select a deliv_doc_type hierarchy before proceeding.");
      return;
    }
    set_data((prev) => ({
      ...prev,
      deliv_doc_type_code: selected_bill_doc_type_h.deliv_doc_type_code,
      bill_doc_type_code: selected_bill_doc_type_h.bill_doc_type_code,
      bill_cancel_code: selected_bill_doc_type_h.bill_cancel_code,
    }));
    on_close();
  };
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[97] px-4">
        ...
        >
          ...
          <div className="text-lg md:text-xl font-bold mb-5 px-7">
            Billing Document Type Hierarchy Selection
          </div>
          {/* - Modal Label */}
          {/* + Modal Body */}
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_bill_doc_type_h_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_bill_doc_type_h_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_bill_doc_type_h?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_bill_doc_type_h(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                on_change={() =>
                                  set_selected_bill_doc_type_h(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                ...
                disabled={!selected_bill_doc_type_h}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Bill_Doc_Type_H;

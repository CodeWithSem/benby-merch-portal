// Page Source 2: Select_Dist_Channel
// Title Source 2: Distribution Channel
...
const Select_Dist_Channel = ({
  ...
  const [selected_dist_channel, set_selected_dist_channel] = useState(null);

  useEffect(() => {
    let data = [...dist_channel_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.dist_channel_code.toLowerCase().includes(q) ||
          data.dist_channel_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_dist_channel_list(data.slice(start_idx, end_idx));
  }, [dist_channel_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    dist_channel_list.filter(
      (data) =>
  ...
  const handle_select_dist_channel = () => {
    if (!selected_dist_channel) {
      alert("Please select a dist_channel before proceeding.");
      return;
    }
    set_data((prev) => ({
      ...prev,
      dist_channel_code: selected_dist_channel.dist_channel_code,
    }));
    on_close();
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_dist_channel_list.length === 0 ? (
                      ...
                      filtered_dist_channel_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_dist_channel?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_dist_channel(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_dist_channel?.id === data.id}
                                on_change={() =>
                                  set_selected_dist_channel(data)
                                }
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_dist_channel}
                class_name="w-full md:w-[100px]"
                disabled={!selected_dist_channel}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Dist_Channel;

...
import { api_bulk_upload_sales_org_hierarchy } from "api/firestore_db/maintenance/data_assignment/tbl_sales_org_hierarchy_api";
...
const Upload_Sales_Org_H = ({
  ...
  const [upload_sales_org_hierarchy_list, set_upload_sales_org_hierarchy_list] =
    useState([]);

  // + Client-Side Filtering
  const [
    filtered_upload_sales_org_hierarchy_list,
    set_filtered_upload_sales_org_hierarchy_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_sales_org_hierarchy_list];
    ...
    set_filtered_upload_sales_org_hierarchy_list(indexed_data);
  }, [
    upload_sales_org_hierarchy_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_sales_org_hierarchy_list.filter((u) => {
          const q = debounced_query.toLowerCase();
      ...
      : upload_sales_org_hierarchy_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_2_hierarchy({
      e,
      code_field_1: "sales_org_code",
      code_field_2: "dist_channel_code",
      set_data_callback: set_upload_sales_org_hierarchy_list,
      show_toast,
    });
  };

  const handle_upload_sales_org_hierarchy = async () => {
    if (upload_sales_org_hierarchy_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_sales_org_hierarchy(
        upload_sales_org_hierarchy_list
      );
      ...
      if (response.success) {
        show_toast({
          ...
          message: `${upload_sales_org_hierarchy_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_sales_org_hierarchy_list([]);
        handle_get_sales_org_hierarchy_list();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-base font-bold mb-4">
              Upload Sales Organization Hierarchy
            </div>
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Sales Organization Hierarchy</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_sales_org_hierarchy_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_sales_org_hierarchy_list.map(
                        (row, idx) => {
...
export default Upload_Sales_Org_H;

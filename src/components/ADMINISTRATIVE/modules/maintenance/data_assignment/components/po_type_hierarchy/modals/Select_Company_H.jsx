...
const Select_Company_H = ({
  ...
  const [filtered_company_h_list, set_filtered_company_h_list] = useState([]);
  const [current_page, set_current_page] = useState(1);
  const [rows_per_page, set_rows_per_page] = useState(5);
  const [search_query, set_search_query] = useState("");
  const [selected_company_h, set_selected_company_h] = useState(null);
  ...
  useEffect(() => {
    // 1. Start with company_h_list
    let data = apply_lookups(company_h_list, lookup_columns);
  ...
  const handle_select_company = () => {
    if (!selected_company_h) {
      alert("Please select a company hierarchy before proceeding.");
      return;
    }
    set_data((prev) => ({
      ...prev,
      company_code: selected_company_h.company_code,
      purc_org_code: selected_company_h.purc_org_code,
      purc_group_code: selected_company_h.purc_group_code,
    }));
    on_close();
  };
        ...
        >
              ...
              <div className="max-w-full overflow-x-auto custom-scrollbar">
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_company_h_list.length === 0 ? (
                      <tr>
                        <td
                          colSpan={3}
                      ...
                      filtered_company_h_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_company_h?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_company_h(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_company_h?.id === data.id}
                                on_change={() => set_selected_company_h(data)}
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_company}
                class_name="w-full md:w-[100px]"
                disabled={!selected_company_h}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Company_H;

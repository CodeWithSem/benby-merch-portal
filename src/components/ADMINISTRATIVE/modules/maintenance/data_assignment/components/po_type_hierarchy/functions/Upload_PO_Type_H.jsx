...
import { api_bulk_upload_po_type_hierarchy } from "api/firestore_db/tbl_po_type_hierarchy_api";
...
const Upload_PO_Type_H = ({
  ...
  const [upload_po_type_hierarchy_list, set_upload_po_type_hierarchy_list] =
    useState([]);

  // + Client-Side Filtering
  const [
    filtered_upload_po_type_hierarchy_list,
    set_filtered_upload_po_type_hierarchy_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_po_type_hierarchy_list];
    ...
    set_filtered_upload_po_type_hierarchy_list(indexed_data);
  }, [
    upload_po_type_hierarchy_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_po_type_hierarchy_list.filter((u) => {
          const q = debounced_query.toLowerCase();
      ...
      : upload_po_type_hierarchy_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_4_hierarchy({
      e,
      code_field_1: "po_type_code",
      code_field_2: "company_code",
      code_field_3: "purc_org_code",
      code_field_4: "purc_group_code",
      set_data_callback: set_upload_po_type_hierarchy_list,
      show_toast,
    });
  };

  const handle_upload_po_type_hierarchy = async () => {
    if (upload_po_type_hierarchy_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_po_type_hierarchy(
        upload_po_type_hierarchy_list
      );
      ...
      if (response.success) {
        show_toast({
          ...
          message: `${upload_po_type_hierarchy_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_po_type_hierarchy_list([]);
        handle_get_po_type_hierarchy_list();
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Upload PO Type Hierarchy</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload PO Type Hierarchy</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_po_type_hierarchy_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_po_type_hierarchy_list.map(
                        (row, idx) => {
...
export default Upload_PO_Type_H;

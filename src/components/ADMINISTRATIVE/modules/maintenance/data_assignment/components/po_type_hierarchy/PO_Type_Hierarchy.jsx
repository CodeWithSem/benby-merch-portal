...
import Create_PO_Type_H from "./functions/Create_PO_Type_H";
...
const PO_Type_Hierarchy = ({ set_page }) => {
  ...
  const [po_type_list, set_po_type_list] = useState([]);
  const [company_list, set_company_list] = useState([]);
  const [purc_org_list, set_purc_org_list] = useState([]);
  const [purc_group_list, set_purc_group_list] = useState([]);
  const [company_h_list, set_company_h_list] = useState([]);
  ...
  const handle_get_all_lists = async () => {
    try {
      const [
        ...
        api_get_purc_group_list(),
        api_get_company_hierarchy_list(),
      ]);
      if (po_type_response.success && company_h_response.success) {
        set_po_type_list(po_type_response.data);
        set_company_list(company_response.data);
        set_purc_org_list(purc_org_response.data);
        set_purc_group_list(purc_group_response.data);
        set_company_h_list(company_h_response.data);
        set_display_modal("");
      } else {
        console.error("One or more list fetches failed.", {
          po_type: po_type_response.message,
          company: company_response.message,
          purc_org: purc_org_response.message,
          purc_group: purc_group_response.message,
          company_h: company_h_response.message,
        });
        show_error();
        handle_go_back("main");
      }
  ...
  const def_po_type_hierarchy_data = {
    id: null,
    po_type_code: "",
    company_code: "",
    purc_org_code: "",
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PO_TYPE_HIERARCHY", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const [po_type_hierarchy_list, set_po_type_hierarchy_list] = useState([]);

  const handle_get_po_type_hierarchy_list = async () => {
    set_loading_list(true);
    const response = await api_get_po_type_hierarchy_list();
    if (response.success) {
      set_po_type_hierarchy_list(response.data);
    } else {
  ...
  const handle_truncate_po_type_hierarchy_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_po_type_hierarchy();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_po_type_hierarchy_list, set_filtered_po_type_hierarchy_list] =
    useState([]);
  ...
  const lookup_columns = [
    { code_key: "po_type_code", list: po_type_list, desc_key: "po_type_desc" },
    {
      code_key: "company_code",
      list: company_list,
      desc_key: "company_desc",
    },
    {
      code_key: "purc_org_code",
      list: purc_org_list,
      desc_key: "purc_org_desc",
    },
    {
      code_key: "purc_group_code",
      list: purc_group_list,
      desc_key: "purc_group_desc",
    },
  ];
  ...
  useEffect(() => {
    let temp = [...po_type_hierarchy_list];
    ...
    set_filtered_po_type_hierarchy_list(indexed_data);
  }, [
    po_type_hierarchy_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? po_type_hierarchy_list.filter((u) => {
          const q = debounced_query.toLowerCase();
      ...
      : po_type_hierarchy_list.length) / select_option
  );
  ...
  const handle_create_new_po_type_hierarchy = () => {
    set_sub_page("create_new_po_type_hierarchy");
  };

  const handle_edit_po_type_hierarchy = (data) => {
    set_edit_data(data);
    set_sub_page("edit_po_type_hierarchy");
  };
  const handle_delete_po_type_hierarchy = (data) => {
    set_delete_data(data);
    set_display_modal("delete_po_type_hierarchy");
  };

  const handle_upload_po_type_hierarchy = () => {
    set_sub_page("upload_po_type_hierarchy");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">PO Type Hierarchy</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_po_type_hierarchy_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_po_type_hierarchy}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_po_type_hierarchy}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_po_type_hierarchy_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_po_type_hierarchy_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_po_type_hierarchy_list.map((row, idx) => {
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_po_type_hierarchy(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_po_type_hierarchy(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_po_type_hierarchy" && (
        <Create_PO_Type_H
          ...
          set_po_type_hierarchy_list={set_po_type_hierarchy_list}
      ...
      {sub_page === "edit_po_type_hierarchy" && (
        <Edit_PO_Type_H
          handle_go_back={handle_go_back}
          active_user={active_user}
          reset_new_data={reset_new_data}
      ...
      {sub_page === "upload_po_type_hierarchy" && (
        <Upload_PO_Type_H
          handle_go_back={handle_go_back}
          handle_get_po_type_hierarchy_list={handle_get_po_type_hierarchy_list}
          show_toast={show_toast}
          po_type_list={po_type_list}
          company_list={company_list}
          purc_org_list={purc_org_list}
      ...
      <Delete_PO_Type_H
        is_open={display_modal === "delete_po_type_hierarchy"}
...
export default PO_Type_Hierarchy;

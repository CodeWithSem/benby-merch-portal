...
import { api_bulk_upload_data_hierarchy } from "api/firestore_db/tbl_data_hierarchy_api";
...
const Upload_FUNCTION_NAME_H = ({
  ...
  const [upload_data_hierarchy_list, set_upload_data_hierarchy_list] = useState(
    []
  );

  // + Client-Side Filtering
  const [
    filtered_upload_data_hierarchy_list,
    set_filtered_upload_data_hierarchy_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_data_hierarchy_list];
    ...
    set_filtered_upload_data_hierarchy_list(indexed_data);
  }, [
    upload_data_hierarchy_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    ...
    (debounced_query
      ? upload_data_hierarchy_list.filter((u) => {
          const q = debounced_query.toLowerCase();
      ...
      : upload_data_hierarchy_list.length) / // <-- Use full list length if no query
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_3_hierarchy({
      e,
      code_field_1: "data_source_1_code",
      code_field_2: "data_source_2_code",
      code_field_3: "data_source_3_code",
      set_data_callback: set_upload_data_hierarchy_list,
      show_toast,
    });
  };

  const handle_upload_data_hierarchy = async () => {
    if (upload_data_hierarchy_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_data_hierarchy(
        upload_data_hierarchy_list
      );
      ...
      if (response.success) {
        show_toast({
          ...
          message: `${upload_data_hierarchy_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_data_hierarchy_list([]);
        handle_get_data_hierarchy_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload TITLE_NAME_HIERARCHY</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_data_hierarchy_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_data_hierarchy_list.map((row, idx) => {
...
export default Upload_FUNCTION_NAME_H;

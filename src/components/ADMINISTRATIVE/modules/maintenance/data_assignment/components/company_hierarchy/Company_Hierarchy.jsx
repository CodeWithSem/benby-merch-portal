...
const Company_Hierarchy = ({ set_page }) => {
  ...
  const handle_get_all_lists = async () => {
    try {
      const [company_response, org_response, group_response] =
        await Promise.all([
      ...
      if (
        company_response.success &&
        org_response.success &&
        group_response.success
      ) {
        set_company_list(company_response.data);
        set_purc_org_list(org_response.data);
        set_purc_group_list(group_response.data);
        const res_log = {
          company_log: company_response.data,
          purc_org_log: org_response.data,
          purc_group_log: group_response.data,
        };
        console.log(res_log);
        set_display_modal("");
      } else {
  ...
  const def_company_hierarchy_data = {
    id: null,
    company_code: "",
    purc_org_code: "",
    purc_group_code: "",
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_COMPANY_HIERARCHY", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const [company_hierarchy_list, set_company_hierarchy_list] = useState([]);

  const handle_get_company_hierarchy_list = async () => {
    set_loading_list(true);
    const response = await api_get_company_hierarchy_list();
    if (response.success) {
      set_company_hierarchy_list(response.data);
    } else {
  ...
  const handle_truncate_company_hierarchy_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_company_hierarchy();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const lookup_columns = [
    { code_key: "company_code", list: company_list, desc_key: "company_desc" },
    {
  ...
  useEffect(() => {
    let temp = [...company_hierarchy_list];
    ...
    set_filtered_company_hierarchy_list(indexed_data);
  }, [
    company_hierarchy_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const handle_create_new_company_hierarchy = () => {
    set_sub_page("create_new_company_hierarchy");
  };

  const handle_edit_company_hierarchy = (data) => {
    set_edit_data(data);
    set_sub_page("edit_company_hierarchy");
  };
  const handle_delete_company_hierarchy = (data) => {
    set_delete_data(data);
    set_display_modal("delete_company_hierarchy");
  };

  const handle_upload_company_hierarchy = () => {
    set_sub_page("upload_company_hierarchy");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Company Hierarchy</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_company_hierarchy_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_company_hierarchy}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_company_hierarchy}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_company_hierarchy_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_company_hierarchy_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_company_hierarchy_list.map((row, idx) => {
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_company_hierarchy(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_company_hierarchy(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_company_hierarchy" && (
        <Create_Company_H
      ...
      <Delete_Company_H
        is_open={display_modal === "delete_company_hierarchy"}

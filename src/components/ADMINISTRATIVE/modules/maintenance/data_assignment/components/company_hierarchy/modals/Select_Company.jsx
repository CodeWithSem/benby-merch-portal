...
const Select_Company = ({
  ...
  useEffect(() => {
    let data = [...company_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.company_code.toLowerCase().includes(q) ||
          data.company_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_company_list(data.slice(start_idx, end_idx));
  }, [company_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    company_list.filter(
      (data) =>
        data.company_code.toLowerCase().includes(search_query.toLowerCase()) ||
        data.company_desc.toLowerCase().includes(search_query.toLowerCase())
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_company_list.length === 0 ? (
                      ...
                      filtered_company_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_company?.id === data.id ? "bg-sky-50" : ""
                          }`}
                          onClick={() => set_selected_company(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_company?.id === data.id}
                                on_change={() => set_selected_company(data)}
                              />
                            </div>
                          </td>

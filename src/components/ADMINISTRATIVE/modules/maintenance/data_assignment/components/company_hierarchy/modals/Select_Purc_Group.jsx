...
const Select_Purc_Group = ({
  ...
  const [selected_purc_group, set_selected_purc_group] = useState(null);

  useEffect(() => {
    let data = [...purc_group_list];

    if (search_query.trim() !== "") {
      const q = search_query.toLowerCase();
      data = data.filter(
        (data) =>
          data.purc_group_code.toLowerCase().includes(q) ||
          data.purc_group_desc.toLowerCase().includes(q)
      );
    }

    const start_idx = (current_page - 1) * rows_per_page;
    const end_idx = start_idx + rows_per_page;
    set_filtered_purc_group_list(data.slice(start_idx, end_idx));
  }, [purc_group_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    purc_group_list.filter(
      (data) =>
        data.purc_group_code
          .toLowerCase()
          .includes(search_query.toLowerCase()) ||
        data.purc_group_desc.toLowerCase().includes(search_query.toLowerCase())
  ...
  const handle_select_purc_group = () => {
    if (!selected_purc_group) {
      alert("Please select a purc_group before proceeding.");
      return;
    }
    set_data((prev) => ({
      ...prev,
      purc_group_code: selected_purc_group.purc_group_code,
    }));
    on_close();
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_purc_group_list.length === 0 ? (
                      ...
                      filtered_purc_group_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_purc_group?.id === data.id
                              ? "bg-sky-50"
                              : ""
                          }`}
                          onClick={() => set_selected_purc_group(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_purc_group?.id === data.id}
                                on_change={() => set_selected_purc_group(data)}
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_purc_group}
                class_name="w-full md:w-[100px]"
                disabled={!selected_purc_group}
              >
                Proceed
              </Button>
              <Button
...
export default Select_Purc_Group;

...
const Select_SO_Type = ({
  ...
  useEffect(() => {
    ...
    if (search_query.trim() !== "") {
      ...
      data = data.filter(
        (data) =>
          data.so_type_code.toLowerCase().includes(q) ||
          data.so_type_desc.toLowerCase().includes(q)
      );
    }
  ...
  }, [so_type_list, search_query, current_page, rows_per_page]);

  const total_pages = Math.ceil(
    so_type_list.filter(
      (data) =>
        data.so_type_code.toLowerCase().includes(search_query.toLowerCase()) ||
        data.so_type_desc.toLowerCase().includes(search_query.toLowerCase())
  ...
  const handle_select_so_type = () => {
    if (!selected_so_type) {
      alert("Please select a so_type before proceeding.");
      return;
    }
    set_data((prev) => ({
      ...prev,
      so_type_code: selected_so_type.so_type_code,
    }));
    on_close();
  };
        ...
        >
                ...
                <table className="min-w-full whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered_so_type_list.length === 0 ? (
                      ...
                      filtered_so_type_list.map((data) => (
                        <tr
                          key={data.id}
                          className={`hover:bg-sky-50/50 cursor-pointer text-[12px] ${
                            selected_so_type?.id === data.id ? "bg-sky-50" : ""
                          }`}
                          onClick={() => set_selected_so_type(data)}
                        >
                          <td className="px-5 py-4 sm:px-6 text-center">
                            <div className="flex justify-center items-center">
                              <Checkbox_Field
                                ...
                                box_size={18}
                                icon_size={12}
                                checked={selected_so_type?.id === data.id}
                                on_change={() => set_selected_so_type(data)}
                              />
                            </div>
                          </td>
          ...
          <div className="flex flex-col items-center sm:flex-row sm:justify-between gap-3 mt-5 px-7">
            ...
            <div className="flex justify-center sm:justify-end gap-2 w-full">
              <Button
                variant="primary"
                on_click={handle_select_so_type}

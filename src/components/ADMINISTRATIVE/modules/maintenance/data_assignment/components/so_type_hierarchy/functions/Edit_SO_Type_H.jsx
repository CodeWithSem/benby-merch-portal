...
import { api_update_so_type_hierarchy } from "api/firestore_db/maintenance/data_assignment/tbl_so_type_hierarchy_api";
...
import Select_Sales_Org_H from "../modals/Select_Sales_Org_H";

const Edit_SO_Type_H = ({
  handle_go_back,
  active_user,
  show_toast,
  edit_data,
  set_edit_data,
  set_so_type_hierarchy_list,
  so_type_list,
  sales_org_list,
  dist_channel_list,
  sales_org_h_list,
  ...
  const validate_edit_data = () => {
    ...
    if (!edit_data.sales_org_code.trim()) {
  ...
  const handle_update_so_type_hierarchy = async () => {
    ...
    try {
      ...
      const response = await api_update_so_type_hierarchy(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_so_type_hierarchy_list((prev) =>
          prev.map((item) =>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm SO Type Hierarchy Update
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to edit this SO Type Hierarchy. Once edited, it will
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_update_so_type_hierarchy}
              >
                Yes
              </Button>
              <Button
  ...
  return (
    <React.Fragment>
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Edit SO Type Hierarchy</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-5">
              <div>
                <Text_Code_Field
                  ...
                  code_value={edit_data.sales_org_code}
                  text_value={get_description(
                    edit_data.sales_org_code,
                    sales_org_list,
                    "sales_org_code",
                    "sales_org_desc"
                  )}
                  on_click={() => set_display_sub_modal("select_sales_org_h")}
                  disabled
                />
              </div>
              <div>
                <Text_Code_Field
                  ...
                  code_value={edit_data.dist_channel_code}
                  text_value={get_description(
                    edit_data.dist_channel_code,
                    dist_channel_list,
                    "dist_channel_code",
                    "dist_channel_desc"
                  )}
      ...
      />
      <Select_Sales_Org_H
        is_open={display_sub_modal === "select_sales_org_h"}
        on_close={() => set_display_sub_modal("")}
        width="max-w-[1200px]"
        height="max-h-[700px]"

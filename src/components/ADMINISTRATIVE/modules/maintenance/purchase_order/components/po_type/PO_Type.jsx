...
import Create_PO_Type from "./functions/Create_PO_Type";
import Edit_PO_Type from "./functions/Edit_PO_Type";
import Delete_PO_Type from "./functions/Delete_PO_Type";
import Upload_PO_Type from "./functions/Upload_PO_Type";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const PO_Type = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_po_type_data });
  const [edit_data, set_edit_data] = useState({ ...def_po_type_data });
  const [delete_data, set_delete_data] = useState({ ...def_po_type_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PURCHASE_ORDER_TYPE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "po_type_code", label: "Purchase Order Type Code", sortable: true },
    {
      key: "po_type_desc",
      label: "Purchase Order Type Description",
      sortable: true,
    },
  ...
  // ======================================================
  // id: 1,
  // po_type_code: "Purchase Order Type-001",
  // po_type_desc: "Purchase Order Type Description 1",
  // created_by: "Admin",
  ...
  const handle_get_po_type_list = async () => {
    set_loading_list(true);
    const response = await api_get_po_type_list();
    if (response.success) {
      set_po_type_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_po_type_list();
  }, []);

  const handle_truncate_po_type_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_po_type();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  useEffect(() => {
    ...
    set_filtered_po_type_list(indexed_data);
  }, [
    po_type_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : po_type_list.length) / select_option
  );
  ...
  const handle_create_new_po_type = () => {
    set_sub_page("create_new_po_type");
  };

  const handle_edit_po_type = (data) => {
    set_edit_data(data);
    set_sub_page("edit_po_type");
  };
  const handle_delete_po_type = (data) => {
    set_delete_data(data);
    set_display_modal("delete_po_type");
  };

  const handle_upload_po_type = () => {
    set_sub_page("upload_po_type");
  };
    ...
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Purchase Order Type</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Purchase Order Type</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_po_type_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_po_type}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_po_type}
                  >
                    Upload
                  </Button>
                </div>
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_po_type_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_po_type_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_edit_po_type(row)}
                                      >
                                        <Edit size={19} />
                                      </button>
      ...
      {sub_page === "create_new_po_type" && (
        <Create_PO_Type
          ...
          set_po_type_list={set_po_type_list}
        />
      )}
      {sub_page === "edit_po_type" && (
        <Edit_PO_Type
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_po_type" && (
        <Upload_PO_Type
          handle_go_back={handle_go_back}
          handle_get_po_type_list={handle_get_po_type_list}
          show_toast={show_toast}
        />
      )}
      <Delete_PO_Type
        is_open={display_modal === "delete_po_type"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_po_type_increment}
      />
    </React.Fragment>
  );
};

export default PO_Type;

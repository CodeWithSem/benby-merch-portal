...
import { api_create_po_type } from "api/firestore_db/maintenance/purchase_order/tbl_po_type_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Create_PO_Type = ({
  ...
  const handle_change_po_type_code = (value) => {
    set_new_data((prev) => ({
      ...prev,
      po_type_code: value,
    }));
  ...
  const handle_change_po_type_desc = (value) => {
    set_new_data((prev) => ({
      ...prev,
      po_type_desc: value,
    }));
  ...
  const handle_create_po_type = async () => {
    ...
    try {
      ...
      const response = await api_create_po_type(
        new_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_po_type_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm Purchase Order Type Creation
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new Purchase Order Type. Once created,
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_create_po_type}
              >
                Yes
              </Button>
              <Button
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Purchase Order Type Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  value={new_data.po_type_code || ""}
                  on_change={(e) => handle_change_po_type_code(e.target.value)}
                />
              </div>
              <div className="col-span-2">
                <Text_Field
                  ...
                  value={new_data.po_type_desc || ""}
                  on_change={(e) => handle_change_po_type_desc(e.target.value)}
                />
              </div>
            </div>
          </div>

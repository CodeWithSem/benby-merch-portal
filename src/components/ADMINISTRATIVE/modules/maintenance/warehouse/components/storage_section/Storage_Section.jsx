...
import Create_SSEC from "./functions/Create_SSEC";
...
const Storage_Section = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_STORAGE_SECTION", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const columns = [
    ...
    { key: "ssec_code", label: "Storage Section Code", sortable: true },
    { key: "ssec_desc", label: "Storage Section Description", sortable: true },
  ...
  const [ssec_list, set_ssec_list] = useState([]);

  const handle_get_ssec_list = async () => {
    set_loading_list(true);
    const response = await api_get_ssec_list();
    if (response.success) {
      set_ssec_list(response.data);
    } else {
  ...
  const handle_truncate_ssec_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_ssec();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_ssec_list, set_filtered_ssec_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_ssec_list(indexed_data);
  }, [
    ssec_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : ssec_list.length) / select_option
  );
  ...
  const handle_create_new_ssec = () => {
    set_sub_page("create_new_ssec");
  };

  const handle_edit_ssec = (data) => {
    set_edit_data(data);
    set_sub_page("edit_ssec");
  };
  const handle_delete_ssec = (data) => {
    set_delete_data(data);
    set_display_modal("delete_ssec");
  };

  const handle_upload_ssec = () => {
    set_sub_page("upload_ssec");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Storage Section</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_ssec_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_ssec}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_ssec_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_delete_ssec(row)}
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "upload_ssec" && (
        <Upload_SSEC
          handle_go_back={handle_go_back}
          handle_get_ssec_list={handle_get_ssec_list}
          show_toast={show_toast}
        />
      )}
      <Delete_SSEC
        is_open={display_modal === "delete_ssec"}
...
export default Storage_Section;

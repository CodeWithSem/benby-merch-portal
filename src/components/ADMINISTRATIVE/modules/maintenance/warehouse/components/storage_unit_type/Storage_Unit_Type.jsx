// File Name: Storage_Unit_Type
// Function Name: SUTYPE
// Data Name: sutype
// Column Name: Storage Unit Type
// Title Name: Storage Unit Type
// Sub-module Name: Warehouse
// Table Name: TBL_STORAGE_UNIT_TYPE
// Default Code: SUTYPE
...
import {
  api_get_sutype_list,
  api_set_sutype_increment,
  api_truncate_sutype,
} from "api/firestore_db/maintenance/warehouse/tbl_sutype_api";
...
import Create_SUTYPE from "./functions/Create_SUTYPE";
import Edit_SUTYPE from "./functions/Edit_SUTYPE";
import Delete_SUTYPE from "./functions/Delete_SUTYPE";
import Upload_SUTYPE from "./functions/Upload_SUTYPE";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Storage_Unit_Type = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_sutype_data });
  const [edit_data, set_edit_data] = useState({ ...def_sutype_data });
  const [delete_data, set_delete_data] = useState({ ...def_sutype_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_STORAGE_UNIT_TYPE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "sutype_code", label: "Storage Unit Type Code", sortable: true },
    {
      key: "sutype_desc",
      label: "Storage Unit Type Description",
      sortable: true,
    },
  ...
  const [sutype_list, set_sutype_list] = useState([]);

  const handle_get_sutype_list = async () => {
    set_loading_list(true);
    const response = await api_get_sutype_list();
    if (response.success) {
      set_sutype_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_sutype_list();
  }, []);

  const handle_truncate_sutype_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_sutype();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_sutype_list, set_filtered_sutype_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_sutype_list(indexed_data);
  }, [
    sutype_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : sutype_list.length) / select_option
  );
  ...
  const handle_create_new_sutype = () => {
    set_sub_page("create_new_sutype");
  };

  const handle_edit_sutype = (data) => {
    set_edit_data(data);
    set_sub_page("edit_sutype");
  };
  const handle_delete_sutype = (data) => {
    set_delete_data(data);
    set_display_modal("delete_sutype");
  };

  const handle_upload_sutype = () => {
    set_sub_page("upload_sutype");
  };
    ...
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Storage Unit Type</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Storage Unit Type</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_sutype_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_sutype}
                  >
                    Create New Data
                  </Button>
                  <Button
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_sutype_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_sutype_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_edit_sutype(row)}
                                      >
                                        <Edit size={19} />
                                      </button>
      ...
      {sub_page === "create_new_sutype" && (
        <Create_SUTYPE
          ...
          set_sutype_list={set_sutype_list}
        />
      )}
      {sub_page === "edit_sutype" && (
        <Edit_SUTYPE
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_sutype" && (
        <Upload_SUTYPE
          handle_go_back={handle_go_back}
          handle_get_sutype_list={handle_get_sutype_list}
          show_toast={show_toast}
        />
      )}
      <Delete_SUTYPE
        is_open={display_modal === "delete_sutype"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_sutype_increment}
      />
    </React.Fragment>
  );
};

export default Storage_Unit_Type;

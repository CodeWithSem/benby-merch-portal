...
import { api_bulk_upload_sutype } from "api/firestore_db/maintenance/warehouse/tbl_sutype_api";
...
const Upload_SUTYPE = ({
  ...
  const [upload_sutype_list, set_upload_sutype_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_sutype_list, set_filtered_upload_sutype_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_upload_sutype_list(indexed_data);
  }, [
    upload_sutype_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_sutype_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "sutype_code",
      desc_field: "sutype_desc",
      set_data_callback: set_upload_sutype_list,
      show_toast,
    });
  };

  const handle_upload_sutype = async () => {
    if (upload_sutype_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_sutype(upload_sutype_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_sutype_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_sutype_list([]);
        handle_get_sutype_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Storage Unit Type</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_sutype_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_sutype_list.map((row, idx) => {

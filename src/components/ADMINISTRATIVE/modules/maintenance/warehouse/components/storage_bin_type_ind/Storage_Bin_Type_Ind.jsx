// File Name: Storage_Bin_Type_Ind
...
// Table Name: TBL_STORAGE_BIN_TYPE_INDICATOR
// Default Code: SBTYPE-I

import React, { useEffect, useState } from "react";
import { useToast } from "../../../../../layout/Toast_Provider";
import { Use_App } from "context/app_context";
import {
  api_get_sbtype_ind_list,
  api_set_sbtype_ind_increment,
  api_truncate_sbtype_ind,
} from "api/firestore_db/maintenance/warehouse/tbl_sbtype_ind_api";
...
import Create_SBTYPE_Ind from "./functions/Create_SBTYPE_Ind";
import Edit_SBTYPE_Ind from "./functions/Edit_SBTYPE_Ind";
import Delete_SBTYPE_Ind from "./functions/Delete_SBTYPE_Ind";
import Upload_SBTYPE_Ind from "./functions/Upload_SBTYPE_Ind";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Storage_Bin_Type_Ind = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_sbtype_ind_data });
  const [edit_data, set_edit_data] = useState({ ...def_sbtype_ind_data });
  const [delete_data, set_delete_data] = useState({ ...def_sbtype_ind_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_STORAGE_BIN_TYPE_INDICATOR", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        sbtype_ind_code: `SBTYPE-I-${String(value).padStart(2, "0")}`,
      }));
  ...
  const [sbtype_ind_list, set_sbtype_ind_list] = useState([]);

  const handle_get_sbtype_ind_list = async () => {
    set_loading_list(true);
    const response = await api_get_sbtype_ind_list();
    if (response.success) {
      set_sbtype_ind_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_sbtype_ind_list();
  }, []);

  const handle_truncate_sbtype_ind_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_sbtype_ind();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_sbtype_ind_list, set_filtered_sbtype_ind_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_sbtype_ind_list(indexed_data);
  }, [
    sbtype_ind_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : sbtype_ind_list.length) / select_option
  );
  ...
  const handle_create_new_sbtype_ind = () => {
    set_sub_page("create_new_sbtype_ind");
  };

  const handle_edit_sbtype_ind = (data) => {
    set_edit_data(data);
    set_sub_page("edit_sbtype_ind");
  };
  const handle_delete_sbtype_ind = (data) => {
    set_delete_data(data);
    set_display_modal("delete_sbtype_ind");
  };

  const handle_upload_sbtype_ind = () => {
    set_sub_page("upload_sbtype_ind");
  };
  ...
  return (
    <React.Fragment>
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Storage Bin Type Indicator</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_sbtype_ind_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_sbtype_ind}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_sbtype_ind}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_sbtype_ind_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_sbtype_ind_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_sbtype_ind_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_sbtype_ind" && (
        <Create_SBTYPE_Ind
          ...
          set_sbtype_ind_list={set_sbtype_ind_list}
        />
      )}
      {sub_page === "edit_sbtype_ind" && (
        <Edit_SBTYPE_Ind
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_sbtype_ind" && (
        <Upload_SBTYPE_Ind
          handle_go_back={handle_go_back}
          handle_get_sbtype_ind_list={handle_get_sbtype_ind_list}
          show_toast={show_toast}
        />
      )}
      <Delete_SBTYPE_Ind
        is_open={display_modal === "delete_sbtype_ind"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_sbtype_ind_increment}
      />
    </React.Fragment>
  );
};

export default Storage_Bin_Type_Ind;

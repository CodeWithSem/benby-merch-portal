...
import { api_bulk_upload_pick_area } from "api/firestore_db/maintenance/warehouse/tbl_pick_area_api";
...
const Upload_Picking_Area = ({
  ...
  const [upload_pick_area_list, set_upload_pick_area_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_pick_area_list, set_filtered_upload_pick_area_list] =
    useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_pick_area_list];
    ...
    set_filtered_upload_pick_area_list(indexed_data);
  }, [
    upload_pick_area_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_pick_area_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_pick_area_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "pick_area_code",
      desc_field: "pick_area_desc",
      set_data_callback: set_upload_pick_area_list,
      show_toast,
    });
  };

  const handle_upload_pick_area = async () => {
    if (upload_pick_area_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_pick_area(upload_pick_area_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_pick_area_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_pick_area_list([]);
        handle_get_pick_area_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Picking Area</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_pick_area_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_pick_area_list.map((row, idx) => {
...
export default Upload_Picking_Area;

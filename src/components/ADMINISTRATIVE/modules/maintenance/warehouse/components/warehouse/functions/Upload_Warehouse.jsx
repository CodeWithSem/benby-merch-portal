...
import { api_bulk_upload_warehouse } from "api/firestore_db/maintenance/warehouse/tbl_warehouse_api";
...
const Upload_Warehouse = ({
  ...
  const [upload_warehouse_list, set_upload_warehouse_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_warehouse_list, set_filtered_upload_warehouse_list] =
    useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_warehouse_list];
    ...
    set_filtered_upload_warehouse_list(indexed_data);
  }, [
    upload_warehouse_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_warehouse_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_warehouse_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      ...
      desc_field: "warehouse_desc",
      set_data_callback: set_upload_warehouse_list,
      show_toast,
    });
  };

  const handle_upload_warehouse = async () => {
    if (upload_warehouse_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_warehouse(upload_warehouse_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_warehouse_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_warehouse_list([]);
        handle_get_warehouse_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Warehouse</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_warehouse_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_warehouse_list.map((row, idx) => {
...
export default Upload_Warehouse;

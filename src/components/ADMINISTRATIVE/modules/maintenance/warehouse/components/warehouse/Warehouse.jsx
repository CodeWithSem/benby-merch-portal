...
import {
  ...
  api_set_warehouse_increment,
  api_truncate_warehouse,
} from "api/firestore_db/maintenance/warehouse/tbl_warehouse_api";
...
import Create_Warehouse from "./functions/Create_Warehouse";
import Edit_Warehouse from "./functions/Edit_Warehouse";
import Delete_Warehouse from "./functions/Delete_Warehouse";
import Upload_Warehouse from "./functions/Upload_Warehouse";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Warehouse = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_warehouse_data });
  const [edit_data, set_edit_data] = useState({ ...def_warehouse_data });
  const [delete_data, set_delete_data] = useState({ ...def_warehouse_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_WAREHOUSE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        warehouse_code: `WH-${String(value).padStart(3, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "warehouse_desc", label: "Warehouse Description", sortable: true },
  ...
  const handle_get_warehouse_list = async () => {
    set_loading_list(true);
    const response = await api_get_warehouse_list();
    if (response.success) {
      set_warehouse_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_warehouse_list();
  }, []);

  const handle_truncate_warehouse_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_warehouse();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  useEffect(() => {
    ...
    set_filtered_warehouse_list(indexed_data);
  }, [
    warehouse_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : warehouse_list.length) / select_option
  );
  ...
  const handle_create_new_warehouse = () => {
    set_sub_page("create_new_warehouse");
  };

  const handle_edit_warehouse = (data) => {
    set_edit_data(data);
    set_sub_page("edit_warehouse");
  };
  const handle_delete_warehouse = (data) => {
    set_delete_data(data);
    set_display_modal("delete_warehouse");
  };

  const handle_upload_warehouse = () => {
    set_sub_page("upload_warehouse");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_warehouse_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_warehouse}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_warehouse}
                  >
                    Upload
                  </Button>
                </div>
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_warehouse_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_warehouse_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "upload_warehouse" && (
        <Upload_Warehouse
          handle_go_back={handle_go_back}
          handle_get_warehouse_list={handle_get_warehouse_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Warehouse
        is_open={display_modal === "delete_warehouse"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_warehouse_increment}
      />
    </React.Fragment>
  );
};

...
import { api_bulk_upload_ssec_ind } from "api/firestore_db/maintenance/warehouse/tbl_ssec_ind_api";
...
const Upload_SSEC_Ind = ({
  ...
  const [upload_ssec_ind_list, set_upload_ssec_ind_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_ssec_ind_list, set_filtered_upload_ssec_ind_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_upload_ssec_ind_list(indexed_data);
  }, [
    upload_ssec_ind_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_ssec_ind_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "ssec_ind_code",
      desc_field: "ssec_ind_desc",
      set_data_callback: set_upload_ssec_ind_list,
      show_toast,
    });
  };

  const handle_upload_ssec_ind = async () => {
    if (upload_ssec_ind_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_ssec_ind(upload_ssec_ind_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_ssec_ind_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_ssec_ind_list([]);
        handle_get_ssec_ind_list();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Upload Storage Section Indicator
            </div>
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Storage Section Indicator</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_ssec_ind_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_ssec_ind_list.map((row, idx) => {
...
export default Upload_SSEC_Ind;

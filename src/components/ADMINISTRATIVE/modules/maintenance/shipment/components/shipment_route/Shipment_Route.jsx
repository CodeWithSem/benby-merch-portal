...
import Create_Shipment_Route from "./functions/Create_Shipment_Route";
...
const Shipment_Route = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_SHIPMENT_ROUTE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const [shipment_route_list, set_shipment_route_list] = useState([]);

  const handle_get_shipment_route_list = async () => {
    set_loading_list(true);
    const response = await api_get_shipment_route_list();
    if (response.success) {
      set_shipment_route_list(response.data);
    } else {
  ...
  const handle_truncate_shipment_route_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_shipment_route();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_shipment_route_list, set_filtered_shipment_route_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_shipment_route_list(indexed_data);
  }, [
    shipment_route_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : shipment_route_list.length) / select_option
  );
  ...
  const handle_create_new_shipment_route = () => {
    set_sub_page("create_new_shipment_route");
  };

  const handle_edit_shipment_route = (data) => {
    set_edit_data(data);
    set_sub_page("edit_shipment_route");
  };
  const handle_delete_shipment_route = (data) => {
    set_delete_data(data);
    set_display_modal("delete_shipment_route");
  };

  const handle_upload_shipment_route = () => {
    set_sub_page("upload_shipment_route");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Shipment Route</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_shipment_route_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_shipment_route}
                  >
                    Create New Data
                  </Button>
                  <Button
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_shipment_route_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_shipment_route_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_shipment_route_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_shipment_route(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_shipment_route(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_shipment_route" && (
      ...
      {sub_page === "upload_shipment_route" && (
        <Upload_Shipment_Route
          handle_go_back={handle_go_back}
          handle_get_shipment_route_list={handle_get_shipment_route_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Shipment_Route
        is_open={display_modal === "delete_shipment_route"}

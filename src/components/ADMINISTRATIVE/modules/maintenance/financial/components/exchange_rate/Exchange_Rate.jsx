...
import Create_E_Rate from "./functions/Create_E_Rate";
...
const Exchange_Rate = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_EXCHANGE_RATE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const columns = [
    ...
    { key: "exchange_rate_code", label: "Exchange Rate Code", sortable: true },
    {
      key: "exchange_rate_desc",
      label: "Exchange Rate Description",
      sortable: true,
    },
  ...
  const handle_get_exchange_rate_list = async () => {
    ...
    const response = await api_get_exchange_rate_list();
    if (response.success) {
      set_exchange_rate_list(response.data);
    } else {
  ...
  const handle_truncate_exchange_rate_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_exchange_rate();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_exchange_rate_list, set_filtered_exchange_rate_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_exchange_rate_list(indexed_data);
  }, [
    exchange_rate_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : exchange_rate_list.length) / select_option
  );
  ...
  const handle_create_new_exchange_rate = () => {
    set_sub_page("create_new_exchange_rate");
  };

  const handle_edit_exchange_rate = (data) => {
    set_edit_data(data);
    set_sub_page("edit_exchange_rate");
  };
  const handle_delete_exchange_rate = (data) => {
    set_delete_data(data);
    set_display_modal("delete_exchange_rate");
  };

  const handle_upload_exchange_rate = () => {
    set_sub_page("upload_exchange_rate");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_exchange_rate_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_exchange_rate}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_exchange_rate}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_exchange_rate_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_exchange_rate_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_exchange_rate_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_exchange_rate(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_exchange_rate" && (
        <Create_E_Rate
      ...
      {sub_page === "upload_exchange_rate" && (
        <Upload_E_Rate
          handle_go_back={handle_go_back}
          handle_get_exchange_rate_list={handle_get_exchange_rate_list}
          show_toast={show_toast}
        />
      )}
      <Delete_E_Rate
        is_open={display_modal === "delete_exchange_rate"}

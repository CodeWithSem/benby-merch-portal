...
const Edit_P_Term = ({
  ...
  const validate_edit_data = () => {
    if (!edit_data.payment_method_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!edit_data.payment_method_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_update_payment_method = async () => {
    ...
    try {
      ...
      const response = await api_update_payment_method(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_payment_method_list((prev) =>
          prev.map((item) =>
      ...
      console.error("Failed to create payment_method:", error);
      show_status("error");
    } finally {
      close_confirm_modal();
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to edit this Payment Method. Once edited, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_update_payment_method}
              >
                Yes
              </Button>
              <Button

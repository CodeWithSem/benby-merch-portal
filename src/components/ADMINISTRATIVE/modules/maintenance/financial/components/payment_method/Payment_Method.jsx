...
} from "api/firestore_db/tbl_payment_method_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_P_Method from "./functions/Create_P_Method";
...
const Payment_Method = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PAYMENT_METHOD", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const handle_get_payment_method_list = async () => {
    ...
    const response = await api_get_payment_method_list();
    if (response.success) {
      set_payment_method_list(response.data);
    } else {
  ...
  const handle_truncate_payment_method_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_payment_method();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_payment_method_list, set_filtered_payment_method_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_payment_method_list(indexed_data);
  }, [
    payment_method_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : payment_method_list.length) / select_option
  );
  ...
  const handle_create_new_payment_method = () => {
    set_sub_page("create_new_payment_method");
  };

  const handle_edit_payment_method = (data) => {
    set_edit_data(data);
    set_sub_page("edit_payment_method");
  };
  const handle_delete_payment_method = (data) => {
    set_delete_data(data);
    set_display_modal("delete_payment_method");
  };

  const handle_upload_payment_method = () => {
    set_sub_page("upload_payment_method");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_payment_method_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_payment_method}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_payment_method}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_payment_method_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_payment_method_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_payment_method_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_payment_method(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_payment_method(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_payment_method" && (
        <Create_P_Method
      ...
      {sub_page === "upload_payment_method" && (
        <Upload_P_Method
          handle_go_back={handle_go_back}
          handle_get_payment_method_list={handle_get_payment_method_list}
          show_toast={show_toast}
        />
      )}
      <Delete_P_Method
        is_open={display_modal === "delete_payment_method"}

...
const Edit_IAC = ({
  ...
  const handle_change_inv_acc_center_desc = (value) => {
    set_edit_data({
      ...edit_data,
      inv_acc_center_desc: value,
    });
  };

  const validate_edit_data = () => {
    if (!edit_data.inv_acc_center_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!edit_data.inv_acc_center_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_update_inv_acc_center = async () => {
    ...
    try {
      ...
      const response = await api_update_inv_acc_center(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_inv_acc_center_list((prev) =>
          prev.map((item) =>
      ...
      console.error("Failed to create inv_acc_center:", error);
      show_status("error");
    } finally {
      close_confirm_modal();
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm Inv. Account Center Update
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to edit this Inv. Account Center. Once edited, it
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_update_inv_acc_center}
              >
                Yes
              </Button>
              <Button
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Edit Inv. Account Center</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  type={"text"}
                  value={edit_data.inv_acc_center_code || "-"}
                  disabled
                />
              </div>
              <div className="col-span-2">
                <Text_Field
                  ...
                  value={edit_data.inv_acc_center_desc}
                  on_change={(e) =>
                    handle_change_inv_acc_center_desc(e.target.value)
                  }
                />
              </div>
            </div>

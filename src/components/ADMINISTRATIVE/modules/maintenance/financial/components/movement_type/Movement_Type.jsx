...
} from "api/firestore_db/tbl_movement_type_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_Move_Type from "./functions/Create_Move_Type";
...
const Movement_Type = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_MOVEMENT_TYPE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const columns = [
    ...
    { key: "movement_type_code", label: "Movement Type Code", sortable: true },
    {
      key: "movement_type_desc",
  ...
  const handle_get_movement_type_list = async () => {
    ...
    const response = await api_get_movement_type_list();
    if (response.success) {
      set_movement_type_list(response.data);
    } else {
  ...
  const handle_truncate_movement_type_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_movement_type();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_movement_type_list, set_filtered_movement_type_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_movement_type_list(indexed_data);
  }, [
    movement_type_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : movement_type_list.length) / select_option
  );
  ...
  const handle_create_new_movement_type = () => {
    set_sub_page("create_new_movement_type");
  };

  const handle_edit_movement_type = (data) => {
    set_edit_data(data);
    set_sub_page("edit_movement_type");
  };
  const handle_delete_movement_type = (data) => {
    set_delete_data(data);
    set_display_modal("delete_movement_type");
  };

  const handle_upload_movement_type = () => {
    set_sub_page("upload_movement_type");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Movement Type</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_movement_type_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_movement_type}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_movement_type}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_movement_type_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_movement_type_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_movement_type_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_movement_type(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_movement_type" && (
        <Create_Move_Type
      ...
      {sub_page === "upload_movement_type" && (
        <Upload_Move_Type
          handle_go_back={handle_go_back}
          handle_get_movement_type_list={handle_get_movement_type_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Move_Type
        is_open={display_modal === "delete_movement_type"}

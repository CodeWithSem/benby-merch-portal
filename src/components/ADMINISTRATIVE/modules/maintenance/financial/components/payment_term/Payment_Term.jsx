...
import Create_P_Term from "./functions/Create_P_Term";
...
const Payment_Term = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PAYMENT_TERM", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const columns = [
    ...
    { key: "payment_term_code", label: "Payment Term Code", sortable: true },
    {
  ...
  const handle_get_payment_term_list = async () => {
    ...
    const response = await api_get_payment_term_list();
    if (response.success) {
      set_payment_term_list(response.data);
    } else {
  ...
  const handle_truncate_payment_term_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_payment_term();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_payment_term_list, set_filtered_payment_term_list] = useState(
    []
  );
  ...
  useEffect(() => {
    ...
    set_filtered_payment_term_list(indexed_data);
  }, [
    payment_term_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : payment_term_list.length) / select_option
  );
  ...
  const handle_create_new_payment_term = () => {
    set_sub_page("create_new_payment_term");
  };

  const handle_edit_payment_term = (data) => {
    set_edit_data(data);
    set_sub_page("edit_payment_term");
  };
  const handle_delete_payment_term = (data) => {
    set_delete_data(data);
    set_display_modal("delete_payment_term");
  };

  const handle_upload_payment_term = () => {
    set_sub_page("upload_payment_term");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Payment Term</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_payment_term_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_payment_term}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_payment_term}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_payment_term_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_payment_term_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_payment_term_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_payment_term(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_payment_term" && (
        <Create_P_Term
      ...
      {sub_page === "upload_payment_term" && (
        <Upload_P_Term
          handle_go_back={handle_go_back}
          handle_get_payment_term_list={handle_get_payment_term_list}
          show_toast={show_toast}
        />
      )}
      <Delete_P_Term
        is_open={display_modal === "delete_payment_term"}

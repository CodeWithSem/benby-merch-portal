...
import Create_Taxation from "./functions/Create_Taxation";
import Edit_Taxation from "./functions/Edit_Taxation";
import Delete_Taxation from "./functions/Delete_Taxation";
import Upload_Taxation from "./functions/Upload_Taxation";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Taxation = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_taxation_data });
  const [edit_data, set_edit_data] = useState({ ...def_taxation_data });
  const [delete_data, set_delete_data] = useState({ ...def_taxation_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_TAXATION", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "taxation_code", label: "Taxation Code", sortable: true },
    {
      key: "taxation_desc",
  ...
  const [taxation_list, set_taxation_list] = useState([]);

  const handle_get_taxation_list = async () => {
    set_loading_list(true);
    const response = await api_get_taxation_list();
    if (response.success) {
      set_taxation_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_taxation_list();
  }, []);

  const handle_truncate_taxation_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_taxation();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_taxation_list, set_filtered_taxation_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_taxation_list(indexed_data);
  }, [
    taxation_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : taxation_list.length) / select_option
  );
  ...
  const handle_create_new_taxation = () => {
    set_sub_page("create_new_taxation");
  };

  const handle_edit_taxation = (data) => {
    set_edit_data(data);
    set_sub_page("edit_taxation");
  };
  const handle_delete_taxation = (data) => {
    set_delete_data(data);
    set_display_modal("delete_taxation");
  };

  const handle_upload_taxation = () => {
    set_sub_page("upload_taxation");
  };
    ...
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Taxation</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Taxation</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_taxation_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_taxation}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_taxation}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_taxation_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_taxation_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_taxation_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_taxation" && (
        <Create_Taxation
          ...
          set_taxation_list={set_taxation_list}
        />
      )}
      {sub_page === "edit_taxation" && (
        <Edit_Taxation
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_taxation" && (
        <Upload_Taxation
          handle_go_back={handle_go_back}
          handle_get_taxation_list={handle_get_taxation_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Taxation
        is_open={display_modal === "delete_taxation"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_taxation_increment}
      />
    </React.Fragment>
  );
};

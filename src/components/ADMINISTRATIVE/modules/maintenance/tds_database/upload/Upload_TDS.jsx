...
import { push_tds_to_cloud } from "api/real_time_db/maintenance/tds_database_api";

const Upload_TDS = ({ set_page }) => {
  const { show_toast } = useToast();
  const [show_filter, set_show_filter] = useState(false);

  // TDS Database Columns mapped to API keys
  const columns = [
    { key: "index", label: "NO.", sortable: false },
    { key: "id", label: "ID", sortable: true },
    { key: "tDSFullName", label: "FULL NAME", sortable: true },
    { key: "pC", label: "PC CODE", sortable: true },
    { key: "dominantArea", label: "AREA", sortable: true },
  ...
  const [upload_tds_list, set_upload_tds_list] = useState([]);
  ...
  const handle_fetch_data = async () => {
    ...
    try {
      const response = await axios.get(
        "https://benbyextportal.com/home/api/get/GetTDSDatabase?TDSCODE=0",
        {
          ...
          onDownloadProgress: (progressEvent) => {
            ...
            if (total > 0) {
              set_progress(Math.round((current * 100) / total));
            } else {
      ...
      if (response.data) {
        ...
        set_upload_tds_list(formatted_data);
        show_toast({
          type: "success",
          title: "Data Fetched",
          message: `${formatted_data.length} TDS records ready.`,
      ...
      if (!axios.isCancel(error)) {
        show_toast({
          type: "danger",
          title: "Error",
          message: "Could not retrieve TDS data.",
  ...
  const handle_upload = async () => {
    ...
    try {
      await push_tds_to_cloud(
        upload_tds_list,
        (msg) => set_batch_process(msg),
      ...
      show_toast({
        type: "success",
        title: "TDS Synced",
        message: "Records and Accounts updated successfully.",
        icon: <CheckCircle2 size={21} className="text-green-500" />,
      });
      handle_go_back();
    } catch (error) {
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(upload_tds_list, columns);
  ...
  const render_cell = (col, row) => {
    ...
    //     <span className="text-green-600 font-bold px-2 py-1 rounded-full bg-green-50">
    //       ACTIVE
    //     </span>
    //   ) : (
    //     <span className="text-red-600 font-bold px-2 py-1 rounded-full bg-red-50">
    //       INACTIVE
    //     </span>
    //   );
    // }
  ...
  return (
    <React.Fragment>
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                ...
                <span className="text-gray-800">TDS Upload</span>
              </li>
            </ol>
          </nav>
        </div>
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload TDS Master</h1>
            </div>
                      ...
                      <tr>
                        {active_columns.map((col, i) => (
                          <th
                            ...
                            } ${i === 0 ? "border-l-0" : ""} ${i === active_columns.length - 1 ? "border-r-0" : ""}`}
                          >
                      ...
                      {filtered_data.map((row, idx) => (
                        ...
                        >
                          {active_columns.map((col, i) => (
                            <td
                              key={i}
                              className={`border px-4 py-4 text-[12px] text-gray-600 ${i === 0 ? "border-l-0" : ""} ${i === active_columns.length - 1 ? "border-r-0" : ""}`}
                            >
                              {render_cell(col, row)}
                            </td>
                          ))}
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              <Button
                ...
                disabled={upload_tds_list.length === 0 || is_fetching}
              >
                Upload to Cloud
              </Button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in duration-200">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-slate-500 mb-6">
                Downloading TDS Master records...
              </p>
      ...
      {/* + UPLOADING/PUSHING MODAL */}
      {(is_uploading || show_push_tds_alert) && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex flex-col items-center text-center">
              <div className="mb-4 p-3 bg-blue-50 rounded-full">
                <FileUp size={32} className="text-blue-600 animate-bounce" />
              </div>
              <h3 className="text-lg font-semibold text-slate-800">
                Pushing to Cloud
              </h3>
              <p className="text-sm text-slate-500 mb-6">
                Updating DATA and ACCOUNT tables...
              </p>

              <div className="bg-slate-50 border rounded-lg p-3 mb-6 w-full">
                <span className="text-blue-600 font-mono font-bold text-sm">
                  {batch_process || "Initializing..."}
                </span>
              </div>

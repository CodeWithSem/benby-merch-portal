...
import { api_delete_period_ind } from "api/firestore_db/maintenance/batch/tbl_period_ind_api";
import { format_date_1, get_date_now } from "assets/scripts/format";
import { CheckCircle2, CircleX, X } from "lucide-react";
import Text_Field from "assets/elements/Text_Field";
import Button from "assets/elements/Button";

const Delete_Period_Ind = ({
  is_open,
  on_close,
  width = "max-w-[700px]",
  show_toast,
  ...
  const handle_delete_period_ind = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_period_ind(id);

      if (response.success) {
        set_period_ind_list((prev) => prev.filter((item) => item.id !== id));
        show_status("success");
        close_modal();
      } else {
        show_status("error");
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Delete Period Indicator
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to delete this Period Indicator.
            </p>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={() => handle_delete_period_ind(delete_data.id)}
              >
                Yes
              </Button>
              <Button
            ...
            <div className="w-full">
              <div className="w-full bg-white rounded-lg border">
                <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                  <h1 className="text-lg">Period Indicator Details</h1>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.period_ind_code}
                        disabled
                      />
                    </div>
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.period_ind_desc}
                        disabled
                      />
                    </div>
                  </div>
...
export default Delete_Period_Ind;

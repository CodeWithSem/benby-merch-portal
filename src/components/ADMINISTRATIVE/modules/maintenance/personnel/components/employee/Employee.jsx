...
import {
  ...
  api_set_employee_increment,
  api_truncate_employee,
} from "api/firestore_db/maintenance/personnel/tbl_employee_api";
...
import Create_Employee from "./functions/Create_Employee";
import Edit_Employee from "./functions/Edit_Employee";
import Delete_Employee from "./functions/Delete_Employee";
import Upload_Employee from "./functions/Upload_Employee";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Employee = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_employee_data });
  const [edit_data, set_edit_data] = useState({ ...def_employee_data });
  const [delete_data, set_delete_data] = useState({ ...def_employee_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_EMPLOYEE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        employee_code: `EMP-${String(value).padStart(5, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "employee_desc", label: "Employee Description", sortable: true },
  ...
  const handle_get_employee_list = async () => {
    set_loading_list(true);
    const response = await api_get_employee_list();
    if (response.success) {
      set_employee_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_employee_list();
  }, []);

  const handle_truncate_employee_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_employee();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_employee_list, set_filtered_employee_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_employee_list(indexed_data);
  }, [
    employee_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : employee_list.length) / select_option
  );
  ...
  const handle_create_new_employee = () => {
    set_sub_page("create_new_employee");
  };

  const handle_edit_employee = (data) => {
    set_edit_data(data);
    set_sub_page("edit_employee");
  };
  const handle_delete_employee = (data) => {
    set_delete_data(data);
    set_display_modal("delete_employee");
  };

  const handle_upload_employee = () => {
    set_sub_page("upload_employee");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_employee_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_employee}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_employee}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_employee_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_employee_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_employee_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "upload_employee" && (
        <Upload_Employee
          handle_go_back={handle_go_back}
          handle_get_employee_list={handle_get_employee_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Employee
        is_open={display_modal === "delete_employee"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_employee_increment}
      />
    </React.Fragment>
  );
};

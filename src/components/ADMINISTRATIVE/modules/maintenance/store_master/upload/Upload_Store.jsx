...
import { push_store_master_to_cloud } from "api/real_time_db/maintenance/store_master_api";

const Upload_Store = ({ set_page }) => {
  ...
  const columns = [
    ...
    { key: "cstCode", label: "STORE CODE", sortable: true },
    { key: "cstName1", label: "STORE NAME 1", sortable: true },
    { key: "cstName2", label: "STORE NAME 2", sortable: true },
  ...
  const [upload_store_list, set_upload_store_list] = useState([]);
  ...
  const handle_fetch_data = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        set_upload_store_list(formatted_data);
        show_toast({
          type: "success",
          title: "Data Fetched",
          message: `${formatted_data.length} stores are ready to upload.`,
  ...
  const handle_upload = async () => {
    ...
    try {
      await push_store_master_to_cloud(
        upload_store_list,
      ...
      show_toast({
        ...
        message: "All store records pushed to cloud successfully.",
        icon: <CheckCircle2 size={21} className="text-green-500" />,
      });
      handle_go_back();
    } catch (error) {
  ...
  const {
    ...
    filtered_data,
    total_pages,
  } = client_side_filter(upload_store_list, columns);
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              <Button
                ...
                disabled={upload_store_list.length === 0 || is_fetching}
              >
                Upload to Cloud
              </Button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in duration-200">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-slate-500 mb-6">
                Please wait while we retrieve the store master records...
              </p>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-slate-500 mb-6">
                Synchronizing store records with cloud storage...
              </p>
              <div className="w-full bg-slate-100 rounded-full h-2.5 mb-2 overflow-hidden">
                <div
                  className="bg-green-600 h-full transition-all duration-300 ease-out"

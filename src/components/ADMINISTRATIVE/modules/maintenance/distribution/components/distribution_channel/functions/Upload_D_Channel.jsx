...
import { api_bulk_upload_dist_channel } from "api/firestore_db/maintenance/distribution/tbl_dist_channel_api";
...
const Upload_D_Channel = ({
  ...
  const [upload_dist_channel_list, set_upload_dist_channel_list] = useState([]);

  // + Client-Side Filtering
  const [
    filtered_upload_dist_channel_list,
    set_filtered_upload_dist_channel_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_dist_channel_list];
    ...
    set_filtered_upload_dist_channel_list(indexed_data);
  }, [
    upload_dist_channel_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_dist_channel_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_dist_channel_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "dist_channel_code",
      desc_field: "dist_channel_desc",
      set_data_callback: set_upload_dist_channel_list,
      show_toast,
    });
  };

  const handle_upload_dist_channel = async () => {
    if (upload_dist_channel_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_dist_channel(
        upload_dist_channel_list
      );
      ...
      if (response.success) {
        show_toast({
          ...
          message: `${upload_dist_channel_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_dist_channel_list([]);
        handle_get_dist_channel_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Distribution Channel</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_dist_channel_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_dist_channel_list.map((row, idx) => {
...
export default Upload_D_Channel;

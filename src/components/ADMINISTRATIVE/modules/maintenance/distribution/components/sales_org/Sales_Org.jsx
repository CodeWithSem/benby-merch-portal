// File Name: Sales_Org
// Function Name: Sales_Org
// Data Name: sales_org
// Column Name: Sales Organization
// Table Name: TBL_SALES_ORG
// Title Name: Sales Organization
...
import {
  ...
  api_set_sales_org_increment,
  api_truncate_sales_org,
} from "api/firestore_db/maintenance/distribution/tbl_sales_org_api";
...
import Create_Sales_Org from "./functions/Create_Sales_Org";
import Edit_Sales_Org from "./functions/Edit_Sales_Org";
import Delete_Sales_Org from "./functions/Delete_Sales_Org";
import Upload_Sales_Org from "./functions/Upload_Sales_Org";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Sales_Org = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_sales_org_data });
  const [edit_data, set_edit_data] = useState({ ...def_sales_org_data });
  const [delete_data, set_delete_data] = useState({ ...def_sales_org_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_SALES_ORG", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        sales_org_code: `SA-ORG-${String(value).padStart(3, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "sales_org_code", label: "Sales Organization Code", sortable: true },
    {
      key: "sales_org_desc",
  ...
  const handle_get_sales_org_list = async () => {
    set_loading_list(true);
    const response = await api_get_sales_org_list();
    if (response.success) {
      set_sales_org_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_sales_org_list();
  }, []);

  const handle_truncate_sales_org_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_sales_org();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  useEffect(() => {
    ...
    set_filtered_sales_org_list(indexed_data);
  }, [
    sales_org_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : sales_org_list.length) / select_option
  );
  ...
  const handle_create_new_sales_org = () => {
    set_sub_page("create_new_sales_org");
  };

  const handle_edit_sales_org = (data) => {
    set_edit_data(data);
    set_sub_page("edit_sales_org");
  };
  const handle_delete_sales_org = (data) => {
    set_delete_data(data);
    set_display_modal("delete_sales_org");
  };

  const handle_upload_sales_org = () => {
    set_sub_page("upload_sales_org");
  };
    ...
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Sales Organization</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Sales Organization</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_sales_org_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_sales_org}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_sales_org}
                  >
                    Upload
                  </Button>
                </div>
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_sales_org_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_sales_org_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_sales_org" && (
        <Create_Sales_Org
          ...
          set_sales_org_list={set_sales_org_list}
        />
      )}
      {sub_page === "edit_sales_org" && (
        <Edit_Sales_Org
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_sales_org" && (
        <Upload_Sales_Org
          handle_go_back={handle_go_back}
          handle_get_sales_org_list={handle_get_sales_org_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Sales_Org
        is_open={display_modal === "delete_sales_org"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_sales_org_increment}
      />
    </React.Fragment>
  );
};

export default Sales_Org;

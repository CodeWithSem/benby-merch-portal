// File Name: Ship_Condition
// Function Name: Ship_Condition
// Data Name: ship_condition
// Column Name: Shipping Condition
// Title Name: Shipping Condition
// Sub-module Name: Distribution
// Table Name: TBL_SHIPPING_CONDITION
// Default Code: SH-CON
...
import Create_Ship_Condition from "./functions/Create_Ship_Condition";
...
const Ship_Condition = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_SHIPPING_CONDITION", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const columns = [
    ...
    {
      key: "ship_condition_code",
      label: "Shipping Condition Code",
      sortable: true,
    },
    {
      key: "ship_condition_desc",
      label: "Shipping Condition Description",
      sortable: true,
    },
  ...
  const [ship_condition_list, set_ship_condition_list] = useState([]);

  const handle_get_ship_condition_list = async () => {
    set_loading_list(true);
    const response = await api_get_ship_condition_list();
    if (response.success) {
      set_ship_condition_list(response.data);
    } else {
  ...
  const handle_truncate_ship_condition_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_ship_condition();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_ship_condition_list, set_filtered_ship_condition_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_ship_condition_list(indexed_data);
  }, [
    ship_condition_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : ship_condition_list.length) / select_option
  );
  ...
  const handle_create_new_ship_condition = () => {
    set_sub_page("create_new_ship_condition");
  };

  const handle_edit_ship_condition = (data) => {
    set_edit_data(data);
    set_sub_page("edit_ship_condition");
  };
  const handle_delete_ship_condition = (data) => {
    set_delete_data(data);
    set_display_modal("delete_ship_condition");
  };

  const handle_upload_ship_condition = () => {
    set_sub_page("upload_ship_condition");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Shipping Condition</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_ship_condition_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_ship_condition}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_ship_condition}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_ship_condition_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_ship_condition_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_ship_condition_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_ship_condition(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_ship_condition(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_ship_condition" && (
        <Create_Ship_Condition
      ...
      {sub_page === "upload_ship_condition" && (
        <Upload_Ship_Condition
          handle_go_back={handle_go_back}
          handle_get_ship_condition_list={handle_get_ship_condition_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Ship_Condition
        is_open={display_modal === "delete_ship_condition"}

...
import {
  api_get_price_type_list,
  api_set_price_type_increment,
  api_truncate_price_type,
} from "api/firestore_db/maintenance/pricing/tbl_price_type_api";
...
import Create_Pricing_Type from "./functions/Create_Pricing_Type";
import Edit_Pricing_Type from "./functions/Edit_Pricing_Type";
import Delete_Pricing_Type from "./functions/Delete_Pricing_Type";
import Upload_Pricing_Type from "./functions/Upload_Pricing_Type";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Pricing_Type = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_price_type_data });
  const [edit_data, set_edit_data] = useState({ ...def_price_type_data });
  const [delete_data, set_delete_data] = useState({ ...def_price_type_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PRICING_TYPE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        price_type_code: `PT-${String(value).padStart(2, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "price_type_code", label: "Pricing Type Code", sortable: true },
    {
      key: "price_type_desc",
  ...
  const [price_type_list, set_price_type_list] = useState([]);

  const handle_get_price_type_list = async () => {
    set_loading_list(true);
    const response = await api_get_price_type_list();
    if (response.success) {
      set_price_type_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_price_type_list();
  }, []);

  const handle_truncate_price_type_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_price_type();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_price_type_list, set_filtered_price_type_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_price_type_list(indexed_data);
  }, [
    price_type_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : price_type_list.length) / select_option
  );
  ...
  const handle_create_new_price_type = () => {
    set_sub_page("create_new_price_type");
  };

  const handle_edit_price_type = (data) => {
    set_edit_data(data);
    set_sub_page("edit_price_type");
  };
  const handle_delete_price_type = (data) => {
    set_delete_data(data);
    set_display_modal("delete_price_type");
  };

  const handle_upload_price_type = () => {
    set_sub_page("upload_price_type");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Pricing Type</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Pricing Type</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_price_type_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_price_type}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_price_type}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_price_type_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_price_type_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_price_type_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_price_type" && (
        <Create_Pricing_Type
          ...
          set_price_type_list={set_price_type_list}
        />
      )}
      {sub_page === "edit_price_type" && (
        <Edit_Pricing_Type
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_price_type" && (
        <Upload_Pricing_Type
          handle_go_back={handle_go_back}
          handle_get_price_type_list={handle_get_price_type_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Pricing_Type
        is_open={display_modal === "delete_price_type"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_price_type_increment}
      />
    </React.Fragment>
  );
};

export default Pricing_Type;

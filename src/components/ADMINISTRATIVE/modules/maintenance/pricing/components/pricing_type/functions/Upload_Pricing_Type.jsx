...
import { api_bulk_upload_price_type } from "api/firestore_db/maintenance/pricing/tbl_price_type_api";
...
const Upload_Pricing_Type = ({
  ...
  const [upload_price_type_list, set_upload_price_type_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_price_type_list, set_filtered_upload_price_type_list] =
    useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_price_type_list];
    ...
    set_filtered_upload_price_type_list(indexed_data);
  }, [
    upload_price_type_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_price_type_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_price_type_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "price_type_code",
      desc_field: "price_type_desc",
      set_data_callback: set_upload_price_type_list,
      show_toast,
    });
  };

  const handle_upload_price_type = async () => {
    if (upload_price_type_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_price_type(upload_price_type_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_price_type_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_price_type_list([]);
        handle_get_price_type_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Pricing Type</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_price_type_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_price_type_list.map((row, idx) => {
...
export default Upload_Pricing_Type;

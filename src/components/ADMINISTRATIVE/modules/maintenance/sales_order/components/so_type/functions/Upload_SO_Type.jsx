...
import { api_bulk_upload_so_type } from "api/firestore_db/maintenance/sales_order/tbl_so_type_api";
...
const Upload_SO_Type = ({
  ...
  const [upload_so_type_list, set_upload_so_type_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_so_type_list, set_filtered_upload_so_type_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_upload_so_type_list(indexed_data);
  }, [
    upload_so_type_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_so_type_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "so_type_code",
      desc_field: "so_type_desc",
      set_data_callback: set_upload_so_type_list,
      show_toast,
    });
  };

  const handle_upload_so_type = async () => {
    if (upload_so_type_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_so_type(upload_so_type_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_so_type_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_so_type_list([]);
        handle_get_so_type_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Sales Order Type</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_so_type_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_so_type_list.map((row, idx) => {

// File Name: Truck_Location
// Function Name: Truck_Location
// Data Name: truck_location
// Column Name: Truck Location
// Title Name: Truck Location
// Sub-module Name: Truck
// Table Name: TBL_TRUCK_LOCATION
...
import Create_Truck_Location from "./functions/Create_Truck_Location";
...
const Truck_Location = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_TRUCK_LOCATION", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    {
      key: "truck_location_code",
      label: "Truck Location Code",
      sortable: true,
    },
    {
      key: "truck_location_desc",
      label: "Truck Location Description",
      sortable: true,
    },
  ...
  const [truck_location_list, set_truck_location_list] = useState([]);

  const handle_get_truck_location_list = async () => {
    set_loading_list(true);
    const response = await api_get_truck_location_list();
    if (response.success) {
      set_truck_location_list(response.data);
    } else {
  ...
  const handle_truncate_truck_location_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_truck_location();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_truck_location_list, set_filtered_truck_location_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_truck_location_list(indexed_data);
  }, [
    truck_location_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : truck_location_list.length) / select_option
  );
  ...
  const handle_create_new_truck_location = () => {
    set_sub_page("create_new_truck_location");
  };

  const handle_edit_truck_location = (data) => {
    set_edit_data(data);
    set_sub_page("edit_truck_location");
  };
  const handle_delete_truck_location = (data) => {
    set_delete_data(data);
    set_display_modal("delete_truck_location");
  };

  const handle_upload_truck_location = () => {
    set_sub_page("upload_truck_location");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Truck Location</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_truck_location_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_truck_location}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_truck_location}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_truck_location_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_truck_location_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_truck_location_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_truck_location(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_truck_location(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_truck_location" && (
      ...
      {sub_page === "upload_truck_location" && (
        <Upload_Truck_Location
          handle_go_back={handle_go_back}
          handle_get_truck_location_list={handle_get_truck_location_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Truck_Location
        is_open={display_modal === "delete_truck_location"}

...
import {
  api_get_customer_group_list,
  api_set_customer_group_increment,
  api_truncate_customer_group,
} from "api/firestore_db/maintenance/customer/tbl_customer_group_api";
...
import Create_C_Group from "./functions/Create_C_Group";
import Edit_C_Group from "./functions/Edit_C_Group";
import Delete_C_Group from "./functions/Delete_C_Group";
import Upload_C_Group from "./functions/Upload_C_Group";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Customer_Group = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_customer_group_data });
  const [edit_data, set_edit_data] = useState({ ...def_customer_group_data });
  const [delete_data, set_delete_data] = useState({
    ...def_customer_group_data,
  });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_CUSTOMER_GROUP", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        customer_group_code: `C-G-${String(value).padStart(3, "0")}`,
      }));
  ...
  const columns = [
    ...
    {
      key: "customer_group_code",
      label: "Customer Group Code",
      sortable: true,
    },
    {
      key: "customer_group_desc",
  ...
  const [customer_group_list, set_customer_group_list] = useState([]);

  const handle_get_customer_group_list = async () => {
    set_loading_list(true);
    const response = await api_get_customer_group_list();
    if (response.success) {
      set_customer_group_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_customer_group_list();
  }, []);

  const handle_truncate_customer_group_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_customer_group();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_customer_group_list, set_filtered_customer_group_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_customer_group_list(indexed_data);
  }, [
    customer_group_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : customer_group_list.length) / select_option
  );
  ...
  const handle_create_new_customer_group = () => {
    set_sub_page("create_new_customer_group");
  };

  const handle_edit_customer_group = (data) => {
    set_edit_data(data);
    set_sub_page("edit_customer_group");
  };
  const handle_delete_customer_group = (data) => {
    set_delete_data(data);
    set_display_modal("delete_customer_group");
  };

  const handle_upload_customer_group = () => {
    set_sub_page("upload_customer_group");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Customer Group</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Customer Group</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_customer_group_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_customer_group}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_customer_group}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_customer_group_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_customer_group_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_customer_group_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_customer_group(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_customer_group(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_customer_group" && (
        <Create_C_Group
          ...
          set_customer_group_list={set_customer_group_list}
        />
      )}
      {sub_page === "edit_customer_group" && (
        <Edit_C_Group
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_customer_group" && (
        <Upload_C_Group
          handle_go_back={handle_go_back}
          handle_get_customer_group_list={handle_get_customer_group_list}
          show_toast={show_toast}
        />
      )}
      <Delete_C_Group
        is_open={display_modal === "delete_customer_group"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_customer_group_increment}
      />
    </React.Fragment>
  );
};

export default Customer_Group;

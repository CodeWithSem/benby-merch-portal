// File Name: Customer_Acc_Group
// Function Name: C_Acc_Group
// Data Name: customer_acc_group
// Column Name: Customer Account Group
// Title Name: Customer Account Group
// Sub-module Name: Customer
// Table Name: TBL_CUSTOMER_ACCOUNT_GROUP
// Default Code: C-AG
...
import Create_C_Acc_Group from "./functions/Create_C_Acc_Group";
...
const Customer_Acc_Group = ({ set_page }) => {
  ...
  const def_customer_acc_group_data = {
    id: null,
    customer_acc_group_code: "",
    customer_acc_group_desc: "",
    creation_date: "",
    created_by: "",
    change_date: "",
    change_by: "",
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_CUSTOMER_ACCOUNT_GROUP", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
  ...
  const columns = [
    ...
    {
      key: "customer_acc_group_code",
      label: "Customer Account Group Code",
      sortable: true,
    },
    {
      key: "customer_acc_group_desc",
      label: "Customer Account Group Description",
      sortable: true,
    },
  ...
  const [customer_acc_group_list, set_customer_acc_group_list] = useState([]);

  const handle_get_customer_acc_group_list = async () => {
    set_loading_list(true);
    const response = await api_get_customer_acc_group_list();
    if (response.success) {
      set_customer_acc_group_list(response.data);
    } else {
  ...
  const handle_truncate_customer_acc_group_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_customer_acc_group();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [
    filtered_customer_acc_group_list,
    set_filtered_customer_acc_group_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...customer_acc_group_list];
    ...
    set_filtered_customer_acc_group_list(indexed_data);
  }, [
    customer_acc_group_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? customer_acc_group_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : customer_acc_group_list.length) / select_option
  );
  ...
  const handle_create_new_customer_acc_group = () => {
    set_sub_page("create_new_customer_acc_group");
  };

  const handle_edit_customer_acc_group = (data) => {
    set_edit_data(data);
    set_sub_page("edit_customer_acc_group");
  };
  const handle_delete_customer_acc_group = (data) => {
    set_delete_data(data);
    set_display_modal("delete_customer_acc_group");
  };

  const handle_upload_customer_acc_group = () => {
    set_sub_page("upload_customer_acc_group");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_customer_acc_group_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_customer_acc_group}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_customer_acc_group}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_customer_acc_group_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_customer_acc_group_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_customer_acc_group_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_edit_customer_acc_group(row)
                                        }
                                      >
                                        <Edit size={19} />
                                      </button>
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() =>
                                          handle_delete_customer_acc_group(row)
                                        }
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "create_new_customer_acc_group" && (
        <Create_C_Acc_Group
      ...
      {sub_page === "edit_customer_acc_group" && (
        <Edit_C_Acc_Group
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_customer_acc_group" && (
        <Upload_C_Acc_Group
          handle_go_back={handle_go_back}
          handle_get_customer_acc_group_list={
            handle_get_customer_acc_group_list
      ...
      <Delete_C_Acc_Group
        is_open={display_modal === "delete_customer_acc_group"}
...
export default Customer_Acc_Group;

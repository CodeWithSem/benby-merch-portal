...
} from "api/firestore_db/tbl_currency_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_Currency from "./functions/Create_Currency";
...
const Currency = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_CURRENCY", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "currency_code", label: "Currency Code", sortable: true },
    {
      key: "currency_desc",
  ...
  // ======================================================
  // id: 1,
  // currency_code: "Currency-001",
  // currency_desc: "Currency Description 1",
  // created_by: "Admin",
  ...
  const handle_get_currency_list = async () => {
    set_loading_list(true);
    const response = await api_get_currency_list();
    if (response.success) {
      set_currency_list(response.data);
    } else {
  ...
  const handle_truncate_currency_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_currency();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_currency_list, set_filtered_currency_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_currency_list(indexed_data);
  }, [
    currency_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : currency_list.length) / select_option
  );
  ...
  const handle_create_new_currency = () => {
    set_sub_page("create_new_currency");
  };

  const handle_edit_currency = (data) => {
    set_edit_data(data);
    set_sub_page("edit_currency");
  };
  const handle_delete_currency = (data) => {
    set_delete_data(data);
    set_display_modal("delete_currency");
  };

  const handle_upload_currency = () => {
    set_sub_page("upload_currency");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_currency_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_currency}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_currency}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_currency_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_currency_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_currency_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "upload_currency" && (
        <Upload_Currency
          handle_go_back={handle_go_back}
          handle_get_currency_list={handle_get_currency_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Currency
        is_open={display_modal === "delete_currency"}

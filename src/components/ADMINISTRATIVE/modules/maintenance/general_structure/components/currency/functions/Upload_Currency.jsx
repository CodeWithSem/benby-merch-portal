...
import { api_bulk_upload_currency } from "api/firestore_db/tbl_currency_api";
...
const Upload_Currency = ({
  ...
  const [upload_currency_list, set_upload_currency_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_currency_list, set_filtered_upload_currency_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_upload_currency_list(indexed_data);
  }, [
    upload_currency_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_currency_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      ...
      desc_field: "currency_desc",
      set_data_callback: set_upload_currency_list,
      show_toast,
    });
  };

  const handle_upload_currency = async () => {
    if (upload_currency_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_currency(upload_currency_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_currency_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_currency_list([]);
        handle_get_currency_list();
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Upload Currency</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Currency</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_currency_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_currency_list.map((row, idx) => {
...
export default Upload_Currency;

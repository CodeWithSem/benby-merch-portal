...
import { api_delete_currency } from "api/firestore_db/maintenance/general_structure/tbl_currency_api";
...
const Delete_Currency = ({
  ...
  const handle_delete_currency = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_currency(id);

      if (response.success) {
        set_currency_list((prev) => prev.filter((item) => item.id !== id));
        show_status("success");
        close_modal();
      } else {
        show_status("error");
          ...
          >
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={() => handle_delete_currency(delete_data.id)}
              >
                Yes
              </Button>
              <Button
            ...
            <div className="w-full">
              <div className="w-full bg-white rounded-lg border">
                <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                  <h1 className="text-lg">Currency Details</h1>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="col-span-full">
                      <Text_Field
                        label="Currency Code"
                        type={"text"}
                        value={delete_data.currency_code}
                        disabled
                      />
                    </div>
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.currency_desc}
                        disabled
                      />
                    </div>
                  </div>

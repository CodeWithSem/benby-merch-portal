...
import {
  ...
  api_set_district_increment,
  api_truncate_district,
} from "api/firestore_db/maintenance/general_structure/tbl_district_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_District from "./functions/Create_District";
import Edit_District from "./functions/Edit_District";
import Delete_District from "./functions/Delete_District";
import Upload_District from "./functions/Upload_District";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const District = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_district_data });
  const [edit_data, set_edit_data] = useState({ ...def_district_data });
  const [delete_data, set_delete_data] = useState({ ...def_district_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_DISTRICT", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "district_code", label: "District Code", sortable: true },
    {
      key: "district_desc",
  ...
  // ======================================================
  // id: 1,
  // district_code: "District-001",
  // district_desc: "District Description 1",
  // created_by: "Admin",
  ...
  const handle_get_district_list = async () => {
    set_loading_list(true);
    const response = await api_get_district_list();
    if (response.success) {
      set_district_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_district_list();
  }, []);

  const handle_truncate_district_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_district();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_district_list, set_filtered_district_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_district_list(indexed_data);
  }, [
    district_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : district_list.length) / select_option
  );
  ...
  const handle_create_new_district = () => {
    set_sub_page("create_new_district");
  };

  const handle_edit_district = (data) => {
    set_edit_data(data);
    set_sub_page("edit_district");
  };
  const handle_delete_district = (data) => {
    set_delete_data(data);
    set_display_modal("delete_district");
  };

  const handle_upload_district = () => {
    set_sub_page("upload_district");
  };
  ...
  return (
    <React.Fragment>
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">District</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_district_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_district}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_district}
                  >
                    Upload
                  </Button>
                </div>
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_district_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_district_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "upload_district" && (
        <Upload_District
          handle_go_back={handle_go_back}
          handle_get_district_list={handle_get_district_list}
          show_toast={show_toast}
        />
      )}
      <Delete_District
        is_open={display_modal === "delete_district"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_district_increment}
      />
    </React.Fragment>
  );
};

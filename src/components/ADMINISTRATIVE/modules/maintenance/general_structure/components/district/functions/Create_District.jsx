...
const Create_District = ({
  ...
  const handle_change_district_code = (value) => {
    set_new_data((prev) => ({
      ...prev,
      district_code: value,
    }));
  ...
  const handle_change_district_desc = (value) => {
    set_new_data((prev) => ({
      ...prev,
      district_desc: value,
    }));
  ...
  const validate_new_data = () => {
    if (!new_data.district_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!new_data.district_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_create_district = async () => {
    ...
    try {
      ...
      const response = await api_create_district(
        new_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_district_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new District. Once created, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_create_district}
              >
                Yes
              </Button>
              <Button
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">District Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  value={new_data.district_code || ""}
                  on_change={(e) => handle_change_district_code(e.target.value)}
                />
              </div>
...
export default Create_District;

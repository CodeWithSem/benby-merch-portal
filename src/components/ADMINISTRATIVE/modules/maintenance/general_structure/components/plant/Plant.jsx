...
import Create_Plant from "./functions/Create_Plant";
import Edit_Plant from "./functions/Edit_Plant";
import Delete_Plant from "./functions/Delete_Plant";
import Spinner from "assets/elements/Spinner";
import Upload_Plant from "./functions/Upload_Plant";
import Text_Field from "assets/elements/Text_Field";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Plant = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_plant_data });
  const [edit_data, set_edit_data] = useState({ ...def_plant_data });
  const [delete_data, set_delete_data] = useState({ ...def_plant_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PLANT", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        plant_code: `PL-${String(value).padStart(3, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "plant_desc", label: "Plant Description", sortable: true },
  ...
  const handle_get_plant_list = async () => {
    set_loading_list(true);
    const response = await api_get_plant_list();
    if (response.success) {
      set_plant_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_plant_list();
  }, []);

  const handle_truncate_plant_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_plant();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  useEffect(() => {
    ...
    set_filtered_plant_list(indexed_data);
  }, [
    plant_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : plant_list.length) / select_option
  );
  ...
  const handle_create_new_plant = () => {
    set_sub_page("create_new_plant");
  };

  const handle_edit_plant = (data) => {
    set_edit_data(data);
    set_sub_page("edit_plant");
  };
  const handle_delete_plant = (data) => {
    set_delete_data(data);
    set_display_modal("delete_plant");
  };

  const handle_upload_plant = () => {
    set_sub_page("upload_plant");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_plant_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_plant}
                  >
                    Create New Data
                  </Button>
                  <Button
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_plant_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_plant_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_delete_plant(row)}
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "upload_plant" && (
        <Upload_Plant
          handle_go_back={handle_go_back}
          handle_get_plant_list={handle_get_plant_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Plant
        is_open={display_modal === "delete_plant"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_plant_increment}
      />
    </React.Fragment>
  );
};

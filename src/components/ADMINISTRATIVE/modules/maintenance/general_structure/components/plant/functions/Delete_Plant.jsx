...
import { api_delete_plant } from "api/firestore_db/maintenance/general_structure/tbl_plant_api";
...
const Delete_Plant = ({
  ...
  const handle_delete_plant = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_plant(id);

      if (response.success) {
        set_plant_list((prev) => prev.filter((item) => item.id !== id));
        show_status("success");
        close_modal();
      } else {
        show_status("error");
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to delete this Plant.
            </p>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={() => handle_delete_plant(delete_data.id)}
              >
                Yes
              </Button>
              <Button
    ...
    <React.Fragment>
      <div className="fixed inset-0 flex items-center justify-center z-[97] px-4">
        ...
        >
          ...
          {/* + Modal Label */}
          <div className="text-lg md:text-xl font-bold mb-5">Delete Plant</div>
          {/* - Modal Label */}
          {/* + Modal Body */}
            ...
            <div className="w-full">
              <div className="w-full bg-white rounded-lg border">
                <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                  <h1 className="text-lg">Plant Details</h1>

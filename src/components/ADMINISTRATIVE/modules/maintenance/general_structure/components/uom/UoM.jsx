...
import {
  api_get_uom_list,
  api_set_uom_increment,
  api_truncate_uom,
} from "api/firestore_db/maintenance/general_structure/tbl_uom_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_UoM from "./functions/Create_UoM";
import Edit_UoM from "./functions/Edit_UoM";
import Delete_UoM from "./functions/Delete_UoM";
import Upload_UoM from "./functions/Upload_UoM";
...
const UoM = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_uom_data });
  const [edit_data, set_edit_data] = useState({ ...def_uom_data });
  const [delete_data, set_delete_data] = useState({ ...def_uom_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_UNIT_OF_MEASURE", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "uom_code", label: "Unit of Measure Code", sortable: true },
    {
      key: "uom_desc",
  ...
  const [uom_list, set_uom_list] = useState([]);
  // ======================================================
  // id: 1,
  // uom_code: "UoM-001",
  // uom_desc: "Unit of Measure Description 1",
  // created_by: "Admin",
  ...
  const handle_get_uom_list = async () => {
    set_loading_list(true);
    const response = await api_get_uom_list();
    if (response.success) {
      set_uom_list(response.data);
    } else {
  ...
  const handle_truncate_uom_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_uom();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_uom_list, set_filtered_uom_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_uom_list(indexed_data);
  }, [
    uom_list,
    debounced_query,
    sort_by,
  ...
  const handle_create_new_uom = () => {
    set_sub_page("create_new_uom");
  };

  const handle_edit_uom = (data) => {
    set_edit_data(data);
    set_sub_page("edit_uom");
  };
  const handle_delete_uom = (data) => {
    set_delete_data(data);
    set_display_modal("delete_uom");
  };

  const handle_upload_uom = () => {
    set_sub_page("upload_uom");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Unit of Measure</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Unit of Measure</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_uom_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_uom}
                  >
                    Create New Data
                  </Button>
                  <Button
      ...
      {sub_page === "upload_uom" && (
        <Upload_UoM
          handle_go_back={handle_go_back}
          handle_get_uom_list={handle_get_uom_list}
          show_toast={show_toast}
        />
      )}
      <Delete_UoM
        is_open={display_modal === "delete_uom"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_uom_increment}
      />
    </React.Fragment>
  );
};

...
import { api_bulk_upload_uom } from "api/firestore_db/tbl_uom_api";
...
const Upload_UoM = ({ handle_go_back, handle_get_uom_list, show_toast }) => {
  ...
  const [upload_uom_list, set_upload_uom_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_uom_list, set_filtered_upload_uom_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_upload_uom_list(indexed_data);
  }, [
    upload_uom_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_uom_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      ...
      code_field: "uom_code",
      desc_field: "uom_desc",
      set_data_callback: set_upload_uom_list,
      show_toast,
    });
  };

  const handle_upload_uom = async () => {
    if (upload_uom_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_uom(upload_uom_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_uom_list.length} data has been uploaded.`,
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Upload Unit of Measure</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Unit of Measure</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_uom_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_uom_list.map((row, idx) => {

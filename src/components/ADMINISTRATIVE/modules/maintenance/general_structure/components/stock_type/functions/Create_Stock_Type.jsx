...
import { api_create_stock_type } from "api/firestore_db/tbl_stock_type_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Create_Stock_Type = ({
  ...
  const validate_new_data = () => {
    if (!new_data.stock_type_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!new_data.stock_type_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_create_stock_type = async () => {
    ...
    try {
      ...
      const response = await api_create_stock_type(
        new_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_stock_type_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm Stock Type Creation
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new Stock Type. Once created, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_create_stock_type}
              >
                Yes
              </Button>
              <Button
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Create New Stock Type</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Stock Type Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  type={"text"}
                  value={new_data.stock_type_code || ""}
                  disabled
                />
              </div>
              <div className="col-span-2">
                <Text_Field
                  ...
                  value={new_data.stock_type_desc || ""}
                  on_change={(e) =>
...
export default Create_Stock_Type;

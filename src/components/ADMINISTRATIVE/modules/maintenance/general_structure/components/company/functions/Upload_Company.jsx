...
const Upload_Company = ({
  ...
  const [filtered_upload_company_list, set_filtered_upload_company_list] =
    useState([]);
  const [loading_upload_list, set_loading_upload_list] = useState(false);
  ...
  const handle_excel_upload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      for (let index = 0; index < json.length; index++) {
        ...
        const errors = [];
        const row_num = index + 2; // Excel row number (1 = header)
        ...
        if (
          ...
          errors.push(`Row ${row_num}: company_code must be a string.`);
        }

        // --- REQUIRED: description ---
        if (
          typeof row.company_desc !== "string" ||
          row.company_desc.trim() === ""
        ) {
          errors.push(`Row ${row_num}: company_desc must be a string.`);
        }
        ...
        const code_value = row.company_code.trim().toLowerCase();
      ...
      set_upload_company_list(validated);
  ...
  const handle_upload_company = async () => {
    if (upload_company_list.length === 0) {
      show_toast({
        type: "danger",
        title: "No Data",
        message: "No company data to upload",
      });
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successful",
          message: `${upload_company_list.length} data uploaded!`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });

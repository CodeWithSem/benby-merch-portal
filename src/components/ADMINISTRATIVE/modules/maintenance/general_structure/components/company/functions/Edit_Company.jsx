...
import { api_update_company } from "api/firestore_db/maintenance/general_structure/tbl_company_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Edit_Company = ({
  ...
  const validate_edit_data = () => {
    ...
    if (!edit_data.company_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_update_company = async () => {
    ...
    try {
      ...
      const response = await api_update_company(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_company_list((prev) =>
          prev.map((item) =>
          ...
          >
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_update_company}
              >
                Yes
              </Button>
              <Button

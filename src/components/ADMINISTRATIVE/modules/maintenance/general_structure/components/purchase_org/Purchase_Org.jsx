...
import Create_P_Org from "./functions/Create_P_Org";
import Edit_P_Org from "./functions/Edit_P_Org";
import Delete_P_Org from "./functions/Delete_P_Org";
import Text_Field from "assets/elements/Text_Field";
import Spinner from "assets/elements/Spinner";
import Upload_P_Org from "./functions/Upload_P_Org";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Purchase_Org = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_purc_org_data });
  const [edit_data, set_edit_data] = useState({ ...def_purc_org_data });
  const [delete_data, set_delete_data] = useState({ ...def_purc_org_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PURCHASING_ORG", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        purc_org_code: `P-ORG-${String(value).padStart(3, "0")}`,
      }));
  ...
  // purc_org_desc: "Purchasing Organization Description 1",
  // created_by: "Admin",
  ...
  const handle_get_purc_org_list = async () => {
    set_loading_list(true);
    const response = await api_get_purc_org_list();
    if (response.success) {
      set_purc_org_list(response.data);
    } else {
  ...
  const handle_truncate_purc_org_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_purc_org();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_purc_org_list, set_filtered_purc_org_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_purc_org_list(indexed_data);
  }, [
    purc_org_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : purc_org_list.length) / select_option
  );
  ...
  const handle_create_new_purc_org = () => {
    set_sub_page("create_new_purc_org");
  };

  const handle_edit_purc_org = (data) => {
    set_edit_data(data);
    set_sub_page("edit_purc_org");
  };
  const handle_delete_purc_org = (data) => {
    set_delete_data(data);
    set_display_modal("delete_purc_org");
  };

  const handle_upload_purc_org = () => {
    set_sub_page("upload_purc_org");
  };
    ...
    <React.Fragment>
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Purchasing Organization</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_purc_org_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_purc_org}
                  >
                    Create New Data
                  </Button>
                  <Button
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_purc_org_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_purc_org_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_purc_org_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_purc_org" && (
        <Create_P_Org
          ...
          set_purc_org_list={set_purc_org_list}
        />
      )}
      {sub_page === "edit_purc_org" && (
        <Edit_P_Org
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_purc_org" && (
        <Upload_P_Org
          handle_go_back={handle_go_back}
          handle_get_purc_org_list={handle_get_purc_org_list}
          show_toast={show_toast}
        />
      )}
      <Delete_P_Org
        is_open={display_modal === "delete_purc_org"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_purc_org_increment}
      />
    </React.Fragment>
  );
};

export default Purchase_Org;

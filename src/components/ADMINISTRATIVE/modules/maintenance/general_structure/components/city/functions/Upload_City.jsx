...
import { api_bulk_upload_city } from "api/firestore_db/tbl_city_api";
...
const Upload_City = ({ handle_go_back, handle_get_city_list, show_toast }) => {
  ...
  const [upload_city_list, set_upload_city_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_city_list, set_filtered_upload_city_list] = useState(
    []
  );
  ...
  useEffect(() => {
    ...
    set_filtered_upload_city_list(indexed_data);
  }, [
    upload_city_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_city_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      ...
      desc_field: "city_desc",
      set_data_callback: set_upload_city_list,
      show_toast,
    });
  };

  const handle_upload_city = async () => {
    if (upload_city_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_city(upload_city_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_city_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_city_list([]);
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Upload City</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload City</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_city_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_city_list.map((row, idx) => {

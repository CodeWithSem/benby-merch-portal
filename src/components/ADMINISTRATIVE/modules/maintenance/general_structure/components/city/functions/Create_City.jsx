...
const Create_City = ({
  ...
  const handle_change_city_code = (value) => {
    set_new_data((prev) => ({
      ...prev,
      city_code: value,
    }));
  ...
  const handle_change_city_desc = (value) => {
    set_new_data((prev) => ({
      ...prev,
      city_desc: value,
    }));
  ...
  const handle_create_city = async () => {
    ...
    try {
      ...
      const response = await api_create_city(new_data, active_user?.username);
      if (response.success) {
        console_log(response.data);
        set_city_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new City. Once created, it will be added
              to the database.
            </p>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">City Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  value={new_data.city_code || ""}
                  on_change={(e) => handle_change_city_code(e.target.value)}
                />
              </div>
              <div className="col-span-2">
                <Text_Field
                  ...
                  value={new_data.city_desc || ""}
                  on_change={(e) => handle_change_city_desc(e.target.value)}
                />
              </div>
            </div>
          </div>

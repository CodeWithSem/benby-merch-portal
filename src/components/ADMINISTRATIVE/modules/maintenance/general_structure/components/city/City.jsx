...
import {
  api_get_city_list,
  api_set_city_increment,
  api_truncate_city,
} from "api/firestore_db/maintenance/general_structure/tbl_city_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_City from "./functions/Create_City";
import Edit_City from "./functions/Edit_City";
import Delete_City from "./functions/Delete_City";
import Upload_City from "./functions/Upload_City";
...
const City = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_city_data });
  const [edit_data, set_edit_data] = useState({ ...def_city_data });
  const [delete_data, set_delete_data] = useState({ ...def_city_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_CITY", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const columns = [
    ...
    { key: "city_code", label: "City Code", sortable: true },
    {
      key: "city_desc",
  ...
  const handle_get_city_list = async () => {
    set_loading_list(true);
    const response = await api_get_city_list();
    if (response.success) {
      set_city_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_city_list();
  }, []);

  const handle_truncate_city_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_city();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  useEffect(() => {
    ...
    set_filtered_city_list(indexed_data);
  }, [
    city_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : city_list.length) / select_option
  );
  ...
  const handle_create_new_city = () => {
    set_sub_page("create_new_city");
  };

  const handle_edit_city = (data) => {
    set_edit_data(data);
    set_sub_page("edit_city");
  };
  const handle_delete_city = (data) => {
    set_delete_data(data);
    set_display_modal("delete_city");
  };

  const handle_upload_city = () => {
    set_sub_page("upload_city");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_city_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_city}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_city_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_delete_city(row)}
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "upload_city" && (
        <Upload_City
          handle_go_back={handle_go_back}
          handle_get_city_list={handle_get_city_list}
          show_toast={show_toast}
        />
      )}
      <Delete_City
        is_open={display_modal === "delete_city"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_city_increment}
      />
    </React.Fragment>
  );
};

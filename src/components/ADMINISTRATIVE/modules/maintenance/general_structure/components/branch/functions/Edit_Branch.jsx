...
import { api_update_branch } from "api/firestore_db/tbl_branch_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Edit_Branch = ({
  ...
  const handle_change_branch_desc = (value) => {
    set_edit_data({
      ...edit_data,
      branch_desc: value,
    });
  ...
  const handle_update_branch = async () => {
    ...
    try {
      ...
      const response = await api_update_branch(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_branch_list((prev) =>
          prev.map((item) =>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm Branch Update
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to edit this Branch. Once edited, it will be updated
              to the database.
            </p>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Edit Branch</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Edit Branch</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              ...
              <div className="col-span-2">
                <Text_Field
                  ...
                  value={edit_data.branch_desc}
                  on_change={(e) => handle_change_branch_desc(e.target.value)}
                />
              </div>
            </div>
          </div>

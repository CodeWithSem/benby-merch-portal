...
const Upload_Branch = ({
  ...
  const [upload_branch_list, set_upload_branch_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_branch_list, set_filtered_upload_branch_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_upload_branch_list(indexed_data);
  }, [
    upload_branch_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_branch_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      ...
      set_data_callback: set_upload_branch_list,
      show_toast,
    });
  };

  const handle_upload_branch = async () => {
    if (upload_branch_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_branch(upload_branch_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_branch_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_branch_list([]);
        handle_get_branch_list();
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Branch</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_branch_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_branch_list.map((row, idx) => {

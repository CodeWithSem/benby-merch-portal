...
import { api_delete_branch } from "api/firestore_db/tbl_branch_api";
...
const Delete_Branch = ({
  ...
  const handle_delete_branch = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_branch(id);

      if (response.success) {
        set_branch_list((prev) => prev.filter((item) => item.id !== id));
        show_status("success");
        close_modal();
      } else {
        show_status("error");
          ...
          >
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={() => handle_delete_branch(delete_data.id)}
              >
                Yes
              </Button>
              <Button

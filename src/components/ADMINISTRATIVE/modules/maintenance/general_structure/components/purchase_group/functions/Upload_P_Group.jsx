...
import { api_bulk_upload_purc_group } from "api/firestore_db/tbl_purc_group_api";
...
const Upload_P_Group = ({
  ...
  const [upload_purc_group_list, set_upload_purc_group_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_purc_group_list, set_filtered_upload_purc_group_list] =
    useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_purc_group_list];
    ...
    set_filtered_upload_purc_group_list(indexed_data);
  }, [
    upload_purc_group_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_purc_group_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_purc_group_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "purc_group_code",
      desc_field: "purc_group_desc",
      set_data_callback: set_upload_purc_group_list,
      show_toast,
    });
  };

  const handle_upload_purc_group = async () => {
    if (upload_purc_group_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_purc_group(upload_purc_group_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_purc_group_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_purc_group_list([]);
        handle_get_purc_group_list();
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Upload Purchasing Group</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Purchasing Group</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_purc_group_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_purc_group_list.map((row, idx) => {

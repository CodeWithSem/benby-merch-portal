...
import {
  ...
  api_set_purc_group_increment,
  api_truncate_purc_group,
} from "api/firestore_db/maintenance/general_structure/tbl_purc_group_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_P_Group from "./functions/Create_P_Group";
import Edit_P_Group from "./functions/Edit_P_Group";
import Delete_P_Group from "./functions/Delete_P_Group";
import Upload_P_Group from "./functions/Upload_P_Group";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Purchase_Group = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_purc_group_data });
  const [edit_data, set_edit_data] = useState({ ...def_purc_group_data });
  const [delete_data, set_delete_data] = useState({ ...def_purc_group_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_PURCHASING_GROUP", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        purc_group_code: `P-GRP-${String(value).padStart(3, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "purc_group_code", label: "Purchasing Group Code", sortable: true },
    {
      key: "purc_group_desc",
  ...
  const handle_get_purc_group_list = async () => {
    set_loading_list(true);
    const response = await api_get_purc_group_list();
    if (response.success) {
      set_purc_group_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_purc_group_list();
  }, []);

  const handle_truncate_purc_group_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_purc_group();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_purc_group_list, set_filtered_purc_group_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_purc_group_list(indexed_data);
  }, [
    purc_group_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : purc_group_list.length) / select_option
  );
  ...
  const handle_create_new_purc_group = () => {
    set_sub_page("create_new_purc_group");
  };

  const handle_edit_purc_group = (data) => {
    set_edit_data(data);
    set_sub_page("edit_purc_group");
  };
  const handle_delete_purc_group = (data) => {
    set_delete_data(data);
    set_display_modal("delete_purc_group");
  };

  const handle_upload_purc_group = () => {
    set_sub_page("upload_purc_group");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Purchasing Group</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Purchasing Group</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_purc_group_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_purc_group}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_purc_group}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_purc_group_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_purc_group_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_purc_group_list.map((row, idx) => {
                            // + Cell Renderer
      ...
      {sub_page === "create_new_purc_group" && (
        <Create_P_Group
          ...
          set_purc_group_list={set_purc_group_list}
        />
      )}
      {sub_page === "edit_purc_group" && (
        <Edit_P_Group
          handle_go_back={handle_go_back}
          active_user={active_user}
          show_toast={show_toast}
      ...
      {sub_page === "upload_purc_group" && (
        <Upload_P_Group
          handle_go_back={handle_go_back}
          handle_get_purc_group_list={handle_get_purc_group_list}
          show_toast={show_toast}
        />
      )}
      <Delete_P_Group
        is_open={display_modal === "delete_purc_group"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_purc_group_increment}
      />
    </React.Fragment>
  );
};

export default Purchase_Group;

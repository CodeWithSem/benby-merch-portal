...
import { api_create_pl_status } from "api/firestore_db/tbl_pl_status_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Create_PL_Status = ({
  ...
  const validate_new_data = () => {
    if (!new_data.pl_status_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!new_data.pl_status_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_create_pl_status = async () => {
    ...
    try {
      ...
      const response = await api_create_pl_status(
        new_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_pl_status_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
          ...
          >
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_create_pl_status}
              >
                Yes
              </Button>
              <Button

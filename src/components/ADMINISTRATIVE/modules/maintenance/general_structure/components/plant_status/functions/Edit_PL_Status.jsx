...
import { api_update_pl_status } from "api/firestore_db/tbl_pl_status_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Edit_PL_Status = ({
  ...
  const validate_edit_data = () => {
    if (!edit_data.pl_status_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!edit_data.pl_status_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_update_pl_status = async () => {
    ...
    try {
      ...
      const response = await api_update_pl_status(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_pl_status_list((prev) =>
          prev.map((item) =>
      ...
      console.error("Failed to create pl_status:", error);
      show_status("error");
    } finally {
      close_confirm_modal();
    }
          ...
          >
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_update_pl_status}
              >
                Yes
              </Button>
              <Button

...
import { api_bulk_upload_plant_status } from "api/firestore_db/tbl_plant_status_api";
...
const Upload_PL_Status = ({
  ...
  const [upload_plant_status_list, set_upload_plant_status_list] = useState([]);

  // + Client-Side Filtering
  const [
    filtered_upload_plant_status_list,
    set_filtered_upload_plant_status_list,
  ] = useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_plant_status_list];
    ...
    set_filtered_upload_plant_status_list(indexed_data);
  }, [
    upload_plant_status_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_plant_status_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_plant_status_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "plant_status_code",
      desc_field: "plant_status_desc",
      set_data_callback: set_upload_plant_status_list,
      show_toast,
    });
  };

  const handle_upload_plant_status = async () => {
    if (upload_plant_status_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_plant_status(
        upload_plant_status_list
      );
      ...
      if (response.success) {
        show_toast({
          ...
          message: `${upload_plant_status_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_plant_status_list([]);
        handle_get_plant_status_list();
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Upload Plant Status</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Plant Status</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_plant_status_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_plant_status_list.map((row, idx) => {
...
export default Upload_PL_Status;

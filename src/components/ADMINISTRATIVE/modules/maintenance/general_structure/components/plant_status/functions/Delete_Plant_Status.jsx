...
import { api_delete_plant_status } from "api/firestore_db/maintenance/general_structure/tbl_plant_status_api";
...
const Delete_PL_Status = ({
  ...
  const handle_delete_plant_status = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_plant_status(id);

      if (response.success) {
        set_plant_status_list((prev) => prev.filter((item) => item.id !== id));
        show_status("success");
        close_modal();
      } else {
        show_status("error");
          ...
          >
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={() => handle_delete_plant_status(delete_data.id)}
              >
                Yes
              </Button>
              <Button

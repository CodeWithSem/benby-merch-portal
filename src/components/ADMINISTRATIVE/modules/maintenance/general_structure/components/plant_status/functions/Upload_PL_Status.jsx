...
import { api_bulk_upload_pl_status } from "api/firestore_db/tbl_pl_status_api";
...
const Upload_PL_Status = ({
  ...
  const [upload_pl_status_list, set_upload_pl_status_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_pl_status_list, set_filtered_upload_pl_status_list] =
    useState([]);
  ...
  useEffect(() => {
    let temp = [...upload_pl_status_list];
    ...
    set_filtered_upload_pl_status_list(indexed_data);
  }, [
    upload_pl_status_list,
    debounced_query,
    sort_by,
    sort_order,
    current_page,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ? upload_pl_status_list.filter((u) =>
          columns.some((col) => {
            if (col.key === "actions") return false;
            const val = u[col.key];
            return val
      ...
      : upload_pl_status_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "pl_status_code",
      desc_field: "pl_status_desc",
      set_data_callback: set_upload_pl_status_list,
      show_toast,
    });
  };

  const handle_upload_pl_status = async () => {
    if (upload_pl_status_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_pl_status(upload_pl_status_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_pl_status_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_pl_status_list([]);
        handle_get_pl_status_list();
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_pl_status_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_pl_status_list.map((row, idx) => {

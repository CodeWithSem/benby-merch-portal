...
import { api_update_plant_status } from "api/firestore_db/tbl_plant_status_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Edit_PL_Status = ({
  ...
  const validate_edit_data = () => {
    if (!edit_data.plant_status_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!edit_data.plant_status_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_update_plant_status = async () => {
    ...
    try {
      ...
      const response = await api_update_plant_status(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_plant_status_list((prev) =>
          prev.map((item) =>
      ...
      console.error("Failed to create plant_status:", error);
      show_status("error");
    } finally {
      close_confirm_modal();
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm Plant Status Update
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to edit this Plant Status. Once edited, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_update_plant_status}
              >
                Yes
              </Button>
              <Button
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Edit Plant Status</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Edit Plant Status</h1>
            </div>

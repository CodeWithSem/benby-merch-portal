...
import { api_create_language } from "api/firestore_db/tbl_language_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Create_Language = ({
  ...
  const handle_change_language_code = (value) => {
    set_new_data((prev) => ({
      ...prev,
      language_code: value,
    }));
  ...
  const validate_new_data = () => {
    if (!new_data.language_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!new_data.language_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_create_language = async () => {
    ...
    try {
      ...
      const response = await api_create_language(
        new_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_language_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            <div className="w-full flex justify-center items-center text-lg md:text-xl font-bold mb-4">
              Confirm Language Creation
            </div>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new Language. Once created, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_create_language}
              >
                Yes
              </Button>
              <Button
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Create New Language</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Language Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  value={new_data.language_code || ""}
                  on_change={(e) => handle_change_language_code(e.target.value)}
                />
              </div>
...
export default Create_Language;

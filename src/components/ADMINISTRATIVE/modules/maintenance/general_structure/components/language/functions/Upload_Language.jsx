...
import { api_bulk_upload_language } from "api/firestore_db/tbl_language_api";
...
const Upload_Language = ({
  ...
  const [upload_language_list, set_upload_language_list] = useState([]);

  // + Client-Side Filtering
  const [filtered_upload_language_list, set_filtered_upload_language_list] =
    useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_upload_language_list(indexed_data);
  }, [
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : upload_language_list.length) / select_option
  );
  ...
  const handle_excel_upload = (e) => {
    handle_excel_upload_generic({
      e,
      code_field: "language_code",
      desc_field: "language_desc",
      set_data_callback: set_upload_language_list,
      show_toast,
    });
  };

  const handle_upload_language = async () => {
    if (upload_language_list.length === 0) {
      show_toast({
        type: "danger",
        title: "Invalid Data",
    ...
    try {
      const response = await api_bulk_upload_language(upload_language_list);

      if (response.success) {
        show_toast({
          type: "success",
          title: "Upload Successfully",
          message: `${upload_language_list.length} data has been uploaded.`,
          icon: <CheckCircle2 size={21} className="text-green-500" />,
        });
        set_upload_language_list([]);
        handle_get_language_list();
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Upload Language</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Upload Language</h1>
            </div>
          ...
          <div className="p-5 sm:p-6 border-t">
            <div className="w-full border rounded-lg">
              ...
              <div className="overflow-x-auto">
                {filtered_upload_language_list.length === 0 ? (
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="bg-white">
                      {filtered_upload_language_list.map((row, idx) => {
...
export default Upload_Language;

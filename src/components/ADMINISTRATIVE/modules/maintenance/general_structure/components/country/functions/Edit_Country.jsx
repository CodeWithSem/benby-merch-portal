...
import { api_update_country } from "api/firestore_db/tbl_country_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Edit_Country = ({
  ...
  const validate_edit_data = () => {
    if (!edit_data.country_code.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
    ...
    if (!edit_data.country_desc.trim()) {
      show_toast({
        type: "danger",
        title: "Invalid",
  ...
  const handle_update_country = async () => {
    ...
    try {
      ...
      const response = await api_update_country(
        edit_data,
        active_user?.username
      );
      if (response.success) {
        console_log(response.data);
        set_country_list((prev) =>
          prev.map((item) =>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to edit this Country. Once edited, it will be
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={handle_update_country}
              >
                Yes
              </Button>
              <Button
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Edit Country</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Edit Country</h1>
            </div>

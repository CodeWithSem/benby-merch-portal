...
} from "api/firestore_db/tbl_country_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_Country from "./functions/Create_Country";
...
const Country = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_COUNTRY", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const handle_get_country_list = async () => {
    ...
    const response = await api_get_country_list();
    if (response.success) {
      set_country_list(response.data);
    } else {
  ...
  const handle_truncate_country_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_country();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_country_list, set_filtered_country_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_country_list(indexed_data);
  }, [
    country_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : country_list.length) / select_option
  );
  ...
  const handle_create_new_country = () => {
    set_sub_page("create_new_country");
  };

  const handle_edit_country = (data) => {
    set_edit_data(data);
    set_sub_page("edit_country");
  };
  const handle_delete_country = (data) => {
    set_delete_data(data);
    set_display_modal("delete_country");
  };

  const handle_upload_country = () => {
    set_sub_page("upload_country");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_country_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_country}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    icon={FileUp}
                    icon_position="left"
                    on_click={handle_upload_country}
                  >
                    Upload
                  </Button>
                </div>
              ...
              <div className="p-5 sm:p-6 border-t">
                <div className="w-full border rounded-lg">
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_country_list}
                      ></Button>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    {loading_list ? (
                    ...
                    ) : filtered_country_list.length === 0 ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_country_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_edit_country(row)}
                                      >
                                        <Edit size={19} />
                                      </button>
      ...
      {sub_page === "upload_country" && (
        <Upload_Country
          handle_go_back={handle_go_back}
          handle_get_country_list={handle_get_country_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Country
        is_open={display_modal === "delete_country"}

...
import { api_delete_country } from "api/firestore_db/maintenance/general_structure/tbl_country_api";
...
const Delete_Country = ({
  ...
  const handle_delete_country = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_country(id);

      if (response.success) {
        set_country_list((prev) => prev.filter((item) => item.id !== id));
        show_status("success");
        close_modal();
      } else {
        show_status("error");
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to delete this Country.
            </p>
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
            ...
            <div className="flex justify-center gap-2 mt-4">
              <Button
                ...
                on_click={() => handle_delete_country(delete_data.id)}
              >
                Yes
              </Button>
              <Button
            ...
            <div className="w-full">
              <div className="w-full bg-white rounded-lg border">
                <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                  <h1 className="text-lg">Country Details</h1>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="col-span-full">
                      <Text_Field
                        label="Country Code"
                        type={"text"}
                        value={delete_data.country_code}
                        disabled
                      />
                    </div>
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.country_desc}
                        disabled
                      />
                    </div>
                  </div>

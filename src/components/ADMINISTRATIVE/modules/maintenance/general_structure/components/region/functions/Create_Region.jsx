...
import { api_create_region } from "api/firestore_db/tbl_region_api";
import {
  console_log,
  format_date_1,
  get_date_now,
...
const Create_Region = ({
  ...
  const handle_change_region_code = (value) => {
    set_new_data((prev) => ({
      ...prev,
      region_code: value,
    }));
  ...
  const handle_change_region_desc = (value) => {
    set_new_data((prev) => ({
      ...prev,
      region_desc: value,
    }));
  ...
  const handle_create_region = async () => {
    ...
    try {
      ...
      const response = await api_create_region(new_data, active_user?.username);
      if (response.success) {
        console_log(response.data);
        set_region_list((prev) => [...prev, response.data]);
        show_status("success");
        reset_new_data();
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100]">
          ...
          >
            ...
            <p className="w-full text-center text-sm leading-6 text-gray-500 dark:text-gray-400 pt-4">
              You are about to create a new Region. Once created, it will be
          ...
          <nav>
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li className="flex items-center gap-1.5 text-sm text-gray-500">
                <span>/</span>
                <span className="text-gray-800">Create New Region</span>
              </li>
            </ol>
          </nav>
          {/* - Breadcrumbs */}
        ...
        <div className="w-full bg-white rounded-lg border">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-5">
            <div className="flex items-center gap-3">
              ...
              ></Button>
              <h1 className="text-lg">Region Creation</h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x 0 lg:gap-x-5 gap-y-5 lg:grid-cols-3">
              <div>
                <Text_Field
                  ...
                  value={new_data.region_code || ""}
                  on_change={(e) => handle_change_region_code(e.target.value)}
                />
              </div>

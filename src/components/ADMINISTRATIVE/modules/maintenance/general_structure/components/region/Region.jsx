...
import Create_Region from "./functions/Create_Region";
...
const Region = ({ set_page }) => {
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_REGION", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
      }));
  ...
  const handle_get_region_list = async () => {
    ...
    const response = await api_get_region_list();
    if (response.success) {
      set_region_list(response.data);
    } else {
  ...
  const handle_truncate_region_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_region();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_region_list, set_filtered_region_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_region_list(indexed_data);
  }, [
    region_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : region_list.length) / select_option
  );
  ...
  const handle_create_new_region = () => {
    set_sub_page("create_new_region");
  };

  const handle_edit_region = (data) => {
    set_edit_data(data);
    set_sub_page("edit_region");
  };
  const handle_delete_region = (data) => {
    set_delete_data(data);
    set_display_modal("delete_region");
  };

  const handle_upload_region = () => {
    set_sub_page("upload_region");
  };
  ...
  return (
    <React.Fragment>
                ...
                <div className="flex gap-2">
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_region_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_region}
                  >
                    Create New Data
                  </Button>
                  <Button
                  ...
                  <div className="w-full md:flex md:justify-between p-4 gap-4">
                    <div className="flex items-center text-sm gap-2">
                      ...
                      <Button
                        ...
                        icon={RefreshCw}
                        icon_position="left"
                        on_click={handle_get_region_list}
                      ></Button>
                    </div>
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_region_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_edit_region(row)}
                                      >
                                        <Edit size={19} />
                                      </button>
      ...
      {sub_page === "upload_region" && (
        <Upload_Region
          handle_go_back={handle_go_back}
          handle_get_region_list={handle_get_region_list}
          show_toast={show_toast}
        />
      )}
      <Delete_Region
        is_open={display_modal === "delete_region"}

...
import {
  api_get_scon_list,
  api_set_scon_increment,
  api_truncate_scon,
} from "api/firestore_db/maintenance/general_structure/tbl_scon_api";
import {
  Search,
  ChevronDown,
  ChevronUp,
...
import Create_SCON from "./functions/Create_SCON";
import Edit_SCON from "./functions/Edit_SCON";
import Delete_SCON from "./functions/Delete_SCON";
import Upload_SCON from "./functions/Upload_SCON";
import Set_Increment_ID from "assets/elements/modals/Set_Increment_ID";

const HAS_FILTER = true;

const Storage_Condition = ({ set_page }) => {
  ...
  const [new_data, set_new_data] = useState({ ...def_scon_data });
  const [edit_data, set_edit_data] = useState({ ...def_scon_data });
  const [delete_data, set_delete_data] = useState({ ...def_scon_data });
  ...
  useEffect(() => {
    Get_TBL_INCREMENTAL_ID("TBL_STORAGE_CONDITION", (value) => {
      set_new_data((prev) => ({
        ...prev,
        id: value,
        scon_code: `SCON-${String(value).padStart(2, "0")}`,
      }));
  ...
  const columns = [
    ...
    { key: "scon_code", label: "Storage Condition Code", sortable: true },
    {
      key: "scon_desc",
  ...
  const [scon_list, set_scon_list] = useState([]);
  // ======================================================
  // id: 1,
  // scon_code: "SCON-001",
  ...
  const handle_get_scon_list = async () => {
    set_loading_list(true);
    const response = await api_get_scon_list();
    if (response.success) {
      set_scon_list(response.data);
    } else {
  ...
  useEffect(() => {
    handle_get_scon_list();
  }, []);

  const handle_truncate_scon_list = async () => {
    set_truncate_loading(true);
    const response = await api_truncate_scon();
    if (response.success) {
      show_toast({
        type: "success",
  ...
  const [filtered_scon_list, set_filtered_scon_list] = useState([]);
  ...
  useEffect(() => {
    ...
    set_filtered_scon_list(indexed_data);
  }, [
    scon_list,
    debounced_query,
    sort_by,
  ...
  const total_pages = Math.ceil(
    (debounced_query
      ...
      : scon_list.length) / select_option
  );
  ...
  const handle_create_new_scon = () => {
    set_sub_page("create_new_scon");
  };

  const handle_edit_scon = (data) => {
    set_edit_data(data);
    set_sub_page("edit_scon");
  };
  const handle_delete_scon = (data) => {
    set_delete_data(data);
    set_display_modal("delete_scon");
  };

  const handle_upload_scon = () => {
    set_sub_page("upload_scon");
  };
  ...
  return (
    <React.Fragment>
              ...
              <nav>
                <ol className="flex flex-wrap items-center gap-1.5">
                  ...
                  <li className="flex items-center gap-1.5 text-sm text-gray-500">
                    <span>/</span>
                    <span className="text-gray-800">Storage Condition</span>
                  </li>
                </ol>
              </nav>
              {/* - Breadcrumbs */}
            ...
            <div className="w-full bg-white rounded-lg border">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  ></Button>
                  <h1 className="text-lg">Storage Condition</h1>
                </div>
                <div className="flex gap-2">
                  ...
                  {active_user?.category === "DEV" && (
                    <Button
                      ...
                      on_click={handle_truncate_scon_list}
                    >
                      Truncate
                    </Button>
                  )}
                  <Button
                    ...
                    icon={PlusCircle}
                    icon_position="left"
                    on_click={handle_create_new_scon}
                  >
                    Create New Data
                  </Button>
                  <Button
                    ...
                    {loading_list ? (
                      ...
                      <table className="min-w-full">
                        ...
                        <tbody className="bg-white">
                          {filtered_scon_list.map((row, idx) => {
                            // + Cell Renderer
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <div className="relative group flex jusity-center items-center">
                                      <button
                                        ...
                                        onClick={() => handle_delete_scon(row)}
                                      >
                                        <Trash size={19} />
                                      </button>
      ...
      {sub_page === "upload_scon" && (
        <Upload_SCON
          handle_go_back={handle_go_back}
          handle_get_scon_list={handle_get_scon_list}
          show_toast={show_toast}
        />
      )}
      <Delete_SCON
        is_open={display_modal === "delete_scon"}
      ...
      <Set_Increment_ID
        ...
        api_set_increment_id={api_set_scon_increment}
      />
    </React.Fragment>
  );
};

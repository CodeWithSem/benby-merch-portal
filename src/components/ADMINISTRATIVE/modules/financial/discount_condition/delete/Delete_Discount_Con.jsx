...
import { api_delete_discount_con } from "api/firestore_db/financial/discount_condition/tbl_discount_con_api";

const Delete_Discount_Con = ({
  is_open,
  on_close,
  width = "max-w-[700px]",
  show_toast,
  ...
  const handle_delete_discount_con = async (id) => {
    try {
      set_delete_loading(true);

      const response = await api_delete_discount_con(id, show_toast);

      if (response.success) {
        set_discount_con_list((prev) => prev.filter((item) => item.id !== id));
        close_modal();
      }
    } catch (error) {
      console.error(error);
  ...
  return is_open ? (
    <React.Fragment>
            ...
            <div className="w-full">
              <div className="w-full bg-white rounded-lg border">
                <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                  <h1 className="text-lg">Discount Condition Details</h1>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.discount_con_code}
                        disabled
                      />
                    </div>
                    <div className="col-span-full">
                      <Text_Field
                        ...
                        type={"text"}
                        value={delete_data.discount_con_desc}
                        disabled
                      />
                    </div>
                  </div>
      ...
      <Confirm_Modal
        ...
        description_1="You are about to delete this Discount Condition."
        ...
        on_confirm={() => handle_delete_discount_con(delete_data.id)}
...
export default Delete_Discount_Con;
